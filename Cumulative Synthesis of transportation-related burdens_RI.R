...
#            (RI_MINORITY == "M" | RI_INCOME == "I" | RI_ENGLISH == "E")) %>% 
...
ri_blkgrps_sf <- ne_blkgrp_sf %>% 
  ...
  select(GEOID, NAME, STATE, bg_area_m2, totalpopE, minorityE, minority_pctE, under5E, pct_under5E, under18E, pct_under18E, over64E, pct_over64E, householdsE, eng_hhE, eng_limitE, eng_limit_pctE, age25upE, lthsE, pct_lthsE, povknownE, num2povE, pct2povE, RI_MINORITY, RI_INCOME, PM25_19, OZONE_19, DSLPM_19, CANCER_19, RESP_19, PTRAF_19) %>%
...
load("DATA/transport/RI/noTransit.Rds")
...
load("DATA/FEMA/RI/nfhza_census.Rds")
load("DATA/FEMA/RI/hevac_census.Rds")


# join burdens to a common blkgrp sf and create indicator variable
ri_blkgrps_sf <- ri_blkgrps_sf_noBus %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, NoTransitAccess = "Y") %>% 
  left_join(ri_blkgrps_sf,., by = "GEOID")

ri_blkgrps_sf <- headway80thblkgrps %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, headway80th_bus = "Y") %>% 
  left_join(ri_blkgrps_sf,., by = "GEOID")

ri_blkgrps_sf <- ri_blkgrp_walkability_sf %>% 
  as.data.frame() %>% 
  filter(NatWalkInd <= 5.8) %>% 
  transmute(GEOID = GEOID, leastWalkable = "Y") %>% 
  left_join(ri_blkgrps_sf,., by = "GEOID")

ri_blkgrps_sf <- lai_ri_blkgrp %>% 
  as.data.frame() %>% 
  filter(percent_rank(hh7_t) > 0.8) %>% 
  transmute(GEOID = GEOID, costBurdened = "Y") %>% 
  left_join(ri_blkgrps_sf,., by = "GEOID")

ri_blkgrps_sf <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
ri_blkgrps_sf <- ri_blkgrps_nfhza %>% 
  as.data.frame() %>% 
  group_by(GEOID) %>% 
  ...
  filter(percent_rank(sum_NewMinority/sum_NewPop) >= 0.8 | 
           ...
           percent_rank(sum_NewRI_LOWINC/sum_NewPop) >= 0.8 |
           percent_rank(sum_NewRI_MINORITIES/sum_NewPop) >= 0.8) %>% 
  transmute(GEOID = GEOID, nfhzaRisk = "F") %>% 
  left_join(ri_blkgrps_sf,., by="GEOID")

ri_blkgrps_sf <- ri_blkgrps_hevac %>% 
  as.data.frame() %>% 
  group_by(GEOID) %>% 
  mutate(NewEngHH = Proportion*eng_hhE,
         NewAge25Up = Proportion*age25upE) %>% 
...
ri_tracts_sf <- ri_tracts_sf_noBus %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, NoTransitAccess = "Y") %>% 
  left_join(ri_tracts_sf,., by = "GEOID") 
  
ri_tracts_sf <- headway80thtracts %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, headway80th_bus = "Y") %>% 
  left_join(ri_tracts_sf,., by = "GEOID")

ri_tracts_sf <- ri_tract_walkability_sf %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, leastWalkable = "Y") %>% 
  left_join(ri_tracts_sf,., by = "GEOID")

ri_tracts_sf <- lai_ri_tract %>% 
  as.data.frame() %>% 
  transmute(GEOID = GEOID, costBurdened = "Y") %>% 
  left_join(ri_tracts_sf,., by = "GEOID")

# join NFHZA flood hazard and hurricane evac records where percentage of pop of concern in flood or hurricane evac prone areas is 80th percentile
ri_tracts_sf <- ri_tracts_nfhza %>% 
  as.data.frame() %>% 
  group_by(GEOID) %>% 
...
# replace na's with "N"
ri_blkgrps_sf <- ri_blkgrps_sf %>% 
  mutate(across(NoTransitAccess:costBurdened, 
                ~replace_na(.x, "N"))
         )

ri_tracts_sf <- ri_tracts_sf %>% 
  mutate(across(NoTransitAccess:costBurdened, 
                ~replace_na(.x, "N"))
  )
...
# save output
saveRDS(ri_blkgrps_sf, file = "DATA/ri_blkgrps_sf_CUM.Rds")
...
cum_burden_df <- ri_blkgrps_sf %>% 
  ...
  summarize(across(c(totalpopE,householdsE,minorityE,num2povE,eng_limitE,under5E,under18E,over64E,lthsE,RI_MINORITIES,RI_LOWINC), ~ sum(.x, na.rm = T), .names = "{col}")) %>% 
  pivot_longer(., cols = totalpopE:RI_LOWINC, names_to = "Group") %>% 
  ...
  mutate(Group = case_when(
    ...
    Group == "RI_MINORITIES" ~ "RI Minority",
...
# save to csv
write_csv(cum_burden_df,"tables/RI_cum_burden.csv")
...
cum_burden_df %>% 
  ...
  labs(x = "", y = "Percentage", fill = "Burden\nCategories",
       title = "Percentage of Rhode Island Population within\nCumulative Burden Categories") + 
  theme_light() +
...
# save plot as png
ggsave("images/RI_CUM_BURDEN_Graph.png")
...
m <- ri_blkgrps_sf %>% 
  ...
  tm_fill(col = "BurdenCount", palette = "YlOrRd", 
          title = "Number of\nBurden\nCategories") +
  tm_shape(ri_blkgrps_sf) + tm_borders(lwd = 0.1) +
...
tmap_save(m, "images/RI_CUM_BURDEN_map.png",
          height = 7, width = 8, units = "in", dpi = 600)

# replicate BG map for MA EJ only 
mEJ <- ri_blkgrps_sf %>% 
  filter(RI_MINORITY == "M" | RI_INCOME == "I") %>% 
  filter(BurdenCount > 0) %>% 
  tm_shape(., unit = "mi") + 
  tm_fill(col = "BurdenCount", palette = "YlOrRd", 
          title = "Number of\nBurdens") +
  # tm_shape(ri_blkgrps_sf) + tm_borders(lwd = 0.1) +
  tm_shape(ri_towns_sf) + tm_borders(lwd = 0.1) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) +
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(I95roadSegment) +
...
tmap_save(mEJ, "images/RI_CUM_BURDEN_EJ_map.png",
          height = 7, width = 8, units = "in", dpi = 600)
...
EJbgs <- ri_blkgrps_sf %>% 
  filter(RI_MINORITY == "M" | RI_INCOME == "I") %>% 
  nrow()
EJbgs1_4 <- ri_blkgrps_sf %>% 
  filter(BurdenCount > 0 & 
           (RI_MINORITY == "M" | RI_INCOME == "I")) %>% 
  nrow()
# wha percentage of EJ BGs are high burden?
EJbgs1_4/EJbgs*100
# what percentage of all high burden BGs are these?
hiBurdenBGs <- ri_blkgrps_sf %>% 
  filter(BurdenCount > 0) %>% 
  nrow()
hiBurdenBGs/nrow(ri_blkgrps_sf)*100
...
burden_types_df %>% kable(longtable = T, booktabs = T, 
  ...
  kable_styling(latex_options = c("repeat_header","striped"))
# save as csv
write_csv(burden_types_df,"tables/RI_burden_types.csv")
...
town_pops <- get_acs(geography = "county subdivision", 
                     ...
                     state = "RI", output = "wide", year = 2018) %>% 
...
# save as csv
write_csv(burdens_town_df, "tables/RI_burdens_town.csv")


# table showing EJ towns with block groups with 3 or 4 burdens
burdens_EJtown_df <- ri_blkgrps_sf %>%
  filter(RI_MINORITY == "M" | RI_INCOME == "I") %>%
...
burdens_EJtown_df %>% kable(longtable = T, booktabs = T,
  ...
  kable_styling(latex_options = c("repeat_header","striped")) 
# save as csv
write_csv(burdens_EJtown_df, "tables/RI_burdens_EJtown.csv")

# how many EJ municipalities are there?
EJtowns <- ri_blkgrps_sf %>%
  select(-NAME) %>% 
  filter(RI_MINORITY == "M" | RI_INCOME == "I") %>%
  st_centroid(.) %>% 
  st_intersection(.,ri_towns_sf) %>% 
  ...
  summarize(`Block Groups` = n())
# save as csv
write_csv(EJtowns, "tables/RI_EJtowns.csv")
...
senate_districts <- st_read("DATA/shapefiles/RI_Senate_Districts-shp",
                            "Senate_Districts") %>% 
  st_transform(., crs = 2840) %>% 
  st_make_valid()

# create pop totals for each district
senate_names_pops <- ri_blkgrps_sf %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
...
burdens_senate_df <- ri_blkgrps_sf %>%
  filter(BurdenCount >= 3) %>% 
  select(GEOID, BurdenCount, totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.)),
         BurdenCount = as.character(BurdenCount)) %>% 
  ...
  summarize(Pop = sum(NewPop)) %>% 
  pivot_wider(id_cols = NAMELSAD, names_from = BurdenCount, 
              values_from = Pop) %>% 
  as.data.frame() %>% # change from rowwise_df back to regular df
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
  transmute(`Senate District` = NAMELSAD, `3 Burdens` = `3`, 
            `4 Burdens` = `4`) %>% 
  rowwise() %>% 
  mutate(`3+ Burdens` = sum(c_across(`3 Burdens`:`4 Burdens`))) %>% 
  left_join(., senate_names_pops, by = c("Senate District" = "NAMELSAD")) %>% 
  mutate(`Pct 3 Burdens` = `3 Burdens`/totalpopE*100, .after = `3 Burdens`) %>% 
  mutate(`Pct 4 Burdens` = `4 Burdens`/totalpopE*100, .after = `4 Burdens`) %>% 
  mutate(`Pct 3+ Burdens` = `3+ Burdens`/totalpopE*100, 
         .after = `3+ Burdens`) %>% 
...
burdens_senate_df <- senate_districts %>% 
  as.data.frame() %>% 
  anti_join(., burdens_senate_df, by = c("NAMELSAD" = "Senate District")) %>% 
  transmute(`Senate District` = NAMELSAD) %>% 
...
burdens_senate_df %>% kable(longtable = T, booktabs = T,
  ...
  add_header_above(c(" " = 1, "3 Burdens" = 2, "4 Burdens" = 2, 
                     "3+ Burdens" = 2))
# save as csv
write_csv(burdens_senate_df, "tables/RI_burdens_senate.csv")

# create table of BG counts by district
burdensCnt_senate_df <- ri_blkgrps_sf %>%
  transmute(BurdenCount = as.character(BurdenCount)) %>% 
  st_centroid(.) %>% 
  st_intersection(.,senate_districts) %>% 
  as.data.frame() %>% 
  group_by(BurdenCount, NAMELSAD) %>%
  summarize(`Block Groups` = n()) %>% 
  pivot_wider(id_cols = NAMELSAD, names_from = BurdenCount, 
              values_from = `Block Groups`) %>% 
  mutate(across(everything(), ~replace_na(.x, 0))) %>% 
...
burdensCnt_senate_df %>% kable(longtable = T, booktabs = T,
  ...
  kable_styling(latex_options = c("repeat_header","striped")) 
# save as csv
write_csv(burdensCnt_senate_df, "tables/RI_burdensCnt_senate.csv")

# map of senate districts with block groups meeting 3 - 4 burdens
tmap_mode("plot")
m <- senate_districts %>% 
  right_join(.,burdens_senate_df, by = c("NAMELSAD" = "Senate District")) %>% 
...
tmap_save(m, "images/RI_CUM_BURDEN_SENATE_map.png",
          height = 7, width = 8, units = "in", dpi = 600)


# table showing block groups by state house district that have 3 - 4 burden categories
house_districts <- st_read("DATA/shapefiles/RI_House_Districts-shp",
                            "House_Districts") %>% 
  st_transform(., crs = 2840) %>% 
  st_make_valid()

# create pop totals for each district
house_names_pops <- ri_blkgrps_sf %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
...
# calculate total and pct of pop for each district
burdens_house_df <- ri_blkgrps_sf %>%
  filter(BurdenCount >= 3) %>% 
  select(GEOID, BurdenCount, totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.)),
         BurdenCount = as.character(BurdenCount)) %>% 
  ...
  as.data.frame() %>% # change from rowwise_df back to regular df
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
  transmute(`House District` = NAMELSAD, `3 Burdens` = `3`, 
            `4 Burdens` = `4`) %>% 
  rowwise() %>% 
  mutate(`3+ Burdens` = sum(c_across(`3 Burdens`:`4 Burdens`))) %>% 
  left_join(., house_names_pops, by = c("House District" = "NAMELSAD")) %>% 
  mutate(`Pct 3 Burdens` = `3 Burdens`/totalpopE*100, .after = `3 Burdens`) %>% 
  mutate(`Pct 4 Burdens` = `4 Burdens`/totalpopE*100, .after = `4 Burdens`) %>% 
  mutate(`Pct 3+ Burdens` = `3+ Burdens`/totalpopE*100, 
         .after = `3+ Burdens`) %>% 
...
burdens_house_df <- house_districts %>% 
  as.data.frame() %>% 
  anti_join(., burdens_house_df, by = c("NAMELSAD" = "House District")) %>% 
  transmute(`House District` = NAMELSAD) %>% 
...
burdens_house_df %>% kable(longtable = T, booktabs = T,
  ...
  add_header_above(c(" " = 1, "3 Burdens" = 2, "4 Burdens" = 2, 
                     "3+ Burdens" = 2))
# save as csv
write_csv(burdens_house_df, "tables/RI_burdens_house.csv")

# create table of BG counts by district
burdensCnt_house_df <- ri_blkgrps_sf %>%
  transmute(BurdenCount = as.character(BurdenCount)) %>% 
  st_centroid(.) %>% 
  st_intersection(.,house_districts) %>% 
  as.data.frame() %>% 
...
burdensCnt_house_df %>% kable(longtable = T, booktabs = T,
  ...
  kable_styling(latex_options = c("repeat_header","striped")) 
# save as csv
write_csv(burdensCnt_house_df, "tables/RI_burdensCnt_house.csv")

# map of house districts with block groups meeting 3 - 4 burdens
tmap_mode("plot")
m <- house_districts %>% 
  right_join(.,burdens_house_df, by = c("NAMELSAD" = "House District")) %>% 
  tm_shape(., unit = "mi", bbox = senate_districts) + 
  tm_fill(col = "red", alpha = 0.5) +
...
tmap_save(m, "images/RI_CUM_BURDEN_HOUSE_map.png",
          height = 7, width = 8, units = "in", dpi = 600)
...
#   tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) +
#   tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
#   tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 1, alpha = 0.5) +
#   tm_shape(ri_highways2nd) + tm_lines(col = "seashell4", lwd = 1, alpha = 0.5) +
#   tm_shape(I95roadSegment) +
