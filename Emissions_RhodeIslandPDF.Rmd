---
title: "Transportation Burdens and Emissions in Rhode Island"
...
```
\pagebreak
# Analysis of transportation-related emissions in Rhode Island

This is an analysis of transportation-related emissions and related externalities in Rhode Island.

## PM~2.5~ in Rhode Island
...
PM~2.5~ levels vary significantly across Rhode Island, with highest concentrations in northern Rhode Island, centered around Providence (see Figure \@ref(fig:mapPM25RI) below). 
...
# Create data subsets of Rhode Island
ri_blkgrp_sf <- ne_blkgrp_sf %>% 
  filter(STATE == "Rhode Island") %>% 
  st_transform(., crs = 2840)
ri_blkgrp_sf90 <- ne_blkgrp_sf90 %>% 
  filter(STATE == "Rhode Island") %>% 
  st_transform(., crs = 2840)
...
```{r mapPM25RI, fig.align = "center", fig.cap="Map of 2016 annual PM2.5 concentrations across Rhode Island at Census Block Group level."}
# Map of PM2.5 across Rhode Island
m1 <- tm_shape(ri_blkgrp_sf, unit = "mi") + 
...
tmap_save(m1, "DATA/EJSCREEN/ri_pm25.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_pm25.png")
```

PM~2.5~ concentrations exhibit spatial clustering of both hot spots (i.e. geographic clusters of high values) and cold spots (i.e. geographic clusters of lower values). The map below (Figure \@ref(fig:hotspotPM25RI)) shows statistically significant PM~2.5~ hot spots. 

```{r hotspotPM25RI, fig.align = "center", fig.cap="Hot spot map of 2016 annual PM2.5 concentrations at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>% 
...
tmap_save(m2, "DATA/EJSCREEN/ri_pm25hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_pm25hotspot.png")
```

There is a statistically significant hot spot of PM~2.5~ around Providence and Pawtucket north to Lincoln. Warm clusters extend across the northern half of the state. 

These PM~2.5~ levels vary significantly across the state (see Table \@ref(tab:statsPM) and Figure \@ref(fig:boxplotPM25RI)). See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality. 
...
The Block Group with highest PM~2.5~ concentration value in the state is found in `r ri_town_maxPM` and the lowest is found in `r ri_town_minPM`.

Figure \@ref(fig:boxplotPM25RI) is a boxplot of PM~2.5~ concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median PM~2.5~ concentration for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$PM25_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. Note that a large cluster of dots is concentrated on the far right, at the upper end of concentration values. Most of these are in northern Rhode Island. 

```{r boxplotPM25RI, fig.align = "center", fig.cap="Boxplot of 2016 annual PM2.5 concentrations at Census Block Group level. 1 dot = 1 Block Group."}
...
ri_blkgrp_sf %>% 
  ...
  ggtitle(expression(atop(paste(PM[2.5], " Annual Concentrations in Rhode Island,"), "2016"))) +
  theme_minimal() +
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {abs(round(mean(ri_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1))}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {abs(round(min(ri_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1))}`%, have been in the southern half of the state. 

```{r mapPM25RIchange, fig.align = "center", fig.cap="Map of percent change in annual PM2.5 concentrations across Rhode Island between 2011 and 2016 at Census Block Group level."}
# Map of PM2.5 change across Rhode Island
m3 <- tm_shape(ri_blkgrp_sf, unit = "mi") + 
...
tmap_save(m3, "DATA/EJSCREEN/ri_pm25change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_pm25change.png")
```

Figure \@ref(fig:slopegraphPM25RI) below compares the average annual PM~2.5~ concentrations for the state and the region between 2011 and 2016. The region and the state both showed significant declines since 2011, although note that concentrations in Rhode Island are now above the rest of the region. 

```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted concentrations of PM2.5 between 2011 and 2016 for Rhode Island and New England."}
# Slope graph of PM2.5 by state and region between 2011 and 2016
# Create df of 2019 actual values
PM19wSTAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
PM15wSTAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
### PM~2.5~ in Rhode Island and Priority Populations

In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for priority populations relative to average PM~2.5~ concentrations for the state. For example, People of Color, as identified by Rhode Island Environmental Justice policy, are exposed to PM~2.5~ concentrations that are 6.8% above concentrations for the state as a whole. Similarly, limited English speaking households are exposed to concentrations over 5.2% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of PM~2.5~ 1.5% below the state average. 

```{r barplotPM25PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for priority populations in Rhode Island relative to the state average."}
# Pop Weighted avg of PM2.5 for all Groups in Rhode Island relative to state average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>%
  mutate(RI_People of Color = if_else(RI_MINORITY == "M", totalpopE,0)) %>%
  mutate(RI_People of Color = replace_na(RI_People of Color,0)) %>%
  ...
  dplyr::select(totalpopE,
                ...
                RI_People of Color,
                PM25_19) %>% 
  gather(key = Group, value = Pop, totalpopE:RI_People of Color) %>% 
  group_by(Group) %>% 
  ...
  transmute(`People of Color` = (minorityE/PM25Mean - 1)*100,
         ...
         `RI Low Income` = ( RI_LOWINC/PM25Mean - 1)*100,
         `RI POC` = ( RI_People of Color/PM25Mean - 1)*100) %>%
  ...
  coord_flip() + 
  labs(x = "", y = "", title = expression(atop(paste("Population-Weighted ", PM[2.5], " Exposure"), "(relative to Rhode Island average)"))) + 
  theme_light() +
  ...
  geom_text(aes(x = "Low Income", y = 3.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -1.4, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 1.5, yend = 3.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-1,3))

ggsave("images/RI_PM25_graph.png")
```
...
# Pop Weighted avg of PM2.5 EXPOSURE CHANGE 2011 - 2016 for all Groups in Rhode Island relative to NE average. 
# Create df of 2019 actual values
PM19wAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>%
  # filter(STATE == "Massachusetts") %>%
...
PM15wAvgs_actual <- ri_blkgrp_sf %>% 
  ...
  mutate(Year = 2011)
# Create regional average benchmark
pm15RIavg <- mean(ri_blkgrp_sf$pm_15, na.rm = TRUE)
pm19RIavg <- mean(ri_blkgrp_sf$PM25_19, na.rm = TRUE)
pmRegionalAvg_actual <- data.frame(
  Group = c("STATE AVG", "STATE AVG"),
  PM25wMean = c(pm15RIavg,pm19RIavg),
  Year = c(2011,2016)
)
#rbind tables
...
ggsave("images/RI_PM25_slope.png")
```
...
Ozone (O~3~) levels vary significantly across Rhode Island, with highest concentrations in southwest Rhode Island, and declining toward the north and east (see Figure \@ref(fig:mapO3RI) below).[^NONATTAINMENT] 

```{r mapO3RI, fig.align = "center", fig.cap="Map of 2016 Ozone summer seasonal average of daily maximum 8-hour concentration in air in parts per billion across Rhode Island at Census Block Group level."}

# Map of O3 across Rhode Island
m4 <- tm_shape(ri_blkgrp_sf, unit = "mi") + 
...
tmap_save(m4, "DATA/EJSCREEN/ri_O3.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_O3.png")
```

Ozone (O~3~) concentrations exhibit spatial clustering of both hot spots (i.e. geographic clusters of high values) and cold spots (i.e. geographic clusters of lower values). The map below (Figure \@ref(fig:hotspotO3RI)) shows statistically significant Ozone (O~3~) hot spots.

```{r hotspotO3RI, fig.align = "center", fig.cap="Hot spot map of 2016 Ozone concentrations at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>% 
...
tmap_save(m5, "DATA/EJSCREEN/ri_O3hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_O3hotspot.png")
```

There is a statistically significant hot spot of Ozone (O~3~) centered on Westerly. A warm cluster extends east to Charlestown and north to Richmond. 
...
Outlier <- quantile(ri_blkgrp_sf$OZONE_19,.75, na.rm = TRUE) + 1.5*(quantile(ri_blkgrp_sf$OZONE_19,.75, na.rm = TRUE) - quantile(ri_blkgrp_sf$OZONE_19,.25, na.rm = TRUE))

# identify outliers
ri_town_outlier <- ri_blkgrp_sf %>% 
  ...
  pull(., NAME)
```
The Block Group with highest O~3~ concentration value in the state is found in `r ri_town_max` and the lowest is found in `r ri_town_min`. See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

Figure \@ref(fig:boxplotO3) is a boxplot of O~3~ concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median O~3~ concentration for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$OZONE_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high or low values relative to the rest of the state. In this case, outliers would be represented by ozone values greater than `r Outlier`, which occur in `r glue::glue_collapse(ri_town_outlier,", ", last = ", and ")`.   
...
Since 2011, Ozone (O~3~) levels have decreased slightly across the state, on average by `r {abs(round(mean(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1))}`%. These have changes not been uniform (see Figure \@ref(fig:mapO3RIchange) below). The greatest declines, up to `r abs(round(min(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1))`%, have been along the southeast of the state. By contrast, increases in summer ozone concentrations of up to `r round(max(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`% appear in the extreme north around Woonsocket.

```{r mapO3RIchange, fig.align = "center", fig.cap="Map of percent change in summer seasonal Ozone concentrations across Rhode Island between 2011 and 2016 at Census Block Group level."}
# Map of O3 change across Rhode Island
m6 <- tm_shape(ri_blkgrp_sf, unit = "mi") + 
...
tmap_save(m6, "DATA/EJSCREEN/ri_O3change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_O3change.png")
```

Figure \@ref(fig:slopegraphO3RI) below compares the average summer Ozone (O~3~) concentrations for the state and for the region between 2011 and 2016. Rhode Island's average concentrations have decreased, but remain higher than the region. 

```{r slopegraphO3RI, fig.align = "center", fig.cap="Change in summer average ozone concentrations between 2011 and 2016 for Rhode Island and New England."}
# Slope graph of O3 by state and region between 2011 and 2016
# Create df of 2019 actual values
O319wSTAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  ...
  mutate(Year = 2016)
# Create df of 2015 values
O315wSTAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
### Ozone (O~3~) in Rhode Island and Priority Populations

In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for priority populations relative to average Ozone concentrations for the state. For example, persons over age 64 in Rhode Island are exposed to summer Ozone concentrations that are approximately 0.03% above concentrations for the state as a whole. By contrast, all other priority populations are, on average, exposed to concentrations of Ozone at or below the state average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for priority populations in Rhode Island relative to the state average."}
# Pop Weighted avg of O3 for all Groups in Rhode Island relative to NE average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>%
  ...
  transmute(`People of Color` = (minorityE/O3Mean - 1)*100,
         ...
         `RI Low Income` = ( RI_LOWINC/O3Mean - 1)*100,
         `RI POC` = ( RI_People of Color/O3Mean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
  ...
  geom_point(color = "blue", size = 4, alpha = 0.8) +
  coord_flip() + 
  labs(x = "", y = "", title = expression(atop(paste("Population-Weighted Ozone (", O[3], ") Exposure"), "(relative to Rhode Island average)"))) + 
  theme_light() +
  theme(panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()) +
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 0.3, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -0.6, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 0.1, yend = 0.3), arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-1,1))

ggsave("images/RI_O3_graph.png")
```

Like the region as a whole, these populations have also experienced changes in exposure since 2011. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvgRI). All priority populations have experienced a decrease in population-weighted exposure to Ozone. 

```{r slopegraphO3PopAvgRI, fig.align = "center", fig.cap="Change in population-weighted exposure to summer ozone for priority populations between 2011 and 2016."}
# Pop Weighted avg of O3 EXPOSURE CHANGE 2011 - 2016 for all Groups in Rhode Island relative to NE average. 
# Create df of 2019 actual values
O319wAvgs_actual <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(minorityE,
                num2povE,
...
O315wAvgs_actual <- ri_blkgrp_sf %>% 
  ...
  mutate(Year = 2011)
# Create regional average benchmark
O315RIavg <- mean(ri_blkgrp_sf$o3_15, na.rm = TRUE)
O319RIavg <- mean(ri_blkgrp_sf$OZONE_19, na.rm = TRUE)
O3RegionalAvg_actual <- data.frame(
  Group = c("STATE AVG", "STATE AVG"),
  O3wMean = c(O315RIavg,O319RIavg),
  Year = c(2011,2016)
)
#rbind tables
...
ggsave("images/RI_O3_slope.png")
```


\pagebreak
## Carbon Dioxide (CO~2~) in Rhode Island
...
On-road CO~2~ emissions closely follow major roadways across the state (see Figure \@ref(fig:mapCO2RI) below). 

```{r mapCO2RI, fig.align = "center", fig.cap="Map of 2017 Carbon Dioxide (CO2) annual on-road emissions in metric tons per square kilometer across Rhode Island at Census Block Group level."}

# Map of CO2 across Rhode Island
m7 <- ri_blkgrp_sf %>% 
...
tmap_save(m7, "DATA/EJSCREEN/ri_CO2.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_CO2.png")
```

Significant hot spots, or clusters of high CO~2~ emissions, appear around Providence and East Providence (see Figure \@ref(fig:hotspotCO2RI) below).

```{r hotspotCO2RI, fig.align = "center", fig.cap="Hot spot map of 2017 CO2 emissions at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>% 
  mutate(onroad_2017 = kgco2_2017/1000/(bg_area_m2/1000000)) %>%
...
tmap_save(m8, "DATA/EJSCREEN/ri_CO2hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_CO2hotspot.png")
```

CO~2~ emissions vary across the state with significant outliers (see Table \@ref(tab:statsCO2RI)).   

```{r statsCO2RI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Annual 2017 CO2 emissions (mtons) at Census Block Group level and by state."}
ri_blkgrp_sf %>% 
...
The Block Group with highest CO~2~ emissions value in the state is found in `r ri_town_maxCO2` and the lowest is found in `r ri_town_minCO2`. See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(ri_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(ri_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(ri_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2RIchange) below). Declines have occurred throughout the state. Increased emissions have occurred around Little Compton, West Greenwich, and Narragansett.

```{r mapCO2RIchange, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across Rhode Island between 1990 and 2017 at Census Block Group level."}
# Map of CO2 change across Rhode Island
m9 <- ri_blkgrp_sf %>% 
...
tmap_save(m9, "DATA/EJSCREEN/ri_CO2change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_CO2change.png")
```

At the state level the differences in these changes are also apparent, although less extreme than at the Census block group level. Table \@ref(tab:changeCO2RI) shows summary statistics of CO~2~ emissions for the state as a whole. Figure \@ref(fig:linegraphCO2RI) shows annual CO~2~ emissions between 1990 and 2017. 

```{r changeCO2RI, fig.align = "center", fig.cap="Annual 2017 CO2 emissions (mtons) and Percent Change by state at Census Block Group level."}
library(tidycensus)
...
download.file("https://www2.census.gov/census_1990/CD90_1A_1/stf1a0ri.dbf",
              destfile = "DATA/stf1a0ri.dbf", method = "curl")
# reformat df to work in code below
ne_states_df90 <- read.dbf("DATA/stf1a0ri.dbf", as.is = TRUE) %>% 
...
ri_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total Rhode Island ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```

Rhode Island saw dramatic growth in CO~2~ emissions between 1990 and 2000 and again between 2005 and 2007. Since 2007, however, total emissions have declined significantly. This is reflected in the decline in per capita emissions since 1990, indicating that emissions growth has been slower than population growth (see last three columns in Table \@ref(tab:changeCO2RI)).  


\pagebreak
## Diesel Particulate Matter in Rhode Island
...
DPM emissions are concentrated around greater Providence metro region, also around Newport and Woonsocket (see Figure \@ref(fig:mapDPMRI) below). 

```{r mapDPMRI, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Rhode Island at Census Block Group level."}

# Map of DPM across Rhode Island
m10 <- ri_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m10,"DATA/EJSCREEN/ri_DPM.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_DPM.png")
```

There are significant spatial clusters of high DPM concentrations in Providence (see Figure \@ref(fig:hotspotDPMRI) below).

```{r hotspotDPMRI, fig.align = "center", fig.cap="Hot spot map of 2014 Diesel Particulate Matter emissions at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>%
...
tmap_save(m11, "DATA/EJSCREEN/ri_DPMhotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_DPMhotspot.png")
```

DPM concentrations vary across the state (see Table \@ref(tab:statsDPMRI) and Figure \@ref(fig:boxplotDPMRI) below). See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

```{r statsDPMRI, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Rhode Island at Census Block Group level."}
ri_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
The Block Group with highest DPMr emissions value in the state is found in `r ri_town_maxDPM` and the lowest is found in `r ri_town_minDPM`.

Figure \@ref(fig:boxplotDPMRI) is a boxplot of DPM concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median DPM concentration for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$DSLPM_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.  

```{r boxplotDPMRI, fig.align = "center", fig.cap="Boxplot of 2017 Carbon Dioxoide on-road emissions in metric tons per square kilometer by state at Census Block Group level. 1 dot = 1 Block Group."}
...
### Diesel Particulate Matter in Rhode Island and Priority Populations

In addition to variations in the general geography of Diesel Particulate Matter concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvgRI) below shows population-weighted exposures for priority populations relative to average Diesel Particulate Matter concentraions for the state. For example, People of Color in Rhode Island are exposed to  Diesel Particulate Matter concentrations that are more than 50% above concentrations for the region as a whole. Similarly, low income persons (following Rhode Island's definition) are exposed to concentrations more than 40% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of Diesel Particulate Matter more than 12% below the state average. 

```{r barplotDPMPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of diesel particulate matter across Rhode Island relative to the state average."}
# Pop Weighted avg of DPM for all Groups in Rhode Island relative to NE average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>%
  ...
  transmute(`People of Color` = (minorityE/DPMMean - 1)*100,
         ...
         `RI Low Income` = (RI_LOWINC/DPMMean - 1)*100,
         `RI POC` = (RI_People of Color/DPMMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 25, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -12, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 6, yend = 25), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-20,23))

ggsave("images/RI_DPM_graph.png")
```

There is a strong positive correlation between the proportions of People of Color and ambient concentrations of Diesel Particulate Matter, and moderate to strong correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixDPMPopAvg) in Appendix B).


\pagebreak
## Air Toxics Cancer Risk in Rhode Island
...
Air toxics cancer risks are concentrated in and around Providence within the I-295 belt (see Figure \@ref(fig:mapATCNE) below).

```{r mapATCNE, fig.align = "center", fig.cap="Map of 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Rhode Island at Census Block Group level."}
...
tmap_save(m12, "DATA/EJSCREEN/ri_Cancer.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_Cancer.png")
```

There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics centered on Providence (see Figure \@ref(fig:hotspotATCRI) below).

```{r hotspotATCRI, fig.align = "center", fig.cap="Hot spot map of 2014 lifetime cancer risk from inhalation of air toxics at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>%
...
tmap_save(m13, "DATA/EJSCREEN/ri_CancerHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_CancerHotspot.png")
```
\pagebreak
...
```{r statsATCNE, fig.align = "center", fig.cap="Annual 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Rhode Island at Census Block Group level."}
ri_blkgrp_sf %>%
  as.data.frame() %>% 
...
# extract block groups centroid with highest and lowest CO2 values, intersect with town polygons, and pull out town name
ri_town_maxCancer <- ri_blkgrp_sf %>% 
...
The Block Group with highest cancer risk value in the state is found in `r ri_town_maxCancer` and the lowest is found in `r ri_town_minCaner`. See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

Figure \@ref(fig:boxplotATCNE) is a boxplot of cancer risk by Block Group. The box represents risk values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median lifetime cancer risk value for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$CANCER_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. 

```{r boxplotATCNE, fig.align = "center", fig.cap="Boxplot of lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Rhode Island by state at Census Block Group level. 1 dot = 1 Block Group."}
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in Rhode Island and Priority Populations

In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgRI) below shows population-weighted exposures for priority populations relative to average cancer risk for the state. For example, People of Color, as identified by Rhode Island Environmental Justice policy, experience lifetime cancer risks from inhalation of air toxics that are more than 14% above the state as a whole. Similarly, low income persons, as identified by Rhode Island Environmental Justice policy, experience lifetime cancer risks more than 10% above the state average. By contrast, persons over age 64 are, on average, exposed to risk 3.2% below the state average. 

```{r barplotATCPopAvgRI, fig.align = "center", fig.cap="Population-weighted average lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Rhode Island at Census Block Group level relative to the state average."}
# Pop Weighted avg of DPM for all Groups in Rhode Island relative to NE average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 9, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -3, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 3, yend = 10), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-5,15))

ggsave("images/RI_Cancer_graph.png")
```

There is a strong positive correlation between the proportions of People of Color and cancer risk, and moderate to strong correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixATCPopAvg) in Appendix B).


\pagebreak
## Respiratory Hazard Index in Rhode Island
...
Higher respiratory hazard indices are concentrated in and around Providence (see Figure \@ref(fig:mapRHIRI) below). 

```{r mapRHIRI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Rhode Island at Census Block Group level."}

# Map of RHI across Rhode Island
m14 <- ri_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m14, "DATA/EJSCREEN/ri_RespHzrd.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_RespHzrd.png")
```

There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics in Providence (see Figure \@ref(fig:hotspotRHIRI) below).

```{r hotspotRHIRI, fig.align = "center", fig.cap="Hot spot map of 2014 respiratory hazard index from inhalation of air toxics at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>%
...
tmap_save(m15, "DATA/EJSCREEN/ri_RespHzrdHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_RespHzrdHotspot.png")
```

Respiratory hazard indices from inhalation of air toxics vary across the state (see Table \@ref(tab:statsRHIRI) and Figure \@ref(fig:boxplotRHIRI) below).   

```{r statsRHIRI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Rhode Island at Census Block Group level."}
ri_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
The Block Group with highest respiratory hazard index value in the state is found in `r ri_town_maxRESP` and the lowest is found in `r ri_town_minRESP`. See Table \@ref(tab:PollutantByTown) in Appendix B for values by municipality.

Figure \@ref(fig:boxplotRHIRI) is a boxplot of respiratory hazard indices by Block Group. The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.   

```{r boxplotRHIRI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Rhode Island by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
### Respiratory Hazard Index from Inhalation of Air Toxics in Rhode Island and Priority Populations

In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for priority populations relative to the average hazard index for the state. For example, People of Color, as identified by Rhode Island Environmental Justice policy, experience respiratory hazard indices from inhalation of air toxics that are over 18% above the state as a whole. Similarly, People of Color, as identified by Rhode Island Environmental Justice policy, experience lifetime hazard index values more than 14% above the state average. By contrast, persons over age 64 are, on average, exposed to respiratory hazard indices over 4% below the state average. 

```{r barplotRHIPopAvgRI, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Rhode Island at Census Block Group level relative to the state average."}
# Pop Weighted avg of RHI for all Groups in Rhode Island relative to NE average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>%
  ...
  transmute(`People of Color` = (minorityE/RESPMean - 1)*100,
         ...
         `RI Low Income` = (RI_LOWINC/RESPMean - 1)*100,
         `RI POC` = (RI_People of Color/RESPMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 10, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -4, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 3, yend = 10), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-6,20))

ggsave("images/RI_Resp_graph.png")
```

There is a strong positive correlation between the proportions of People of Color and respiratory hazard index, and moderate to strong positive correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixRHIPopAvg) in Appendix B).


\pagebreak
## Traffic Proximity and Volume in Rhode Island
...
Exposure to high annual daily traffic volume is concentrated along major traffic corridors, as well as within larger municipalities (see Figure \@ref(fig:mapTPVRI) below). 

```{r mapTPVRI, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Rhode Island at Census Block Group level."}

# Map of TPV across Rhode Island
m16 <- ri_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m16, "DATA/EJSCREEN/ri_Traffic.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_Traffic.png")
```

There appear to be significant spatial clusters of traffic proximity and volume in Providence and Pawtucket (see Figure \@ref(fig:hotspotTPVRI) below).

```{r hotspotTPVRI, fig.align = "center", fig.cap="Hot spot map of 2017 traffic proximity and volume at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>%
...
tmap_save(m17, "DATA/EJSCREEN/ri_TrafficHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ri_TrafficHotspot.png")
```

Traffic proximity and volume exposure vary across the state (see Table \@ref(tab:statsTPVRI) and Figure \@ref(fig:boxplotTPVRI) below).   

```{r statsTPVRI, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Rhode Island at Census Block Group level."}
ri_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
# extract block groups centroid with highest and lowest values, intersect with town polygons, and pull out town name
ri_town_maxTraffic <- ri_blkgrp_sf %>% 
...
ri_town_minTraffic <- ri_blkgrp_sf %>% 
...
The Block Group with highest Traffic Proximity and Volume value in the state is found in `r ri_town_maxTraffic` and the lowest is found in `r ri_town_minTraffic`. See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

Figure \@ref(fig:boxplotTPVRI) is a boxplot of traffic proximity and volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPVRI, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Rhode Island at Census Block Group level."}
...
### Traffic Proximity and Volume Exposure in Rhode Island and Priority Populations

In addition to variations in the general geography of the traffic proximity and volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for priority populations relative to average traffic proximity and volume for the state. For example, People of Color, as identified by Rhode Island Environmental Justice policy, and limited English speaking households are exposed to Traffic Proximity and Volume more than twice the level of the state average (110% and 106%, respectively). By contrast, persons over age 64 are, on average, are exposed to Traffic Proximity and Volume of more than 22% below the regional average. 

```{r barplotTPVPopAvgNE, fig.align = "center", fig.cap="Population-weighted average Traffic Proximity and Volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Rhode Island at Census Block Group level relative to the state average."}
# Pop Weighted avg of TPV for all Groups in Rhode Island relative to NE average
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>%
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 60, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -20, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 10, yend = 50), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-30,120))

ggsave("images/RI_Traffic_graph.png")
```

There is a strong positive correlation between the proportions of People of Color and traffic proximity and volume, and moderate positive correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixTPVPopAvg) in Appendix B).
...
[^EPAPM10risk]: See EPA SUMMARY OF PROPOSAL TO RETAIN THE AIR QUALITY STANDARDS FOR PARTICLE POLLUTION. https://www.epa.gov/sites/production/files/2020-04/documents/fari_sheet_pm_naaqs_proposal.pdf
...
[^NONATTAINMENT]: Note that the EJSCREEN values do not directly indicate nonattainment of the NAAQS standard because the EJSCREEN data is based on estimates from a combination of modeling and monitoring for a single year, while nonattainment is determined for a large area (often a county) based on three years of monitoring data. For example, five counties in Rhode Island have been designated as "nonattainment" status for NAAQS ozone standards as of March 2020. For a list of nonattainment counties see EPA 8-Hour Ozone Designated Area State/Area/County Report. https://www3.epa.gov/airquality/greenbook/jbcs.html#RI 
...
The total or sum of the products (i.e., POC x PM~2.5~ concentrations) is then divided by the sum of the weights (i.e., total People of Color), so that 324.5/45 = `r round(324.5/45,2)`. The result is a weighted average PM~2.5~ concentration that is influenced by the number of People of Color. 
...
ggcorrplot(corrPollutants, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Pollutant Correlation Matrix for Rhode Island", 
...
corrPM25 <- ri_blkgrp_sf %>% 
  ...
  dplyr::transmute(PM25 = PM25_19, 
                ...
                `RI POC` = RI_People of Color) %>% 
  drop_na() %>% 
  cor(method = "spearman")
# corrPM25
ggcorrplot(corrPM25, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "PM2.5 Correlation Matrix for Rhode Island", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           lab_size = 2)
```
...
ggcorrplot(corrO3, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Ozone Correlation Matrix for Rhode Island", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           lab_size = 2)
```

```{r boxplotCO2RI, fig.align = "center", fig.cap="Boxplot of Carbon Dioxoide on-road emissions in metric tons per square kilometer by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of CO2 by state for 2017
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(onroad_2017 = kgco2_2017/1000/(bg_area_m2/1000000)) %>%
...
ggcorrplot(corrDPM, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "DPM Correlation Matrix for Rhode Island", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           lab_size = 2,
...
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Cancer Risk Correlation Matrix for Rhode Island", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           lab_size = 2,
           ggtheme = ggplot2::theme_minimal())
```
...
# corrPM25
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Respiratory Hazard Index \nCorrelation Matrix for Rhode Island", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           ggtheme = ggplot2::theme_minimal(),
           lab_size = 2)
```
...
ne_towns_sf %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    caption = "Average Pollutant and Transportation Burden Values by Municipality in Rhode Island", align = "r") %>% 
