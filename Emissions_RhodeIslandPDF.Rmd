...
ri_blkgrp_sf %>% 
  ...
  transmute(`People of Color` = (minorityE/PM25Mean - 1)*100,
         ...
         `RI POC` = ( RI_MINORITY/PM25Mean - 1)*100) %>%
...
ri_blkgrp_sf %>% 
  ...
  transmute(`People of Color` = (minorityE/O3Mean - 1)*100,
         ...
         `RI POC` = ( RI_MINORITY/O3Mean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
...
```{r hotspotCO2RI, fig.align = "center", fig.cap="Hot spot map of 2017 on-road CO2 emissions at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(ri_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- ri_blkgrp_sf[!empty_geo,] %>% 
  mutate(onroad_2017 = kgco2_2017/1000/(bg_area_m2/1000000)) %>%
...
```{r statsCO2RI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Annual 2017 On-road CO2 emissions (mtons) at Census Block Group level and by state."}
ri_blkgrp_sf %>% 
...
```{r mapCO2RIchange, fig.align = "center", fig.cap="Map of percent change in annual on-road CO2 emissions across Rhode Island between 1990 and 2017 at Census Block Group level."}
...
```{r changeCO2RI, fig.align = "center", fig.cap="Annual 2017 On-road CO2 emissions (mtons) and Percent Change by state at Census Block Group level."}
library(tidycensus)
...
ri_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total Rhode Island On-road ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```
...
ri_blkgrp_sf %>% 
  ...
  transmute(`People of Color` = (minorityE/DPMMean - 1)*100,
         ...
         `RI POC` = (RI_MINORITY/DPMMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Diploma", y = 25, label = "Above state\naverage"),
            color = "gray48") +
...
ri_blkgrp_sf %>% 
  ...
  transmute(`People of Color` = (minorityE/RESPMean - 1)*100,
         ...
         `RI POC` = (RI_MINORITY/RESPMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Diploma", y = 10, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Diploma", y = -4, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 3, yend = 10), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
