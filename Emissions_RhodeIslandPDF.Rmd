---
title: "Transportation Emissions in Rhode Island"
...
PM~2.5~ levels vary significantly across Rhode Island, with highest concentrations in southwest Rhode Island, with concentration declining from Stamford to Hartford (see Figure \@ref(fig:mapPM25RI) below). 
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {round(mean(ri_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {round(min(ri_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%, have been in the southern half of the state. 
...
```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted exposure to PM2.5 between 2011 and 2016 for Rhode Island and New England."}
...
In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, minorities in Rhode Island are exposed to PM~2.5~ concentrations that are 6.8% above concentrations for the state as a whole. Similarly, language isolated households are exposed to concentrations over 5% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of PM~2.5~ at or below the state average. 
...
ri_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Low Income", y = 3.5, label = "Above state avg"),
            color = "gray48") +
...
There is a weak positive relationship between the proportion of minority residents or language-isolated households and the concentration of PM~2.5~ (see Appendix).
...
These Ozone (O~3~) levels vary significantly across the state. Figure \@ref(fig:boxplotO3RI) is a boxplot of O~3~ concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median O~3~ concentration for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$OZONE_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high or low values relative to the rest of the state.   

```{r boxplotO3RI, fig.align = "center", fig.cap="Boxplot of 2016 Ozone summer seasonal average of daily maximum 8-hour concentrations in air in parts per billion by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of PM2.5 by state for 2016
ri_blkgrp_sf %>% 
  as.data.frame() %>% 
...
Since 2011, Ozone (O~3~) levels have increased slightly across the state, on average by `r {round(mean(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)}`%. These have changes not been uniform (see Figure \@ref(fig:mapO3RIchange) below). The greatest declines, up to `r round(min(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`%, have been along the southeast the state. By contrast, increases in summer ozone concentrations of up to `r round(max(ri_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`% appear in the extreme north around Woonsocket.
...
In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the region. For example, persons over age 64 in Rhode Island are exposed to summer Ozone concentrations that are approximately 0.03% above concentrations for the region as a whole. By contrast, all other popuations of concer are, on average, exposed to concentrations of Ozone at or below the regional average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in Rhode Island relative to the regional average."}
...
ri_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = .2, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Low Income", xend = "Low Income", y = 0.1, yend = 0.3), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-0.5,0.3))
```
...
On-road CO~2~ emissions closely follow major roadways arcoss the region (see Figure \@ref(fig:mapCO2RI) below). 
...
The Block Group with highest CO~2~ emissions value in the state is found in `r ri_town_maxCO2` and the lowest is found in `r ri_town_minCO2`.
...
Diesel Particulate Matter (DPM) emissions are concentrated around greater Providence metro region, also around Newport and Woonsocket (see Figure \@ref(fig:mapDPMRI) below). 
...
There are significant spatial clusters of high Diesel Particulate Matter concentrations in Providence (see Figure \@ref(fig:hotspotDPMRI) below).
...
Diesel Particulate Matter concentrations vary across the state (see Table \@ref(tab:statsDPMRI) and Figure \@ref(fig:boxplotDPMRI) below).  
...
The Block Group with highest Diesel Particulate Matter emissions value in the state is found in `r ri_town_maxDPM` and the lowest is found in `r ri_town_minDPM`.

Figure \@ref(fig:boxplotDPMRI) is a boxplot of DPM concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median DPM concentration for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$PM25_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.  
...
ri_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 25, label = "Above state avg"),
            color = "gray48") +
...
There is a strong positive correlation between the proportions of minorities and ambient concentrations of Diesel Particulate Matter, and moderate to strong correlations with limited English households, low income persons, and adults without a high school diploma (see Appendix).
...
Air Toxics Cancer Risk are concentrated in and around Providence within the I-295 belt (see Figure \@ref(fig:mapATCNE) below).
...
The Block Group with highest cancer risk value in the state is found in `r ri_town_maxCancer` and the lowest is found in `r ri_town_minCaner`.
...
In addition to variations in the general geography of Lifetime Cancer Risk from Inhalation of Air Toxics, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Lifetime Cancer Risk from Inhalation of Air Toxics for the state. For example, minorities in Rhode Island (following Rhode Island's definition) experience lifetime cancer risks from inhalation of air toxics that are more than 14% above the state as a whole. Similarly, low income persons (following Rhode Island's definition) experience lifetime cancer risks more than 10% above the state average. By contrast, persons over age 64 are, on average, exposed to risk 3.2% below the state average. 
...
ri_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 9, label = "Above state avg"),
            color = "gray48") +
...
There is a strong positive correlation between the proportions of minorities and cancer risk, and moderate to strong correlations with limited English households, low income persons, and adults without a high school diploma (see Appendix).
...
The Block Group with highest respiratory hazard index value in the state is found in `r ri_town_maxRESP` and the lowest is found in `r ri_town_minRESP`.

Figure \@ref(fig:boxplotRHIRI) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.   
...
In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Respiratory Hazard Index from Inhalation of Air Toxics for the state. For example, minorities in Rhode Island (following Rhode Island's definition) experience respiratory hazard indices from inhalation of air toxics that are over 18% above the state as a whole. Similarly, minorities (following Rhode Island's definition) experience lifetime hazard index values more than 14% above the state average. By contrast, persons over age 64 are, on average, exposed to respiratory hazard indices over 4% below the state average. 

```{r barplotRHIPopAvgRI, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Rhode Island at Census Block Group level relative to the regional average."}
...
ri_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 10, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Under 5", xend = "Under 5", y = 3, yend = 10), 
               arrow = arrow(length = unit(0.3,"cm"))) +
...
There is a strong positive correlation between the proportions of minorities and respiratory hazard index, and moderate to strong positive correlations with limited English households, low income persons, and adults without a high school diploma (see Appendix).
...
EPA's EJSCREEN provides an indicator of traffic exposure measured as residential proximity to roads weighted by traffic volume. More specifically, EJSCREEN's Traffic Proximity and Volume indicator is a count of vehicles (average annual daily traffic) at major roads within 500 meters of residential areas (i.e., Census Blocks) divided by distance in kilometers (km).[^EPATraffic] For example, a residential area at 100 meters distance from a single highway with 33,000 AADT (average annual daily traffic) would result in a score of 33,000/100=330, which is approximately the median person’s block group traffic proximity indicator value in New England. The Traffic Proximity and Volume indicator values are aggregated at the Census Block Group level. 
...
The Block Group with highest Traffic Proximity and Volume value in the state is found in `r ri_town_maxTraffic` and the lowest is found in `r ri_town_minTraffic`.

Figure \@ref(fig:boxplotTPVRI) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(ri_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  
...
In addition to variations in the general geography of the Traffic Proximity and Volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for populations of concern relative to average Traffic Proximity and Volume for the state. For example, minorities (following Rhode Island's definition) and language isolated households are exposed to Traffic Proximity and Volume more than twice the level of the state average (110% and 106%, respectively). By contrast, persons over age 64 are, on average, are exposed to Traffic Proximity and Volume of more than 22% below the regional average. 
...
ri_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 60, label = "Above state avg"),
            color = "gray48") +
...
There is a strong positive correlation between the proportions of minorities and traffic proximity and volume, and moderate positive correlations with limited English households, low income persons, and adults without a high school diploma (see Appendix).
...
```{r cormatrixRHIPopAvg, fig.align = "center", fig.cap="Spearman's correlation matrix of lifetime cancer risk and the proportions of populations of concern by Census Block Group."}
# Create a correlation matrix of cancer risk and populations of concern
corrARI <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>%
