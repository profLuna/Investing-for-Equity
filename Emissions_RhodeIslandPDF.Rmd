...
### PM~2.5~ in Rhode Island and Populations of Concern

In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, minorities, as identified by Rhode Island Environmental Justice policy, are exposed to PM~2.5~ concentrations that are 6.8% above concentrations for the state as a whole. Similarly, limited English speaking households are exposed to concentrations over 5.2% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of PM~2.5~ 1.5% below the state average. 

```{r barplotPM25PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for populations of concern in Rhode Island relative to the state average."}
...
ri_blkgrp_sf %>% 
  ...
  dplyr::select(totalpopE,
                ...
                RI_MINORITIES,
                PM25_19) %>% 
  gather(key = Group, value = Pop, totalpopE:RI_MINORITIES) %>% 
  group_by(Group) %>% 
  ...
  transmute(Minority = (minorityE/PM25Mean - 1)*100,
         ...
         `RI Minority` = ( RI_MINORITIES/PM25Mean - 1)*100) %>%
...
### Ozone (O~3~) in Rhode Island and Populations of Concern

In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the state. For example, persons over age 64 in Rhode Island are exposed to summer Ozone concentrations that are approximately 0.03% above concentrations for the state as a whole. By contrast, all other populations of concern are, on average, exposed to concentrations of Ozone at or below the state average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in Rhode Island relative to the state average."}
...
ri_blkgrp_sf %>% 
  ...
  transmute(Minority = (minorityE/O3Mean - 1)*100,
         ...
         `RI Minority` = ( RI_MINORITIES/O3Mean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
...
Like the region as a whole, these populations have also experienced changes in exposure since 2011. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvgRI). All populations of concern have experienced a decrease in population-weighted exposure to Ozone. 

```{r slopegraphO3PopAvgRI, fig.align = "center", fig.cap="Change in population-weighted exposure to summer ozone for populations of concern between 2011 and 2016."}
...
### Diesel Particulate Matter in Rhode Island and Populations of Concern

In addition to variations in the general geography of Diesel Particulate Matter concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Diesel Particulate Matter concentraions for the state. For example, minorities in Rhode Island are exposed to  Diesel Particulate Matter concentrations that are more than 50% above concentrations for the region as a whole. Similarly, low income persons (following Rhode Island's definition) are exposed to concentrations more than 40% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of Diesel Particulate Matter more than 12% below the state average. 
...
ri_blkgrp_sf %>% 
  ...
  transmute(Minority = (minorityE/DPMMean - 1)*100,
         ...
         `RI Minority` = (RI_MINORITIES/DPMMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in Rhode Island and Populations of Concern

In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgRI) below shows population-weighted exposures for populations of concern relative to average cancer risk for the state. For example, minorities, as identified by Rhode Island Environmental Justice policy, experience lifetime cancer risks from inhalation of air toxics that are more than 14% above the state as a whole. Similarly, low income persons, as identified by Rhode Island Environmental Justice policy, experience lifetime cancer risks more than 10% above the state average. By contrast, persons over age 64 are, on average, exposed to risk 3.2% below the state average. 
...
There is a strong positive correlation between the proportions of minorities and cancer risk, and moderate to strong correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixATCPopAvg) in Appendix B).
...
### Respiratory Hazard Index from Inhalation of Air Toxics in Rhode Island and Populations of Concern

In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for populations of concern relative to the average hazard index for the state. For example, minorities, as identified by Rhode Island Environmental Justice policy, experience respiratory hazard indices from inhalation of air toxics that are over 18% above the state as a whole. Similarly, minorities, as identified by Rhode Island Environmental Justice policy, experience lifetime hazard index values more than 14% above the state average. By contrast, persons over age 64 are, on average, exposed to respiratory hazard indices over 4% below the state average. 
...
ri_blkgrp_sf %>% 
  ...
  transmute(Minority = (minorityE/RESPMean - 1)*100,
         ...
         `RI Minority` = (RI_MINORITIES/RESPMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
  ggplot(aes(x = reorder(Group,Pct), 
             y = Pct)) +
  geom_segment(aes(x = reorder(Group,Pct), 
...
There is a strong positive correlation between the proportions of minorities and respiratory hazard index, and moderate to strong positive correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixRHIPopAvg) in Appendix B).
...
### Traffic Proximity and Volume Exposure in Rhode Island and Populations of Concern

In addition to variations in the general geography of the traffic proximity and volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for populations of concern relative to average traffic proximity and volume for the state. For example, minorities, as identified by Rhode Island Environmental Justice policy, and limited English speaking households are exposed to Traffic Proximity and Volume more than twice the level of the state average (110% and 106%, respectively). By contrast, persons over age 64 are, on average, are exposed to Traffic Proximity and Volume of more than 22% below the regional average. 
...
There is a strong positive correlation between the proportions of minorities and traffic proximity and volume, and moderate positive correlations with limited English households, low income persons, and adults without a high school diploma (see Figure \@ref(fig:cormatrixTPVPopAvg) in Appendix B).
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "Rhode Island Low Income threshold: 30% or more people in households where the household income is less than or equal to twice the federal poverty level",
