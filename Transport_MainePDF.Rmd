...
# Analysis of walkability and transportation cost burden in Maine

This is an analysis of walkability and transportation cost burden in Maine.
...
# 
# # read in park-in-ride lots from Maine Open Geodata Portal at https://geodata.vermont.gov/ . Keep only park-in-rides that are serviced by public transit. 
# me_parknride_sf <- st_read(dsn = "DATA/transport/ME/ME_Park_and_Rides",
#                            layer = "ME_Park_and_Rides") %>% 
#   filter(TRANSIT != "NONE") %>% 
#   st_transform(., crs = 2802) %>% 
#   st_make_valid()
...
# me_busBuff400m <- st_buffer(me_stops_sf_Bus,dist = 400) %>% 
#   st_union() %>% 
#   st_as_sf()
# 
# # Create buffers around park-in-ride stops (4800m)
# me_prBuff4800m <- st_buffer(me_parknride_sf, dist = 4800) %>% 
#   st_union() %>% 
#   st_as_sf()
# 
# me_transitAllBuff <- rbind(me_busBuff400m,
#                            me_prBuff4800m) %>% 
#   st_union() %>% 
#   st_as_sf() %>% 
#   mutate(Sqm = st_area(.))
...
# 
# me_prBuff4800m_sf <- me_blkgrps_developed %>% 
#   select(GEOID,NewArea:NewLths) %>% 
#   mutate(OldArea = NewArea) %>% 
#   st_intersection(me_prBuff4800m,.) %>% 
#   mutate(NewArea = st_area(.),
#          Proportion = NewArea/OldArea,
#          NewPop = as.integer(NewPop*Proportion),
#          NewMinority = as.integer(NewMinority*Proportion),
...
#          NewLths = as.integer(NewLths*Proportion))
# 
# me_TransitAllBuff_sf <- me_blkgrps_developed %>% 
#   select(GEOID,NewArea:NewLths) %>% 
#   mutate(OldArea = NewArea) %>% 
#   st_intersection(me_transitAllBuff,.) %>% 
#   mutate(NewArea = st_area(.),
#          Proportion = NewArea/OldArea,
#          NewPop = as.integer(NewPop*Proportion),
#          NewMinority = as.integer(NewMinority*Proportion),
...
# 
# me_prBuff4800mTracts_sf <- me_tracts_developed %>% 
#   select(GEOID,NewDisabled:NewHHNoCar) %>% 
#   mutate(OldArea = st_area(.)) %>% 
#   st_intersection(me_prBuff4800m,.) %>% 
#   mutate(NewArea = st_area(.),
...
#          NewNoCar = as.integer(NewHHNoCar*Proportion))
# 
# me_TransitAllBuffTracts_sf <- me_tracts_developed %>% 
#   select(GEOID,NewDisabled:NewHHNoCar) %>% 
#   mutate(OldArea = st_area(.)) %>% 
#   st_intersection(me_transitAllBuff,.) %>% 
#   mutate(NewArea = st_area(.),
...
# 
# me_prBuff4800m_df <- me_prBuff4800m_sf  %>% 
#   as.data.frame() %>% 
#   summarize(`Total Pop` = sum(NewPop),
#             Minority = sum(NewMinority),
#             `Under 5` = sum(NewUnder5),
...
# 
# me_TransitAllBuff_df <- me_TransitAllBuff_sf %>% 
#   as.data.frame() %>% 
#   summarize(`Total Pop` = sum(NewPop),
#             Minority = sum(NewMinority),
#             `Under 5` = sum(NewUnder5),
...
#   gather(key = Group, value = BusPop)
# 
# me_prBuff4800mTracts_df <- me_prBuff4800mTracts_sf %>% 
#   as.data.frame() %>% 
#   summarize(`Disabled` = sum(NewDisabled),
#             `No Car HH` = sum(NewNoCar)) %>% 
#   gather(key = Group, value = ParknRidePop)
# 
# me_TransitAllBuffTracts_df <- me_TransitAllBuffTracts_sf %>% 
#   as.data.frame() %>% 
#   summarize(`Disabled` = sum(NewDisabled),
#             `No Car HH` = sum(NewNoCar)) %>% 
#   gather(key = Group, value = TransitPop)
...
#   left_join(.,me_prBuff4800mTracts_df, by = "Group") %>% 
#   left_join(.,me_TransitAllBuffTracts_df, by = "Group")
# 
...
#   left_join(., me_prBuff4800m_df, by = "Group") %>% 
#   left_join(., me_TransitAllBuff_df, by = "Group") %>% 
#   rbind(.,me_tract_pops_df) %>% 
#   mutate(PctBus = BusPop/MEPop*100,
#          PctParknRide = ParknRidePop/MEPop*100,
#          PctAllTransit = TransitPop/MEPop*100)
...
# me_busBuff1mile <- st_buffer(me_stops_sf_Bus,dist = 1609.34) %>% 
#   st_union() %>% 
#   st_as_sf()
...
# me_prBuff10mile <- me_parknride_sf %>% 
#   st_buffer(., dist = (10*1609.34)) %>% 
#   st_union() %>% 
#   st_as_sf()
# # identify block groups that do not intersect with 10 mile buffer
# me_blkgrps_sf_noPR <- me_blkgrps_developed %>% 
#   filter(st_disjoint(.,me_prBuff10mile,sparse = FALSE))
# # create simplified version of block groups for faster mapping
# me_blkgrps_sf_noPR_simple <- me_blkgrps_sf_noPR %>% 
...
# me_tracts_sf_noPR <- me_tracts_developed %>% 
#   filter(st_disjoint(.,me_prBuff10mile,sparse = FALSE))
# # create simplified version of tracts for faster mapping
# me_tracts_sf_noPR_simple <- me_tracts_sf_noPR %>% 
...
# me_transitBuffAll <- rbind(me_busBuff1mile,
#                              me_prBuff10mile) %>% 
#   st_union() %>% 
#   st_as_sf() %>% 
#   tmaptools::crop_shape(.,me_state_sf_cb,polygon = TRUE)
# 
# # identify block groups that do not intersect with any transit buffer
# me_blkgrps_sf_noTransit <- me_blkgrps_developed %>% 
#   filter(st_disjoint(.,me_transitBuffAll,sparse = FALSE))
# # identify tracts that do not intersect with 5 mile buffer
# me_tracts_sf_noTransit <- me_tracts_developed %>% 
#   filter(st_disjoint(.,me_transitBuffAll,sparse = FALSE))
# # create simplified versions for faster mapping
# me_blkgrps_sf_noTransit_simple <- me_blkgrps_sf_noTransit %>% 
#   st_simplify(., dTolerance = 100) %>% 
#   filter(!st_is_empty(.)) %>% 
#   st_make_valid()
# me_tracts_sf_noTransit_simple <- me_tracts_sf_noTransit %>% 
#   st_simplify(., dTolerance = 100) %>% 
#   filter(!st_is_empty(.)) %>% 
#   st_make_valid()
```
...
m <- tm_shape(me_blkgrp_walkability_sf) +
