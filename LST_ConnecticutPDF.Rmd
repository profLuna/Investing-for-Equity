---
title: "Heat Risk in Connecticut"
...
# ct_bmap <- ne_states_sf_cb %>% 
#   st_transform(., crs = 4326) %>% 
#   st_bbox(.) %>% 
#   read_osm(., type = "stamen-watercolor")
# 
# # map basemap
# tm_shape(ct_bmap) + tm_rgb() + tm_shape(ne_states_sf_cb) + tm_borders()
...
# 
# # Crop LST to Connecticut
# # isolate data for Connecticut
...
# 
# # create urban area polygon for Connecticut
# ct_urban_sf <- us_urban_sf %>%
#   st_centroid(.) %>%
#   st_transform(., crs = 2775) %>%
#   st_join(., ct_state_sf_cb, join = st_within, left = FALSE) %>%
#   as.data.frame() %>%
...
# ct_rural_sf <- ct_state_sf_cb %>%
#   group_by() %>%
#   summarize() %>%
#   st_difference(., ct_urban_sf) %>%
#   st_make_valid()
# 
# # # select blkgrps that do not intersect wtih urban polygons - rural blkgrps
# # ct_blkgrpLST_rural <- st_disjoint(ct_blkgrp_sf,
# #                                   ct_urban_sf,sparse = FALSE) %>%
# #   apply(., 1, all) %>% # convert dense matrix to logical vector
# #   ct_blkgrpLST_sf[.,] # subset blkgrps where there is NO intersection
# #
# # # select blkgrps that intersect with urban polygons - urban blkgrps
# # ct_blkgrpLST_urban <- st_intersects(ct_blkgrp_sf,
# #                                     ct_urban_sf,sparse = FALSE) %>%
# #   apply(., 1, any) %>% # convert dense matrix to logical vector
# #   ct_blkgrpLST_sf[.,] # subset blkgrps where there is ANY intersection
# #
# # ct_blkgrp_urban_simple <- ct_blkgrpLST_urban %>%
# #   group_by() %>%
# #   summarize()
# #
# # ct_blkgrp_rural_simple <- ct_blkgrpLST_rural %>%
# #   group_by() %>%
# #   summarize()
# 
# # crop LST to urban
...
# 
# meanLSTDayNightDiff_df <- ct_blkgrp_sf %>%
#   dplyr::select(GEOID) %>%
#   as_Spatial() %>%
...
# # meanLSTavgRural_df <- ct_blkgrp_rural_simple %>%
# #   as_Spatial() %>%
...
# 
# save(ct_blkgrpLST_sf, ct_urban_sf, ct_rural_sf, file = "DATA/LST/ct_blkgrpLST_sf.rds")
# 
# # clean up rasters
...
ct_blkgrpLST_sf <- ct_blkgrpLST_sf %>%
...
ct_state_sf_cb <- ne_states_sf_cb %>% 
  filter(NAME == "Connecticut") %>% 
  st_transform(., crs = st_crs(ct_blkgrpLST_sf))
```


## Day-Night Average Land Surface Temperatures

Day-Night Average Land Surface Temperatures (LST) represent a simple average of LST values collected during the day (11:48am - 2pm) and during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. These day-night average temperatures varied significantly across Connecticut, with highest temperatures apparent in the most urbanized areas of the state. The highest temperatures closely follow I-95 from Stamford to New Haven and I-91 from New Haven to Hartford (see Figure \@ref(fig:mapLSTavgCT)). 

```{r mapLSTavgCT, fig.align = "center", fig.cap="Map of July-Aug 2019 day-night Average Land Surface Temperatures (LST) across Connecticut at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_24hrLST.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_24hrLST.png")
```

```{r LST24outliers, include=FALSE}
# Identify municipalities with high heat outliers in Connecticut
ct_LST24outliers <- ct_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTavgCT) is a boxplot of average day-night temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$meanAvgLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LST24outliers`. 

```{r boxplotLSTavgCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 day-night average Land Surface Temperatures (LST) across Connecticut at Census Block Group level."}
...
The UHI effect analyzed here is defined as the difference in temperature between urbanized areas and rural areas in Connecticut.[^urban] Specifically, the average temperature of the rural areas of Connecticut is subtracted from the average temperatures of each Census Block Group. The resulting value shows how much warmer or cooler each Census Block Group is when compared to the rural background reference. Positive values indicate that a Census Block Group is warmer than the rural background average and may be experiencing the UHI effect. The larger the value, the stronger the effect. 

The UHI effect for day-night average temperatures in Connecticut are most pronounced in the inner core of urbanized areas (see Figure \@ref(fig:mapUHIavgCT)), particularly around Hatford, New Britain, and Fairfield. Temperatures in these areas are up to `r round(max(ct_blkgrpLST_sf$UHI24avg, na.rm = TRUE),1)`° warmer than the rural background average. By contrast, the northwestern and northeastern corners of the state are closer to the rural average. 

```{r mapUHIavgCT, fig.align = "center", fig.cap="Map of July-Aug 2019 day-night average Urban Heat Island (UHI) effect across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_24hrUHI.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_24hrUHI.png")
```

```{r LST24UHIoutliers, include=FALSE}
ct_LST24UHIoutliers <- ct_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotUHIavgCT) is a boxplot of day-night average Urban Heat Island (UHI) effect by Block Group. The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median UHI for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$UHI24avg, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LST24UHIoutliers`. 

```{r boxplotUHIavgCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 day-night average Urban Heat Island (UHI) across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHI24avg) %>% 
...
# Pop Weighted avg of 24hr LST exposure for all Groups in Connecticut relative to CT average
LST24hrPop <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE, 0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
  ...
  filter(Group %in% c("minorityE","eng_limitE","num2povE","lthsE","under5E","over64E","CT_LOWINC")) %>% 
...
# Pop Weighted avg of LST24hr for all Groups in Connecticut relative to CT average
LST24hrPopTab <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE, 0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>%
  # filter(STATE == "Connecticut") %>%
  ...
  transmute(Minority = (minorityE/LST24hrMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/LST24hrMean - 1)*100) %>%
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLST24hrTempPopAvgCT) and Table \@ref(tab:tabLST24hrPopAvgCT) show population-weighted exposures for priority populations relative to the day-night average LST for the state of `r round(mean(ct_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Connecticut were exposed to a population-weighted day-night average LST of `r round(max(LST24hrPop$LST24hrwMean),1)`°, approximately `r round(max(LST24hrPop$LST24hrwMean) - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[2,4]`) above the state day-night average LST. Similarly, low income persons, as defined by the state's environmental justice policy,  were exposed to a population-weighted day-night average LST of `r LST24hrPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(LST24hrwMean),1)) %>% pull()`°, over `r round(LST24hrPop %>% filter(Group == "CT Low Income") %>% summarize(max(LST24hrwMean)) %>% pull() - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[1,4]`) above the state day-night average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted day-night average LSTs of `r LST24hrPopTab[6,3]` (`r LST24hrPopTab[6,4]`) below the state average.

```{r plotLST24hrTempPopAvgCT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average temperatures for priority populations in Connecticut relative to the state average."}
LST24hrPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 81.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 78, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 81, yend = 82), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(77,83))

ggsave("images/CT_LST24_graph.png")
```
\pagebreak
```{r tabLST24hrPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average LST for priority populations in Connecticut relative to the state average."}
LST24hrPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::footnote(., symbol = paste0("State average day-night LST is ", round(mean(ct_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),2), "°"))
```

```{r UHI24hrTempPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Connecticut relative to CT average
UHI24hrPop <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE, 0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>%  
  # filter(STATE == "Connecticut") %>%
...
UHI24hrPopTab <- ct_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/UHI24hrMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/UHI24hrMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHI24hrTempPopAvgCT) and Table \@ref(tab:tabUHI24hrPopAvgCT) show population-weighted exposures for priority populations relative to the day-night average Urban Heat Island (UHI) effect for Connecticut. All priority populations are exposed to the UHI effect on average. Indeed, the average UHI effect across Connecticut is approximately `r round(mean(ct_blkgrpLST_sf$UHI24avg,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Connecticut were exposed to a population-weighted day-night average UHI effect of `r round(max(UHI24hrPop$UHI24hrwMean),1)`° (`r UHI24hrPopTab[2,4]`) above the rural average. This value is more than `r round(max(UHI24hrPop$UHI24hrwMean) - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[1,4]`) above the state day-night average UHI. Similarly, low income persons, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average UHI effect of `r UHI24hrPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(UHI24hrwMean),1)) %>% pull()`°, approximately `r round(UHI24hrPop %>% filter(Group == "CT Low Income") %>% summarize(max(UHI24hrwMean)) %>% pull() - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[1,4]`) above the state day-night average UHI. By contrast, persons over age 64 were, on average, exposed to a population-weighted day-night average UHI `r UHI24hrPopTab[6,3]` (`r UHI24hrPopTab[6,4]`) below the state average.

```{r plotUHI24hrTempPopAvgCT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for priority populations in Connecticut."}
UHI24hrPop %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
...
ggsave("images/CT_UHI24_graph.png")
```


```{r tabUHI24hrPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for priority populations in Connecticut relative to the rural average."}
UHI24hrPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average day-night UHI effect is ", round(mean(ct_blkgrpLST_sf$UHI24avg,na.rm=TRUE),2), "°"))
```

```{r LST24hrHighRiskStats, include=FALSE}
Pop24hr <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
CTINC24hr <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(CT_INCOME == "I" & pctile24hrLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))

CTINC24hrPct <- round(CTINC24hr/statepopsdf$TotalCT_LOWINC*100,1) %>% 
  pull()
```

Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLST24hrHighRiskLEH) and \@ref(fig:mapLST24hrHighRiskCTINC) highlight Census Block Groups across Connecticut which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and were in the highest quintile for limited English speaking households or categorized as Environmental Justice communities for low income persons.[^LST24quintile] Approximately `r LEH24hr %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r CTINC24hr %>% format(., big.mark = ",") %>% pull()` low income persons were resident in these highest quintile Block Groups. These numbers represent `r LEH24hrPct`% and `r CTINC24hrPct`%, respectively, of those populations in the state. By comparison, `r HH24hrPct`% of all households, and `r Pop24hrPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLST24hrHighRiskLEH) and \@ref(tab:tabLST24hrHighRiskCTINC) show breakdowns by municipality of how many limited English speaking households and low income persons these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other priority populations can be found in Figures \@ref(fig:mapLST24hrHighRiskMIN) to \@ref(fig:mapLST24hrHighRiskO64) and Tables \@ref(tab:tabLST24hrHighRiskMIN) to \@ref(tab:tabLST24hrHighRiskO64) in Appendix B.
...
# LST24EngLang5 <- ct_blkgrpLST_sf %>%
#   filter(LST24EngLangScore == 5)

LST24EngLangCT5 <- ct_blkgrpLST_sf %>%
  filter(rankEngLang == 5)

bbox_new <- st_bbox(LST24EngLangCT5) # current bounding box
...
m <- tm_shape(LST24EngLangCT5, unit = "mi") + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ny_state_sf) + tm_borders(lwd = 0.2, alpha = 0.8) +
  tm_shape(ct_highways) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ct_LST24EngLangCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24EngLangCT5.png")
```

\pagebreak
```{r tabLST24hrHighRiskLEH, fig.align = "center", fig.cap="Limited English speaking households in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
LST24EngLangCT5 %>% 
  st_centroid(.) %>% 
  st_intersection(ct_towns_sf,.) %>% 
  as.data.frame() %>% 
...
```{r mapLST24hrHighRiskCTINC, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and 80th percentile of low income, as defined by Connecticut Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
# LST24Minority5 <- ct_blkgrpLST_sf %>%
#   filter(LST24MinorScore == 5)

LST24LowIncomeCT5 <- ct_blkgrpLST_sf %>%
  filter(CT_INCOME == "I" & pctile24hrLST >= 0.8)

bbox_new <- st_bbox(LST24LowIncomeCT5) # current bounding box
...
m <- tm_shape(LST24LowIncomeCT5, unit = "mi", bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ny_state_sf) + tm_borders(lwd = 0.2, alpha = 0.8) +
  tm_shape(ct_highways) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ct_LST24LowIncomeCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24LowIncomeCT5.png")
```

\pagebreak
```{r tabLST24hrHighRiskCTINC, fig.align = "center", fig.cap="Low income persons, as defined by Connecticut Environmental Justice Policy, in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
...
LST24LowIncomeCT5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "CT Low Income in Highest Quintile of Day-Night Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day-Night LST (°F)" = 3)) %>% 
  kableExtra::footnote(., symbol = "As defined by Connecticut Environmental Justice Policy")
```

\pagebreak
## Daytime Average Land Surface Temperatures

Daytime Average Land Surface Temperatures (LST) represent LST values collected during the day (11:48am - 2pm) over eight days, from July 28 to August 4, 2019. Daytime average LST for the state was `r round(mean(ct_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. Daytime Land Surface Temperatures (LST) varied significantly across Connecticut, with highest temperatures apparent in the most urbanized areas of the state, particularly along I-95 and I-91, from Stamford to New Haven to Hartford. Other areas of high temperature appear around Waterbury and Danbury (see Figure \@ref(fig:mapLSTdayCT) below). Across Connecticut, approximately `r ct_blkgrpLST_sf %>% as.data.frame() %>% filter(pctileDayLST >= 0.8) %>% group_by() %>% summarize(sum(totalpopE, na.rm = TRUE)/10^3) %>% pull() %>% round(.,0)` thousand people live in Block Groups that exceeded the 80th percentile (temperatures over `r round(quantile(ct_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),1)`°) for daytime average temperatures.  

```{r mapLSTdayCT, fig.align = "center", fig.cap="Map of July-August 2019 average daytime Land Surface Temperatures (LST) across Connecticut at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_DayLST.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_DayLST.png")
```

```{r LSTDayoutliers, include=FALSE}
# Identify municipalities with high heat outliers
ct_LSTDayoutliers <- ct_blkgrpLST_sf %>% 
  arrange(desc(meanAvgLST)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(ct_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotLSTdayCT) is a boxplot of average daytime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$meanDayLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LSTDayoutliers`.

```{r boxplotLSTdayCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime average Land Surface Temperatures (LST) across Connecticut at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(meanDayLST) %>% 
...
The UHI effect for daytime average temperatures in Connecticut are most pronounced in the more densely urbanized areas (see Figure \@ref(fig:mapUHIdayCT)), particularly around Hartford and New Britain, and also along the I-95 corridor from Stamford to New Haven. Temperatures in these areas are up to `r round(max(ct_blkgrpLST_sf$UHIDay,na.rm = TRUE),1)`° warmer than the rural background average.  

```{r mapUHIdayCT, fig.align = "center", fig.cap="Map of July-Aug 2019 daytime Urban Heat Island (UHI) effect across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_DayUHI.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_DayUHI.png")
```

```{r LSTDayUHIoutliers, include=FALSE}
ct_LSTDayUHIoutliers <- ct_blkgrpLST_sf %>% 
  arrange(desc(UHIDay)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(ct_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotUHIDayCT) is a boxplot of average daytime Urban Heat Island (UHI) effect values by Block Group. The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median value for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$UHIDay, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LSTDayUHIoutliers`. 

```{r boxplotUHIDayCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime Urban Heat Island (UHI) across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHIDay) %>% 
...
# Pop Weighted avg of LST day for all Groups in Connecticut relative to CT average
LSTDayPopTab <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
  ...
  transmute(Minority = (minorityE/LSTDayMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/LSTDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTDayTempPopAvgCT) and Table \@ref(tab:tabLSTDayPopAvgCT) show population-weighted exposures for priority populations relative to the average daytime average LST for the state, which is `r round(mean(ct_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Connecticut were exposed to a population-weighted daytime average LST of `r round(max(LSTDayPop$LSTDaywMean),1)`°, approximately `r round(max(LSTDayPop$LSTDaywMean) - max(LSTDayPop$LSTDayMean),1)`° (`r LSTDayPopTab[2,4]`) above the state daytime average LST. Similarly, low income persons, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average LST of approximately `r LSTDayPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, `r round(LSTDayPop %>% filter(Group == "CT Low Income") %>% summarize(max(LSTDaywMean)) %>% pull() - LSTDayPop %>% summarize(max(LSTDayMean)) %>% pull(),1)`° (`r LSTDayPopTab[1,4]`) above the state daytime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average LSTs of `r LSTDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, approximately `r LSTDayPopTab[6,3]` below the state average.

```{r plotLSTDayTempPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to average daytime Land Surface Temperatures for priority populations in Connecticut relative to the state average."}
LSTDayPop %>% 
  mutate(Group = recode(Group, "No HS Dip" = "No HS Diploma",
  ...
  geom_text(aes(x = "Low Income", y = 94, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 89, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 94, yend = 97), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(88,96))

ggsave("images/CT_LSTDay_graph.png")
```

\pagebreak
```{r tabLSTDayPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to Daytime LST for priority populations in Connecticut relative to the state average."}
LSTDayPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime LST is ", round(mean(ct_blkgrpLST_sf$meanDayLST,na.rm=TRUE),2), "°"))
```

```{r UHIDayPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Connecticut relative to CT average
UHIDayPop <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
...
# Pop Weighted avg of UHI for all Groups in Connecticut relative to CT average
UHIDayPopTab <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
  ...
  transmute(Minority = (minorityE/UHIDayMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/UHIDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHIDayTempPopAvgCT) and Table \@ref(tab:tabUHIDayPopAvgCT) show population-weighted exposures for priority populations relative to the average daytime average Urban Heat Island (UHI) effect for the state. All priority populations are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ct_blkgrpLST_sf$UHIDay,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Connecticut, were exposed to a population-weighted daytime average UHI effect of `r round(max(UHIDayPop$UHIDaywMean),1)`° above the rural average. This value is more than `r round(max(UHIDayPop$UHIDaywMean) - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[2,4]`) above the state daytime average UHI. Similarly, low income persons, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r UHIDayPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`°, approximately `r round(UHIDayPop %>% filter(Group == "CT Low Income") %>% summarize(max(UHIDaywMean)) %>% pull() - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[1,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average of `r UHIDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`°, approximately UHI `r UHIDayPopTab[6,3]` (`r UHIDayPopTab[6,4]`) below the state average.

```{r plotUHIDayTempPopAvgCT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for priority populations in Connecticut."}
UHIDayPop %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
...
ggsave("images/CT_UHIDay_graph.png")
```


```{r tabUHIDayPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for priority populations in Connecticut relative to the rural average."}
UHIDayPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime UHI effect is ", round(mean(ct_blkgrpLST_sf$UHIDay,na.rm=TRUE),2), "°"))
```

```{r LSTDayHighRiskStats, include=FALSE}
PopDay <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
CTINCDay <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTDayMinorScore == 5) %>% 
  filter(CT_INCOME == "I" & pctileDayLST >= 0.8) %>% 
  summarize(sum(minorityE,na.rm = TRUE))

CTINCDayPct <- round(CTINCDay/statepopsdf$TotalCT_LOWINC*100,1) %>% 
  pull()
```

Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTDayHighRiskLEH) and \@ref(fig:mapLSTDayHighRiskCTINC) highlight Census Block Groups across Connecticut which exhibited daytime average LSTs in the top quintile (i.e., 80th percentile) and were in the highest quintile for limited English speaking households or categorized as Environmental Justice communities for low income persons.[^Dayquintile] Approximately `r LEHDay %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r CTINCDay %>% format(., big.mark = ",") %>% pull()` low income persons were resident in these highest quintile Block Groups. These numbers represent `r LEHDayPct`% and `r CTINCDayPct`%, respectively, of those populations in the state. By comparison, `r HHDayPct`% of all households, and `r PopDayPct`% of the general population were resident in these same Block Groups.  
Tables \@ref(tab:tabLSTDayHighRiskLEH) and \@ref(tab:tabLSTDayHighRiskCTINC) show breakdowns by municipality of the numbers of limited English speaking households and low income persons resident in these top quintile Block Groups. Maps and tables for other priority populations can be found in Figures \@ref(fig:mapLSTDayHighRiskMIN) to \@ref(fig:mapLSTDayHighRiskO64) and Tables \@ref(tab:tabLSTDayHighRiskMIN) to \@ref(tab:tabLSTDayHighRiskO64 in Appendix B.
...
LSTDayEngLangCT5 <- ct_blkgrpLST_sf %>%
  filter(rankEngLang == 5)

bbox_new <- st_bbox(LSTDayEngLangCT5) # current bounding box
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ct_LSTDayEngLangCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayEngLangCT5.png")
```
...
```{r mapLSTDayHighRiskCTINC, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and 80th percentile of percentage of low income persons, as defined by Connecticut Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTDayLowIncomeCT5 <- ct_blkgrpLST_sf %>%
  filter(CT_INCOME == "I" & pctileDayLST >= 0.8)

bbox_new <- st_bbox(LSTDayLowIncomeCT5) # current bounding box
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ct_LSTDayLowIncomeCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayLowIncomeCT5.png")
```
\pagebreak
```{r tabLSTDayHighRiskCTINC, fig.align = "center", fig.cap="Low income persons, as defined by Connecticut Environmental Justice Policy, in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayLowIncomeCT5 %>% 
  st_centroid(.) %>% 
  st_intersection(ct_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "CT Low Income in Highest Quintile of Daytime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"CT Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Daytime LST (°F)" = 3)) %>% 
  kableExtra::footnote(., symbol = "As defined by Connecticut Environmental Justice Policy")
...
Nighttime Land Surface Temperatures (LST) represent LST values collected during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. Nighttime average LST for the state was `r round(mean(ct_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`°. Nighttime Land Surface Temperatures (LST) varied significantly across Connecticut, with highest temperatures apparent in or near major cities, particularly in Hartford and extending south to Meriden, as well as along the I-95 corridor (see Figure \@ref(fig:mapLSTnightCT) below). 

```{r mapLSTnightCT, fig.align = "center", fig.cap="Map of July-August 2019 average nighttime Land Surface Temperatures (LST) across Connecticut at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_NightLST.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_NightLST.png")
```

```{r LSTNightoutliers, include=FALSE}
ct_LSTNightoutliers <- ct_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTnightCT) is a boxplot of average nighttime temperatures by Block Group. The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$meanNightLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large dots on either end represents outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LSTNightoutliers`. 

```{r boxplotLSTnightCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Land Surface Temperatures (LST) across Connecticut at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
The UHI effect for nighttime average temperatures in Connecticut are most pronounced in and around Hartford and New Britain (see Figure \@ref(fig:mapUHInightCT)). Temperatures in these areas are up to `r round(max(ct_blkgrpLST_sf$UHINight,na.rm = TRUE),1)`° warmer than the rural background average. By contrast, most of western Connecticut is cooler than rural background average. 

```{r mapUHInightCT, fig.align = "center", fig.cap="Map of July-Aug 2019 nighttime Urban Heat Island (UHI) effect across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Connecticut
m <- tm_shape(ct_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ct_NightUHI.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_NightUHI.png")
```
...
Figure \@ref(fig:boxplotUHINightCT) is a boxplot of nighttime Urban Heat Island (UHI) effects by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ct_blkgrpLST_sf$UHINight, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large black dots represent outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ct_LSTNightoutliers`.

```{r boxplotUHINightCT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Urban Heat Island (UHI) across Connecticut at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHINight) %>% 
...
# Pop Weighted avg of LST night for all Groups in Connecticut relative to CT average
LSTNightPopTab <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
  ...
  transmute(Minority = (minorityE/LSTNightMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/LSTNightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTNightTempPopAvgCT) and Table \@ref(tab:tabLSTNightPopAvgCT) show population-weighted exposures for priority populations relative to the average nighttime average LST of `r round(mean(ct_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`° for the state. For example, limited English speaking households in Connecticut were exposed to a population-weighted nighttime average LST of `r round(max(LSTNightPop$LSTNightwMean),1)`°, approximately `r round(max(LSTNightPop$LSTNightwMean) - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[2,4]`) above the state nighttime average LST. Similarly, low income persons, as defined by the state's environmental justice policy, were exposed to a population-weighted nighttime average LST of `r LSTNightPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, over `r round(LSTNightPop %>% filter(Group == "CT Low Income") %>% summarize(max(LSTNightwMean)) %>% pull() - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[1,4]`) above the state nighttime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted nighttime average LSTs of `r LSTNightPopTab[6,2]`, approximately `r LSTNightPopTab[6,3]` below the state average.

```{r plotLSTNightTempPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to average nighttime Land Surfact Temperatures for priority populations in Connecticut relative to the state average."}
LSTNightPop %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  geom_text(aes(x = "Low Income", y = 69, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 67, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 66.9, yend = 67.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(66.5,69.5))

ggsave("images/CT_LSTNight_graph.png")
```

\pagebreak
```{r tabLSTNightPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime LST for priority populations in Connecticut relative to the state average."}
LSTNightPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime LST is ", round(mean(ct_blkgrpLST_sf$meanNightLST,na.rm=TRUE),2), "°"))
```

```{r UHINightPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Connecticut relative to CT average
UHINightPop <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>% 
  # filter(STATE == "Connecticut") %>%
...
# Pop Weighted avg of PM2.5 for all Groups in Connecticut relative to CT average
UHINightPopTab <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE,0)) %>% 
  mutate(CT_LOWINC = replace_na(CT_LOWINC,0)) %>%
  # filter(STATE == "Connecticut") %>%
  ...
  transmute(Minority = (minorityE/UHINightMean - 1)*100,
         ...
         `CT Low Income` = (CT_LOWINC/UHINightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHINightTempPopAvgCT) and Table \@ref(tab:tabUHINightPopAvgCT) show population-weighted exposures for priority populations relative to the average daytime average Urban Heat Island (UHI) effect for the state. All priority populations are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ct_blkgrpLST_sf$UHINight,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Connecticut were exposed to a population-weighted daytime average UHI effect of `r round(max(UHINightPop$UHINightwMean),1)`° above the rural average. This value is more than `r round(max(UHINightPop$UHINightwMean) - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[2,4]`) above the state daytime average UHI. Similarly, low income persons, as defined by Connecticut Environmental Justice Policy, were exposed to a population-weighted daytime average UHI effect of `r UHINightPop %>% filter(Group == "CT Low Income") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, approximately `r round(UHINightPop %>% filter(Group == "CT Low Income") %>% summarize(max(UHINightwMean)) %>% pull() - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[1,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI of `r UHINightPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`° (`r UHINightPopTab[6,4]`) below the state average.

```{r plotUHINightTempPopAvgCT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for priority populations in Connecticut."}
UHINightPop %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
...
ggsave("images/CT_UHINight_graph.png")
```


```{r tabUHINightPopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for priority populations in Connecticut relative to the rural average."}
UHINightPopTab %>% 
  arrange(Group) %>% 
  mutate(Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma")) %>%
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime UHI effect is ", round(mean(ct_blkgrpLST_sf$UHINight,na.rm=TRUE),2), "°"))
```

```{r LSTNightHighRiskStats, include=FALSE}
PopNight <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
HHNight <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(LSTNightEngLangScore == 5) %>%
  # filter(CT_ENGLISH == "E" & pctileNightLST >= 0.8) %>%
  summarize(sum(householdsE,na.rm = TRUE))

HHNightPct <- round(HHNight/statepopsdf$TotalHH*100,1) %>% 
  pull()

LEHNight <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(LSTNightEngLangScore == 5) %>%
  # filter(CT_ENGLISH == "E" & pctileNightLST >= 0.8) %>%
  summarize(sum(eng_limitE,na.rm = TRUE))
...
CTINCNight <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTNightMinorScore == 5) %>% 
  filter(CT_INCOME == "I" & pctileNightLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))

CTINCNightPct <- round(CTINCNight/statepopsdf$TotalCT_LOWINC*100,1) %>% 
  pull()
```

Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskLEH) and \@ref(fig:mapLSTNightHighRiskCTINC) highlight Census Block Groups across Connecticut which exhibited nighttime average LSTs in the top quintile (i.e., 80th percentile) and were in the highest quintile for limited English speaking households or categorized as Environmental Justice communities for low income persons.[^Nightquintile] Approximately `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r CTINCNight %>% format(., big.mark = ",") %>% pull()` low income persons were resident in these highest quintile Block Groups. These numbers represent `r LEHNightPct`% and `r CTINCNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLSTNightHighRiskLEH) and \@ref(tab:tabLSTNightHighRiskCTINC) show breakdowns by municipality of how many limited English speaking households and low income persons these represent, and the nighttime average LST values for those same Census Block Groups. Maps and tables for other priority populations can be found in Figures \@ref(fig:mapLSTNightHighRiskMIN) to \@ref(fig:mapLSTNightHighRiskO64) and Tables \@ref(tab:tabLSTNightHighRiskMIN) to \@ref(tab:tabLSTNightHighRiskO64) in Appendix B.
...
LSTNightEngLangCT5 <- ct_blkgrpLST_sf %>%
  filter(LSTNightEngLangScore == 5)

bbox_new <- st_bbox(LSTNightEngLangCT5) # current bounding box
...
tmap_save(m, "DATA/LST/ct_LSTNightEngLangCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightEngLangCT5.png")
```
...
```{r mapLSTNightHighRiskCTINC, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and 80th percentile of percentage of low income persons, as defined by Connecticut Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTNightLowIncomeCT5 <- ct_blkgrpLST_sf %>%
  filter(CT_INCOME == "I" & pctileNightLST >= 0.8)

bbox_new <- st_bbox(LSTNightLowIncomeCT5) # current bounding box
...
tmap_save(m, "DATA/LST/ct_LSTNightLowIncomeCT5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightLowIncomeCT5.png")
```
\pagebreak
```{r tabLSTNightHighRiskCTINC, fig.align = "center", fig.cap="Low income persons in Census Block Groups at or above 80th percentile of Nighttime average Land Surface Temperature."}
LSTNightLowIncomeCT5 %>% 
  st_centroid(.) %>% 
  st_intersection(ct_towns_sf,.) %>% 
  as.data.frame() %>% 
  mutate(CT_LOWINC = if_else(CT_INCOME == "I", totalpopE, 0)) %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "CT Low Income in Highest Quintile of Nighttime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"CT Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3)) %>% 
...
[^Williams2020]: See Augusta A. Williams, Joseph G. Allen, Paul J. Catalano, and John D. Spengler. 2020. "The Role of Individual and Small-Area Social and Environmental Factors on Heat Vulnerability to Mortality Within and Outside of the Home in Boston, CT." *Climate* 8(2): 29. https://doi.org/10.3390/cli8020029
...
[^LST24quintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of day-night average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ct_blkgrpLST_sf$meanAvgLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ct_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 

[^Dayquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ct_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ct_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 
...
[^Nightquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ct_blkgrpLST_sf$meanNightLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ct_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region.
...
The LST rasters were then mosaiced to create a single scene for each time period and clipped to Connecticut. 
...
isd_ct_DayNight_sf <- readRDS("DATA/LST/isd_ne_DayNight_sf.Rds") %>% 
  st_transform(., crs = st_crs(ct_state_sf_cb)) %>% 
  st_intersection(ct_state_sf_cb,.) 
```

In order to validate land surface temperature (LST) values, MODIS LST data was compared to ground-based air temperature measurements for the same time periods across Connecticut. Weather data was acquired from NOAA's Integrated Surface Data ('ISD') files from the NOAA National Centers for Environmental Information (https://www.ncdc.noaa.gov/isd/data-access) using the Commmon Access tool (https://www.ncei.noaa.gov/access/search/data-search/global-hourly). This yielded `r nrow(isd_ct_DayNight_sf)` weather stations across Connecticut with air temperature data available for the same period as the MODIS LST data (i.e., July 28 - August 4, 2019; see Figure \@ref(fig:ISDstations)). 

```{r ISDstations, fig.align = "center", fig.cap="NOAA Integrated Surface Data Weather Stations across Connecticut."}
tm_shape(ct_state_sf_cb, unit = "mi") + tm_borders(lwd = 0.2) +
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(isd_ct_DayNight_sf) + tm_dots(shape = 4, size = 0.08) +
...
cor_day <- cor.test(x = isd_ct_DayNight_sf$gridDay, y = isd_ct_DayNight_sf$tempFday, use = "pairwise.complete.obs", method = "spearman")
...
cor_night <- cor.test(x = isd_ct_DayNight_sf$gridNight, y = isd_ct_DayNight_sf$tempFnight, use = "pairwise.complete.obs", method = "spearman")
...
cor_daynight <- cor.test(x = isd_ct_DayNight_sf$gridDayNight, y = isd_ct_DayNight_sf$tempFdaynight, use = "pairwise.complete.obs", method = "spearman")
...
# Compare day average LST to weather station air temp data for same time period
lm_day <- lm(tempFday ~ gridDay, data = isd_ct_DayNight_sf)
RMSE_day <- sqrt(mean(lm_day$residuals^2))

# Compare night average LST to weather station air temp data for same time period
lm_night <- lm(tempFnight ~ gridNight, data = isd_ct_DayNight_sf)
RMSE_night <- sqrt(mean(lm_night$residuals^2))

# Compare day-night average LST to weather station air temp data for same time period
lm_daynight <- lm(tempFdaynight ~ gridDayNight, data = isd_ct_DayNight_sf)
...
```{r LSTvalidation, warning=FALSE, message=FALSE, fig.align = "center", fig.cap="Comparison of MODIS LST values and air temperatures for same time periods from NOAA Integrated Surface Data Weather Stations across Connecticut."}

day <- isd_ct_DayNight_sf %>% 
...
g <- arrangeGrob(day,night,daynight,ncol=1,
                 ...
                 widths = unit(5,"inches"))
ggsave(g, filename = "DATA/LST/ct_validate.png", 
       width = 5, height = 8, units = "in")
knitr::include_graphics("DATA/LST/ct_validate.png")
```


### Urban Heat Island effects {-}

Urban Heat Island (UHI) effects are measured as the difference between LST raster values across Connecticut for each time period and the average LST for rural states for the same time periods. It is represented by the following formula:

*UHI effect = LST - Mean Rural LST*

For example, to calculate the UHI effects for daytime temperatures, daytime LST pixels falling within the rural areas of Connecticut were averaged to provide a mean rural LST benchmark value. This mean rural LST value was then subtracted from the daytime LST values of all pixels across the state. The resulting value for each pixel indicates the magnitude or size of the difference, and the sign (+/-) indicates whether it is warmer or cooler than the mean rural LST.

Rural areas of Connecticut were identified according to the U.S. Census Bureau's urban-rural classification. The Census Bureau identifies two types of urban areas:
...
Urban areas TIGER/Line spatial files were downloaded from the Census Bureau via API using the `tigris` package in R. Areas of Connecticut not within the urban areas polygons were then extracted to represent rural areas. LST raster pixels falling within these non-urban or rual polygons were extracted and a mean LST value calculated for each time period (i.e., day, night, and day-night average) to represent the mean rural LST benchmark for calculating the UHI effect.
...
data.frame(BG = c("BG1","BG2","BG3","BG4"),
  ...
  kableExtra::kable(longtable = T, booktabs = T, 
                    ...
                    align = "l", 
                    col.names = c(names(.)[1:2], "People of Color", "Non-POC", names(.)[5])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header"))
```
...
The total or sum of the products (i.e., POC x Temperature) is then divided by the sum of the weights (i.e., total People of Color), so that 3475/45 = `r round(3475/45,2)`. The result is a weighted average temperature that is influenced by the number of People of Color. 
...
bbox_new <- st_bbox(LST24Minority5) # current bounding box
...
tmap_save(m, "DATA/LST/ct_LST24Minority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24Minority5.png")
```
...
LST24Minority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "People of Color in Highest Quintile of Day-Night Avg LST by Municipality", align = "r", 
                    col.names = c(names(.)[1:2], "People of Color", "Pct of POC", names(.)[5:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day-Night LST (°F)" = 3))
```
...
tmap_save(m, "DATA/LST/ct_LST24NoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24NoHSDip5.png")
```
...
```{r mapLST24hrHighRiskLI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and Low Income persons by Census Block Group, July-Aug 2019."}
LST24LowInc5 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LST24LowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24LowInc5.png")
```
\pagebreak
...
tmap_save(m, "DATA/LST/ct_LST24Under55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24Under55.png")
```
...
tmap_save(m, "DATA/LST/ct_LST24Over645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LST24Over645.png")
```
...
LSTDayMinority5 <- ct_blkgrpLST_sf %>%
  filter(LSTDayMinorScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ct_LSTDayMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayMinority5.png")
```
\pagebreak
...
LSTDayMinority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "People of Color in Highest Quintile of Daytime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2], "People of Color", "Pct of POC", names(.)[5:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
LSTDayNoHSDip5 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LSTDayNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayNoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/ct_LSTDayLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayLowInc5.png")
```
...
tmap_save(m, "DATA/LST/ct_LSTDayUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayUnder55.png")
```
...
tmap_save(m, "DATA/LST/ct_LSTDayOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTDayOver645.png")
```
...
LSTNightMinority5 <- ct_blkgrpLST_sf %>%
  filter(LSTNightMinorScore == 5)
...
tmap_save(m, "DATA/LST/ct_LSTNightMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightMinority5.png")
```
\pagebreak
...
LSTNightMinority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "People of Color in Highest Quintile of Nighttime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2], "People of Color", "Pct of POC", names(.)[5:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3))
...
LSTNightLowInc5 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LSTNightLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightLowInc5.png")
```
...
LSTNightNoHSDip5 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LSTNightNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightNoHSDip5.png")
```
...
LSTNightUnder55 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LSTNightUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightUnder55.png")
```
...
LSTNightOver645 <- ct_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ct_LSTNightOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ct_LSTNightOver645.png")
```
...
```{r cormatrixLSTpopsCT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and the proportions of priority populations by Census Block Group."}
# Create a correlation matrix of PM25 and priority populations
corrLSTpops <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
ggcorrplot(corrLSTpops, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Correlation Matrix of LST \nand Populations for Connecticut", 
...
```{r cormatrixLSTAvgCTpollutants, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and pollution by Census Block Group."}
# Create a correlation matrix of PM25 and priority populations
corrLSTemissions <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  dplyr::transmute(`Day-Night Avg LST` = meanNightLST,
                   `Day LST` = meanDayLST,
                   `Night LST` = meanNightLST,
...
ggcorrplot(corrLSTemissions, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Correlation Matrix of LST \nand Air Pollution for Connecticut",
           legend.title = "Spearman's \nCorrelation\nCoefficient", 
           lab_size = 2)
```
...
# 
# # convert to sp
# ct_blkgrpLST_sp <- as_Spatial(ct_blkgrpLST_sf)
# 
# #calculate kernel bandwidth
# GWRbandwidth <- gwr.sel(ct_blkgrpLST_sp$PM25_19 ~ ct_blkgrpLST_sp$meanAvgLST, data=ct_blkgrpLST_sp, adapt=T)
