...
```{r tabLST24hrPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average LST for populations of concern in Maine relative to the state average."}
LST24hrPopTab %>% 
  arrange(Group) %>% 
...
```{r tabUHI24hrPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Maine relative to the rural average."}
UHI24hrPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
tmap_save(m, "DATA/LST/me_LST24EngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24EngLang5.png")
```
...
LST24Minority5 <- me_blkgrpLST_sf %>%
  filter(LST24MinorScore == 5)
...
tmap_save(m, "DATA/LST/me_LST24Minority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24Minority5.png")
```
...
Daytime Average Land Surface Temperatures (LST) represent LST values collected during the day (11:48am - 2pm) over eight days, from July 28 to August 4, 2019. Daytime average LST for the state was `r round(mean(me_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. Daytime Land Surface Temperatures (LST) varied significantly across Maine, with highest temperatures apparent in the southeastern half of the state, particularly along I-95, from Saco to Bangor. Other areas of high temperature appear around Waterville, Rockland, and Houlton (see Figure \@ref(fig:mapLSTdayME) below). Across Maine, approximately `r me_blkgrpLST_sf %>% as.data.frame() %>% filter(pctileDayLST >= 0.8) %>% group_by() %>% summarize(sum(totalpopE, na.rm = TRUE)/10^3) %>% pull() %>% round(.,0)` thousand people live in Block Groups that exceeded the 80th percentile (temperatures over `r round(quantile(me_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),1)`°) for daytime average temperatures.  

```{r mapLSTdayME, fig.align = "center", fig.cap="Map of July-August 2019 average daytime Land Surface Temperatures (LST) across Maine at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Maine
m <- tm_shape(me_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/me_DayLST.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_DayLST.png")
```

```{r LSTDayoutliers, include=FALSE}
# Identify municipalities with high heat outliers
me_LSTDayoutliers <- me_blkgrpLST_sf %>% 
  arrange(desc(meanAvgLST)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(me_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotLSTdayME) is a boxplot of average daytime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(me_blkgrpLST_sf$meanDayLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r me_LSTDayoutliers`.

```{r boxplotLSTdayME, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime average Land Surface Temperatures (LST) across Maine at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(meanDayLST) %>% 
...
The UHI effect for daytime average temperatures in Maine are most pronounced in the more densely urbanized areas (see Figure \@ref(fig:mapUHIdayME)), particularly in the Portland, Lewiston, and Bangor. Temperatures in these areas are up to `r round(max(me_blkgrpLST_sf$UHIDay,na.rm = TRUE),1)`° warmer than the rural background average.  

```{r mapUHIdayME, fig.align = "center", fig.cap="Map of July-Aug 2019 daytime Urban Heat Island (UHI) effect across Maine at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Maine
m <- tm_shape(me_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/me_DayUHI.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_DayUHI.png")
```

```{r LSTDayUHIoutliers, include=FALSE}
me_LSTDayUHIoutliers <- me_blkgrpLST_sf %>% 
  arrange(desc(UHIDay)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(me_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotUHIDayME) is a boxplot of average daytime Urban Heat Island (UHI) effect values by Block Group . The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median value for all Block Groups, which in this case is `r round(median(me_blkgrpLST_sf$UHIDay, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r me_LSTDayUHIoutliers`. 

```{r boxplotUHIDayME, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime Urban Heat Island (UHI) across Maine at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHIDay) %>% 
...
# Pop Weighted avg of LST day for all Groups in Maine relative to ME average
LSTDayPopTab <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Maine") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
LSTDayPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 84, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 81.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 94, yend = 97), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(81,87))

ggsave("DATA/LST/me_DayLST_lolli.png")
```


```{r tabLSTDayPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to Daytime LST for populations of concern in Maine relative to the state average."}
LSTDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime LST is ", round(mean(me_blkgrpLST_sf$meanDayLST,na.rm=TRUE),2), "°"))
```

```{r UHIDayPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Maine relative to ME average
UHIDayPop <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Maine") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
```{r tabUHIDayPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Maine relative to the rural average."}
UHIDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime UHI effect is ", round(mean(me_blkgrpLST_sf$UHIDay,na.rm=TRUE),2), "°"))
```

```{r LSTDayHighRiskStats, include=FALSE}
PopDay <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTDayHighRiskLEH) and \@ref(fig:mapLSTDayHighRiskMIN) highlight Census Block Groups across Maine which exhibited daytime average LSTs in the top quintile (i.e., 80th percentile) and also contained high percentages of limited English speaking households or minorities in the top quintile for the state.[^Dayquintile] Approximately `r LEHDay %>% format(., big.mark = ",") %>% pull()` limited English households and `r MINDay %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHDayPct`% and `r MINDayPct`%, respectively, of those populations in the state. By comparison, `r HHDayPct`% of all households, and `r PopDayPct`% of the general population were resident in these same Block Groups.  
...
LSTDayEngLang5 <- me_blkgrpLST_sf %>%
  filter(LSTDayEngLangScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/me_LSTDayEngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayEngLang5.png")
```
...
LSTDayMinority5 <- me_blkgrpLST_sf %>%
  filter(LSTDayMinorScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/me_LSTDayMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayMinority5.png")
```
\pagebreak
...
Nighttime Land Surface Temperatures (LST) represent LST values collected during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. Nighttime average LST for the state was `r round(mean(me_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`°. Nighttime Land Surface Temperatures (LST) varied significantly across Maine, with highest temperatures apparent in the southeastern part of the state, particularly Kittery and York, Portland, Brunswick, Augusta, Waterville, and Bar Harbor (see Figure \@ref(fig:mapLSTnightME) below). 

```{r mapLSTnightME, fig.align = "center", fig.cap="Map of July-August 2019 average nighttime Land Surface Temperatures (LST) across Maine at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Maine
m <- tm_shape(me_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/me_NightLST.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_NightLST.png")
```

```{r LSTNightoutliers, include=FALSE}
me_LSTNightoutliers <- me_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTnightME) is a boxplot of average nighttime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(me_blkgrpLST_sf$meanNightLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large dots on either end represents outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r me_LSTNightoutliers`. 

```{r boxplotLSTnightME, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Land Surface Temperatures (LST) across Maine at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
The UHI effect for nighttime average temperatures in Maine are most pronounced in the southeast region of the state, especially around Kittery, York, and Portland (see Figure \@ref(fig:mapUHInightME)). Temperatures in these areas are up to `r round(max(me_blkgrpLST_sf$UHINight,na.rm = TRUE),1)`° warmer than the rural background average. The warm region northwest of Portland is due to Sebago Lake. 

```{r mapUHInightME, fig.align = "center", fig.cap="Map of July-Aug 2019 nighttime Urban Heat Island (UHI) effect across Maine at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Maine
m <- tm_shape(me_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/me_NightUHI.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_NightUHI.png")
```
...
Figure \@ref(fig:boxplotUHINightME) is a boxplot of nighttime Urban Heat Island (UHI) effects by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(me_blkgrpLST_sf$UHINight, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large black dots represent outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r me_LSTNightoutliers`.

```{r boxplotUHINightME, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Urban Heat Island (UHI) across Maine at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHINight) %>% 
...
# Pop Weighted avg of PM2.5 for all Groups in Maine relative to ME average
LSTNightPopTab <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Maine") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTNightTempPopAvgME) and Table \@ref(tab:tabLSTNightPopAvgME) show population-weighted exposures for populations of concern relative to the average nighttime average LST of `r round(mean(me_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`° for the state. For example, limited English speaking households in Maine were exposed to a population-weighted nighttime average LST of `r round(max(LSTNightPop$LSTNightwMean),1)`°, approximately `r round(max(LSTNightPop$LSTNightwMean) - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[1,4]`) above the state nighttime average LST. Similarly, minorities were exposed to a population-weighted nighttime average LST of `r LSTNightPop %>% filter(Group == "Minority") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, over `r round(LSTNightPop %>% filter(Group == "Minority") %>% summarize(max(LSTNightwMean)) %>% pull() - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[3,4]`) above the state nighttime average LST. By contrast, adults without a high school diploma were, on average, exposed to population-weighted nighttime average LSTs `r LSTNightPop %>% filter(Group == "No HS Dip") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, which is `r LSTNightPopTab[2,3]` (`r LSTNightPopTab[2,4]`) below the state average.

```{r plotLSTNightTempPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to average nighttime Land Surfact Temperatures for populations of concern in Maine relative to the state average."}
LSTNightPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 61.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 60.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 66.9, yend = 67.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(60,62))
```


```{r tabLSTNightPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime LST for populations of concern in Maine relative to the state average."}
LSTNightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime LST is ", round(mean(me_blkgrpLST_sf$meanNightLST,na.rm=TRUE),2), "°"))
```

```{r UHINightPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Maine relative to ME average
UHINightPop <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Maine") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
# Pop Weighted avg of PM2.5 for all Groups in Maine relative to ME average
UHINightPopTab <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Maine") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
Figure \@ref(fig:plotUHINightTempPopAvgME) and Table \@ref(tab:tabUHINightPopAvgME) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(me_blkgrpLST_sf$UHINight,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Maine were exposed to a population-weighted daytime average UHI effect of `r round(max(UHINightPop$UHINightwMean),1)`° above the rural average. This value is more than `r round(max(UHINightPop$UHINightwMean) - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[1,4]`) above the state daytime average UHI. Similarly, minorities were exposed to a population-weighted daytime average UHI effect of `r UHINightPop %>% filter(Group == "Minority") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, approximately `r round(UHINightPop %>% filter(Group == "Minority") %>% summarize(max(UHINightwMean)) %>% pull() - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[3,4]`) above the state daytime average UHI. By contrast, adults without a high school diploma were, on average, exposed to population-weighted daytime average UHI of `r UHINightPop %>% filter(Group == "No HS Dip") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, which is `r UHINightPopTab[2,3]` (`r UHINightPopTab[2,4]`) below the state average.

```{r plotUHINightTempPopAvgME, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Maine."}
UHINightPop %>% 
...
```{r tabUHINightPopAvgME, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Maine relative to the rural average."}
UHINightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime UHI effect is ", round(mean(me_blkgrpLST_sf$UHINight,na.rm=TRUE),2), "°"))
```

```{r LSTNightHighRiskStats, include=FALSE}
PopNight <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskLEH) and \@ref(fig:mapLSTNightHighRiskMIN) highlight Census Block Groups across Maine which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and also contained high percentages of limited English speaking households or minorities in the top quintile for the state.[^Nightquintile] Approximately `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English households and `r MINNight %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHNightPct`% and `r MINNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 
...
LSTNightEngLang5 <- me_blkgrpLST_sf %>%
  filter(LSTNightEngLangScore == 5)
...
tmap_save(m, "DATA/LST/me_LSTNightEngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightEngLang5.png")
```
...
LSTNightMinority5 <- me_blkgrpLST_sf %>%
  filter(LSTNightMinorScore == 5)
...
tmap_save(m, "DATA/LST/me_LSTNightMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightMinority5.png")
```
\pagebreak
...
[^Williams2020]: See Augusta A. Williams, Joseph G. Allen, Paul J. Catalano, and John D. Spengler. 2020. "The Role of Individual and Small-Area Social and Environmental Factors on Heat Vulnerability to Mortality Within and Outside of the Home in Boston, ME." *Climate* 8(2): 29. https://doi.org/10.3390/cli8020029
...
[^LST24quintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of day-night average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(me_blkgrpLST_sf$meanAvgLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(me_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 

[^Dayquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(me_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(me_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 
...
[^Nightquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(me_blkgrpLST_sf$meanNightLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(me_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region.
...
The LST rasters were then mosaiced to create a single scene for each time period and clipped to Maine. 
...
isd_me_DayNight_sf <- readRDS("DATA/LST/isd_ne_DayNight_sf.Rds") %>% 
  st_transform(., crs = st_crs(me_state_sf_cb)) %>% 
  st_intersection(me_state_sf_cb,.) 
```

In order to validate land surface temperature (LST) values, MODIS LST data was compared to ground-based air temperature measurements for the same time periods across Maine. Weather data was acquired from NOAA's Integrated Surface Data ('ISD') files from the NOAA National Centers for Environmental Information (https://www.ncdc.noaa.gov/isd/data-access) using the Commmon Access tool (https://www.ncei.noaa.gov/access/search/data-search/global-hourly). This yielded `r nrow(isd_me_DayNight_sf)` weather stations across Maine with air temperature data available for the same period as the MODIS LST data (i.e., July 28 - August 4, 2019; see Figure \@ref(fig:ISDstations)). 

```{r ISDstations, fig.align = "center", fig.cap="NOAA Integrated Surface Data Weather Stations across Maine."}
tm_shape(me_state_sf_cb, unit = "mi") + tm_borders(lwd = 0.2) +
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(isd_me_DayNight_sf) + tm_dots(shape = 4, size = 0.08) +
...
cor_day <- cor.test(x = isd_me_DayNight_sf$gridDay, y = isd_me_DayNight_sf$tempFday, use = "pairwise.complete.obs", method = "spearman")
...
cor_night <- cor.test(x = isd_me_DayNight_sf$gridNight, y = isd_me_DayNight_sf$tempFnight, use = "pairwise.complete.obs", method = "spearman")
...
cor_daynight <- cor.test(x = isd_me_DayNight_sf$gridDayNight, y = isd_me_DayNight_sf$tempFdaynight, use = "pairwise.complete.obs", method = "spearman")
...
# Compare day average LST to weather station air temp data for same time period
lm_day <- lm(tempFday ~ gridDay, data = isd_me_DayNight_sf)
RMSE_day <- sqrt(mean(lm_day$residuals^2))

# Compare night average LST to weather station air temp data for same time period
lm_night <- lm(tempFnight ~ gridNight, data = isd_me_DayNight_sf)
RMSE_night <- sqrt(mean(lm_night$residuals^2))

# Compare day-night average LST to weather station air temp data for same time period
lm_daynight <- lm(tempFdaynight ~ gridDayNight, data = isd_me_DayNight_sf)
RMSE_daynight <- sqrt(mean(lm_daynight$residuals^2))
```

Linear models are statistically significant predictors of air temperature data (see Figure \@ref(fig:LSTvalidation)). Day-Night and Day Average LST values are better predictors of air temperature than Night LST. The latter has a Root Mean Square Error (RMSE) of `r round(RMSE_night,2)`°, while Day-Night Avg LST has a RMSE of `r round(RMSE_daynight,2)`°, indicating modest deviation of predicted air temperatures from observed values. These prediction error levels are better than average when compared with published research on the use of MODIS LST as a proxy for air temperature.

```{r LSTvalidation, warning=FALSE, message=FALSE, fig.align = "center", fig.cap="Comparison of MODIS LST values and air temperatures for same time periods from NOAA Integrated Surface Data Weather Stations across Maine."}

day <- isd_me_DayNight_sf %>% 
...
g <- arrangeGrob(day,night,daynight,ncol=1,
                 ...
                 widths = unit(5,"inches"))
ggsave(g, filename = "DATA/LST/me_validate.png", 
       width = 5, height = 8, units = "in")
knitr::include_graphics("DATA/LST/me_validate.png")
```


### Urban Heat Island effects {-}

Urban Heat Island (UHI) effects are measured as the difference between LST raster values across Maine for each time period and the average LST for rural states for the same time periods. It is represented by the following formula:

*UHI effect = LST - Mean Rural LST*

For example, to calculate the UHI effects for daytime temperatures, daytime LST pixels falling within the rural states of Maine were all averaged to provide a mean rural LST benchmark value. This mean rural LST value was then subtracted from the daytime LST values of all pixels across the state. The resulting value for each pixel indicates the magnitude or size of the difference, and the sign (+/-) indicates whether it is warmer or cooler than the mean rural LST.

Rural states of Maine were identified according to the U.S. Census Bureau's urban-rural classification. The Census Bureau identifies two types of urban areas:
...
Urban areas TIGER/Line spatial files were downloaded from the Census Bureau via API using the `tigris` package in R. Areas of Maine not within the urban areas polygons were then extracted to represent rural states. LST raster pixels falling within these non-urban or rual polygons were extracted and a mean LST value calculated for each time period (i.e., day, night, and day-night average) to represent the mean rural LST benchmark for calculating the UHI effect.
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "Maine Low Income threshold: 25% or more of households with median household incomes below 65% statewide median",
                                     "Maine Language Isolation threshold: 25% or more people in households where all adults speak English less than \"very well\"",
                                     "Maine Minority threshold: 25% or more are persons of Hispanic or Latino origin or persons who are not White",
...
tmap_save(m, "DATA/LST/me_LST24LowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24LowInc5.png")
```
...
tmap_save(m, "DATA/LST/me_LST24NoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24NoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/me_LST24Over645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24Over645.png")
```
...
tmap_save(m, "DATA/LST/me_LST24Under55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LST24Under55.png")
```
...
tmap_save(m, "DATA/LST/me_LSTDayLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayLowInc5.png")
```
...
LSTDayNoHSDip5 <- me_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/me_LSTDayNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayNoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/me_LSTDayOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayOver645.png")
```
...
tmap_save(m, "DATA/LST/me_LSTDayUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTDayUnder55.png")
```
...
LSTNightLowInc5 <- me_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/me_LSTNightLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightLowInc5.png")
```
...
LSTNightNoHSDip5 <- me_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/me_LSTNightNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightNoHSDip5.png")
```
...
LSTNightOver645 <- me_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/me_LSTNightOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightOver645.png")
```
...
LSTNightUnder55 <- me_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/me_LSTNightUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/me_LSTNightUnder55.png")
```
...
```{r cormatrixLSTpopsME, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and the proportions of populations of concern by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTpops <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
```{r cormatrixLSTAvgMEpollutants, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and pollution by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTemissions <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  dplyr::transmute(`Day-Night Avg LST` = meanAvgLST,
                   `Day LST` = meanDayLST,
                   `Night LST` = meanNightLST,
...
# 
# # convert to sp
# me_blkgrpLST_sp <- as_Spatial(me_blkgrpLST_sf)
# 
# #calculate kernel bandwidth
# GWRbandwidth <- gwr.sel(me_blkgrpLST_sp$PM25_19 ~ me_blkgrpLST_sp$meanAvgLST, data=me_blkgrpLST_sp, adapt=T)
