---
title: "Transportation Emissions in Maine"
...
PM~2.5~ levels vary significantly across Maine, with highest concentrations in the southeast from Saco to Bangor along I-95 (see Figure \@ref(fig:mapPM25RI) below). 
...
```{r mapPM25RI, fig.align = "center", fig.cap="Map of 2016 annual PM2.5 concentrations across Maine at Census Block Group level."}
...
m1 <- tm_shape(me_blkgrp_sf, unit = "mi") + 
  ...
  tm_symbols(shape = I295, border.lwd = NA, size = 0.2) +
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {round(mean(me_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {round(min(me_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%, have been in the south (around Saco) and northeast (around Presque Isle) of the state. 

```{r mapPM25RIchange, fig.align = "center", fig.cap="Map of percent change in annual PM2.5 concentrations across Maine between 2011 and 2016 at Census Block Group level."}
...
Figure \@ref(fig:slopegraphPM25RI) below compares the average annual PM~2.5~ concentrations for the state and the region between 2011 and 2016. The region and the state both showed significant declines since 2011. Concentrations in Maine remain below the rest of the region. 

```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted exposure to PM2.5 between 2011 and 2016 for Maine and New England."}
...
me_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Low Income", y = 1, label = "Above state avg"),
            color = "gray48") +
...
Ozone (O~3~) levels vary significantly across Maine, with highest concentrations along the coast from Kittery to Bar Harbor (see Figure \@ref(fig:mapO3RI) below).[^NONATTAINMENT] 

```{r mapO3RI, fig.align = "center", fig.cap="Map of 2016 Ozone summer seasonal average of daily maximum 8-hour concentration in air in parts per billion across Maine at Census Block Group level."}
...
Since 2011, Ozone (O~3~) levels have decreased slightly across the state, on average by `r {round(mean(me_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)}`%. These have changes not been uniform (see Figure \@ref(fig:mapO3RIchange) below). The greatest declines, up to `r round(min(me_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`%, have been in the north central part of the state. By contrast, increases in summer ozone concentrations of up to `r round(max(me_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`% appear south of Bethel and also around Bangor.

```{r mapO3RIchange, fig.align = "center", fig.cap="Map of percent change in summer seasonal Ozone concentrations across Maine between 2011 and 2016 at Census Block Group level."}
...
Figure \@ref(fig:slopegraphO3RI) below compares the average summer Ozone (O~3~) concentrations for the state and for the region between 2011 and 2016. Maine's average concentrations have decreased and remain lower than the region. 

```{r slopegraphO3RI, fig.align = "center", fig.cap="Change in summer average ozone concentrations between 2011 and 2016 for Maine and New England."}
...
In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the region. For example, persons over age 64 in Maine are exposed to summer Ozone concentrations that are approximately 0.5% above concentrations for the state as a whole. By contrast, adults without a high school diploma are, on average, exposed to concentrations of Ozone at or below the state average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in Maine relative to the regional average."}
...
me_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Minority", y = 1, label = "Above state avg"),
            color = "gray48") +
...
Like the region as a whole, all populations have experienced declines in exposure since 2011. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvgRI).  
...
```{r mapCO2RI, fig.align = "center", fig.cap="Map of 2017 Carbon Dioxide (CO2) annual on-road emissions in metric tons per square kilometer across Maine at Census Block Group level."}
...
Significant hot spots, or clusters of high CO~2~ emissions, appear in Portland and Bangor (see Figure \@ref(fig:hotspotCO2RI) below).
...
Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(me_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(me_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(me_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2RIchange) below). Declines have occurred in the west and northeast. Increased emissions have occurred in throughout the south and central areas of the state, with the greatest emissions growth in the south between Kittery and Portland.

```{r mapCO2RIchange, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across Maine between 1990 and 2017 at Census Block Group level."}
...
```{r linegraphCO2RI, fig.align = "center", fig.cap="Total CO2 emissions 1990 to 2017 for Maine."}
...
Maine saw dramatic growth in CO~2~ emissions between 1990 and 2003 and again between 2009 and 2016. Moreover, emissions growth has exceeded population growth. This is reflected in the increase in per capita emissions since 1990 (see last three columns in Table \@ref(tab:changeCO2RI)).  
...
Diesel Particulate Matter (DPM) emissions are concentrated along the I-95 corridor from Kittery to Bangor (see Figure \@ref(fig:mapDPMRI) below). 

```{r mapDPMRI, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Maine at Census Block Group level."}
...
There are significant spatial clusters of high Diesel Particulate Matter concentrations in Portland (see Figure \@ref(fig:hotspotDPMRI) below).
...
```{r statsDPMRI, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Maine at Census Block Group level."}
me_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
In addition to variations in the general geography of Diesel Particulate Matter concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Diesel Particulate Matter concentraions for the state. For example, limited English households in Maine are exposed to  Diesel Particulate Matter concentrations that are more than 74% above concentrations for the state as a whole. Similarly, minorities are exposed to concentrations more than 39% above the state average. By contrast, adults without a high school diploma are, on average, exposed to concentrations of Diesel Particulate Matter below the state average. 

```{r barplotDPMPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of Diesel Particulate Matter across Maine relative to the regional average."}
...
me_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 20, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Low Income", xend = "Low Income", y = 5, yend = 25), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-15,80))
```

There is a weak positive correlation between the proportions of minorities and ambient concentrations of Diesel Particulate Matter (see Appendix).
...
Air Toxics Cancer Risk are concentrated along the I-95 corridor from Kittery to Bangor (see Figure \@ref(fig:mapATCNE) below).
...
There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics in municipalities along the I-95 corridor, especially Kittery, Saco, and Portland (see Figure \@ref(fig:hotspotATCRI) below).
...
me_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 2, label = "Above state avg"),
            color = "gray48") +
...
There is a weak positive correlation between the proportions of minorities and cancer risk (see Appendix).
...
Higher Respiratory Hazard Indices are concentrated along the I-95 corridor from Kittery to Bangor, and also north of Bethel (see Figure \@ref(fig:mapRHIRI) below). 

```{r mapRHIRI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Maine at Census Block Group level."}
...
There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics in Portland (see Figure \@ref(fig:hotspotRHIRI) below).
...
```{r statsRHIRI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Maine at Census Block Group level."}
me_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
Figure \@ref(fig:boxplotRHIRI) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(me_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.    

```{r boxplotRHIRI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Maine by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
me_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Respiratory Hazard Index from Inhalation of Air Toxics for the state. For example, limited English households in Maine experience respiratory hazard indices from inhalation of air toxics that are over 10% above the state as a whole. Similarly, minorities experience lifetime hazard index values more than 7% above the state average. By contrast, persons over age 64 are exposed to an avearge hazard index below the state average. 

```{r barplotRHIPopAvgRI, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Maine at Census Block Group level relative to the regional average."}
...
There is a weak positive correlation between the proportions of minorities and respiratory hazard index (see Appendix).
...
```{r mapTPVRI, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Maine at Census Block Group level."}
...
There appear to be significant spatial clusters of traffic proximity and volume in Portland and Lewiston (see Figure \@ref(fig:hotspotTPVRI) below).
...
```{r statsTPVRI, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Maine at Census Block Group level."}
me_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
Figure \@ref(fig:boxplotTPVRI) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(me_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPVRI, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Maine at Census Block Group level."}
...
me_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 50, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Under 5", xend = "Under 5", y = 20, yend = 60), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-30,200))
```
