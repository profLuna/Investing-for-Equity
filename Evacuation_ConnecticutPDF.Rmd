...
save(ct_blkgrps_nfhza,ct_tracts_nfhza,
...
ct_FloodPops_df %>% 
  ...
  arrange(-FloodPop) %>% 
  mutate(Group = ifelse(Group == "Minority", "People of Color", Group)) %>% 
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','),
...
ct_FloodPops_df %>% 
  ...
  geom_text(aes(x = "No HS Dip", y = 7.75, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = 6.8, label = "Below state\naverage"),
            color = "gray48") +
...
# save data for later analysis
save(ct_blkgrps_hevac, ct_tracts_hevac, 
