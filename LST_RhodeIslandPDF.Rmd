...
ggsave("DATA/LST/ri_DayLST_lolli.png")
```
...
LSTDayMinorityRI5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Minority in Highest Quintile of Daytime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"RI Minority*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskRIMIN) and \@ref(fig:mapLSTNightHighRiskLEH) highlight Census Block Groups across Rhode Island which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and were also designated as Environmental Justice communities for minorities or were in the top quintile for limited English speaking households.[^Nightquintile] Approximately `r RIMINNight %>% format(., big.mark = ",") %>% pull()` minorities and `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English speaking households were resident in these highest quintile Block Groups. These numbers represent `r RIMINNightPct`% and `r LEHNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLSTNightHighRiskRIMIN) and \@ref(tab:tabLSTNightHighRiskLEH) show breakdowns by municipality of how many limited English speaking households and minorities these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLSTNightHighRiskRIINC) to \@ref(fig:mapLSTNightHighRiskO64) and Tables \@ref(tab:tabLSTNightHighRiskRIINC) to \@ref(tab:tabLSTNightHighRiskO64) in Appendix B.
...
\pagebreak
```{r tabLSTNightHighRiskRIMIN, fig.align = "center", fig.cap="Minorities, as defined by Rhode Island Environmental Justice Policy, in Census Block Groups at or above 80th percentile of nighttime average Land Surface Temperature."}
LSTNightMinorityRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Minority Persons in Highest Quintile of Night Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"RI Minority*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
