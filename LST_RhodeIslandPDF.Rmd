...
statepopsdf <- ri_blkgrpLST_sf %>% 
  ...
  mutate(RI_ENGLISH = if_else(RI_ENGLISH == "E", totalpopE,0)) %>%
  mutate(RI_ENGLISH = replace_na(RI_ENGLISH,0)) %>% 
  group_by(STATE) %>% 
  summarize(TotalPop = sum(totalpopE, na.rm = TRUE),
            ...
            TotalRI_ENGLISH = sum(RI_ENGLISH, na.rm = TRUE))
...
m <- tm_shape(ri_blkgrpLST_sf, unit = "mi") + 
  ...
  tm_shape(ri_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
LST24hrPop <- ri_blkgrpLST_sf %>% 
  ...
  dplyr::select(totalpopE,
                ...
                RI_ENGLISH,
                meanAvgLST) %>% 
  gather(key = Group, value = Pop, totalpopE:RI_ENGLISH) %>% 
  group_by(Group) %>% 
  summarize(LST24hrwMean = weighted.mean(x = meanAvgLST, w = Pop, na.rm = TRUE),
            LST24hrMean = mean(meanAvgLST, na.rm = TRUE)) %>% 
  filter(Group %in% c("minorityE","eng_limitE","num2povE","lthsE","under5E","over64E","RI_LOWINC","RI_MINORITY","RI_ENGLISH")) %>% 
  mutate(Group = case_when(
    ...
    Group == "RI_ENGLISH" ~ "RI Limited English HH"))
...
LST24hrPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/LST24hrMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/LST24hrMean - 1)*100) %>%
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLST24hrTempPopAvgRI) and Table \@ref(tab:tabLST24hrPopAvgRI) show population-weighted exposures for populations of concern relative to the day-night average LST for the state of `r round(mean(ri_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average LST of `r round(max(LST24hrPop$LST24hrwMean),1)`°, approximately `r round(max(LST24hrPop$LST24hrwMean) - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[1,4]`) above the state day-night average LST. Similarly, minorities, as defined by the state's environmental justice policy,  were exposed to a population-weighted day-night average LST of `r LST24hrPop %>% filter(Group == "RI Minority") %>% summarize(round(max(LST24hrwMean),1)) %>% pull()`°, over `r round(LST24hrPop %>% filter(Group == "RI Minority") %>% summarize(max(LST24hrwMean)) %>% pull() - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[3,4]`) above the state day-night average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted day-night average LSTs of `r LST24hrPopTab[8,3]` below the state average.

```{r plotLST24hrTempPopAvgRI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Rhode Island relative to the state average."}
LST24hrPop %>% 
...
```
\pagebreak
```{r tabLST24hrPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average LST for populations of concern in Rhode Island relative to the state average."}
LST24hrPopTab %>% 
  arrange(Group) %>% 
...
UHI24hrPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/UHI24hrMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/UHI24hrMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHI24hrTempPopAvgRI) and Table \@ref(tab:tabUHI24hrPopAvgRI) show population-weighted exposures for populations of concern relative to the day-night average Urban Heat Island (UHI) effect for Rhode Island. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across Rhode Island is approximately `r round(mean(ri_blkgrpLST_sf$UHI24avg,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average UHI effect of `r round(max(UHI24hrPop$UHI24hrwMean),1)`° above the rural average. This value is more than `r round(max(UHI24hrPop$UHI24hrwMean) - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[1,4]`) above the state day-night average UHI. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average UHI effect of `r UHI24hrPop %>% filter(Group == "RI Minority") %>% summarize(round(max(UHI24hrwMean),1)) %>% pull()`°, approximately `r round(UHI24hrPop %>% filter(Group == "RI Minority") %>% summarize(max(UHI24hrwMean)) %>% pull() - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[5,4]`) above the state day-night average UHI. By contrast, persons over age 64 were, on average, were exposed to a population-weighted day-night average UHI `r UHI24hrPopTab[8,3]` (`r UHI24hrPopTab[8,4]`) below the state average.

```{r plotUHI24hrTempPopAvgRI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Rhode Island."}
UHI24hrPop %>% 
...
```{r tabUHI24hrPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Rhode Island relative to the rural average."}
UHI24hrPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
Pop24hr <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(RI_ENGLISH == "E" & pctile24hrLST >= 0.8) %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLST24hrHighRiskRILEH) and \@ref(fig:mapLST24hrHighRiskRIMIN) highlight Census Block Groups across Rhode Island which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and were also categorized as Environmental Justice communities for limited English speaking households or minorities.[^LST24quintile] Approximately `r LEH24hr %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r MIN24hr %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEH24hrPct`% and `r MIN24hrPct`%, respectively, of those populations in the state. By comparison, `r HH24hrPct`% of all households, and `r Pop24hrPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLST24hrHighRiskRILEH) and \@ref(tab:tabLST24hrHighRiskRIMIN) show breakdowns by municipality of how many limited English speaking households and minorities these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLST24hrHighRiskLEH) to \@ref(fig:mapLST24hrHighRiskU5) and Tables \@ref(tab:tabLST24hrHighRiskLEH) to \@ref(tab:tabLST24hrHighRiskU5) in Appendix B.
...
LST24EngLangRI5 <- ri_blkgrpLST_sf %>%
  filter(RI_ENGLISH == "E" & pctile24hrLST >= 0.8)

bbox_new <- st_bbox(LST24EngLangRI5) # current bounding box
...
m <- tm_shape(LST24EngLangRI5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ri_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ri_LST24EngLangRI5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LST24EngLangRI5.png")
```

\pagebreak
```{r tabLST24hrHighRiskRILEH, fig.align = "center", fig.cap="Limited English Housheolds in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
LST24EngLangRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    ...
                    digits = 1,
                    caption = "RI Limited English Speaking Households in Highest Quintile of Day-Night Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Limited English Households*",names(.)[4:7])) %>% 
...
```{r mapLST24hrHighRiskRIMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and 80th percentile of percentage of Minorities, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
...
\pagebreak
```{r tabLST24hrHighRiskRIMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
...
LST24MinorityRI5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Minorities in Highest Quintile of Day-Night Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Minorities*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day-Night LST (°F)" = 3)) %>% 
  kableExtra::footnote(., symbol = "As defined by Rhode Island Environmental Justice Policy")
...
Daytime Average Land Surface Temperatures (LST) represent LST values collected during the day (11:48am - 2pm) over eight days, from July 28 to August 4, 2019. Daytime average LST for the state was `r round(mean(ri_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. Daytime Land Surface Temperatures (LST) varied significantly across Rhode Island, with highest temperatures apparent in the most urbanized areas of the state, particularly within Route 128/I-95, including the Boston area, and along I-91 from Springfield to Holyoke. Other areas of high temperature appear around Worcester, Lawrence, Lowell, Brockton, Fall River, and New Bedford (see Figure \@ref(fig:mapLSTdayRI) below). Across Rhode Island, approximately `r ri_blkgrpLST_sf %>% as.data.frame() %>% filter(pctileDayLST >= 0.8) %>% group_by() %>% summarize(sum(totalpopE, na.rm = TRUE)/10^6) %>% pull() %>% round(.,2)` million people live in Block Groups that exceeded the 80th percentile (temperatures over `r round(quantile(ri_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),1)`°) for daytime average temperatures.  
...
LSTDayPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/LSTDayMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/LSTDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTDayTempPopAvgRI) and Table \@ref(tab:tabLSTDayPopAvgRI) show population-weighted exposures for populations of concern relative to the average daytime average LST for the state, which is `r round(mean(ri_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average LST of `r round(max(LSTDayPop$LSTDaywMean),1)`°, approximately `r round(max(LSTDayPop$LSTDaywMean) - max(LSTDayPop$LSTDayMean),1)`° (`r LSTDayPopTab[3,4]`) above the state daytime average LST. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average LST of approximately `r LSTDayPop %>% filter(Group == "RI Minority") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, `r round(LSTDayPop %>% filter(Group == "RI Minority") %>% summarize(max(LSTDaywMean)) %>% pull() - LSTDayPop %>% summarize(max(LSTDayMean)) %>% pull(),1)`° (`r LSTDayPopTab[5,4]`) above the state daytime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average LSTs of `r LSTDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, approximately `r LSTDayPopTab[8,3]` below the state average.

```{r plotLSTDayTempPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to average daytime Land Surface Temperatures for populations of concern in Rhode Island relative to the state average."}
LSTDayPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 97, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 91, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 94, yend = 97), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(90,99))
```

\pagebreak
```{r tabLSTDayPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Daytime LST for populations of concern in Rhode Island relative to the state average."}
LSTDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
UHIDayPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/UHIDayMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/UHIDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHIDayTempPopAvgRI) and Table \@ref(tab:tabUHIDayPopAvgRI) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ri_blkgrpLST_sf$UHIDay,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r round(max(UHIDayPop$UHIDaywMean),1)`° above the rural average. This value is more than `r round(max(UHIDayPop$UHIDaywMean) - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[3,4]`) above the state daytime average UHI. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r UHIDayPop %>% filter(Group == "RI Minority") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`°, approximately `r round(UHIDayPop %>% filter(Group == "RI Minority") %>% summarize(max(UHIDaywMean)) %>% pull() - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[5,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI `r UHIDayPopTab[8,3]`° (`r UHIDayPopTab[8,4]`) below the state average.

```{r plotUHIDayTempPopAvgRI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Rhode Island."}
UHIDayPop %>% 
...
```{r tabUHIDayPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Rhode Island relative to the rural average."}
UHIDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
PopDay <- ri_blkgrpLST_sf %>% 
  ...
  filter(RI_ENGLISH == "E" & pctileDayLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTDayHighRiskRILEH) and \@ref(fig:mapLSTDayHighRiskRIMIN) highlight Census Block Groups across Rhode Island which exhibited daytime average LSTs in the top quintile (i.e., 80th percentile) and were categorized as Environmental Justice communities for limited English speaking households or minorities.[^Dayquintile] Approximately `r LEHDay %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r MINDay %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHDayPct`% and `r MINDayPct`%, respectively, of those populations in the state. By comparison, `r HHDayPct`% of all households, and `r PopDayPct`% of the general population were resident in these same Block Groups.  

Tables \@ref(tab:tabLSTDayHighRiskRILEH) and \@ref(tab:tabLSTDayHighRiskRIMIN) show breakdowns by municipality of the numbers of limited English speaking households and minorities resident in these top quintile Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLSTDayHighRiskLEH) to \@ref(fig:mapLSTDayHighRiskU5) and Tables \@ref(tab:tabLSTDayHighRiskLEH) to \@ref(tab:tabLSTDayHighRiskU5) in Appendix B.

```{r mapLSTDayHighRiskRILEH, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and 80th percentile of percentage of Limited English Speaking Households, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTDayEngLangRI5 <- ri_blkgrpLST_sf %>%
  filter(RI_ENGLISH == "E" & pctileDayLST >= 0.8)

bbox_new <- st_bbox(LSTDayEngLangRI5) # current bounding box
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ri_LSTDayEngLangRI5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LSTDayEngLangRI5.png")
```

\pagebreak
```{r tabLSTDayHighRiskRILEH, fig.align = "center", fig.cap="Limited English Housheolds in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayEngLangRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Limited English Speaking Households in Highest Quintile of Daytime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Limited English Households*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
```{r mapLSTDayHighRiskRIMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and 80th percentile of percentage of Minorities, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
...
```
\pagebreak
```{r tabLSTDayHighRiskRIMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayMinorityRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Minorities in Highest Quintile of Daytime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Minorities*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Daytime LST (°F)" = 3)) %>% 
...
The UHI effect for nighttime average temperatures in Rhode Island are most pronounced in areas immediately north and west of Boston, as well as in Springfield, Lawrence, Lowell, Buzzards Bay, especially around Fall River and New Bedford, and on the Cape (see Figure \@ref(fig:mapUHInightRI)). Temperatures in these areas are up to `r round(max(ri_blkgrpLST_sf$UHINight,na.rm = TRUE),1)`° warmer than the rural background average. By contrast, most of western Rhode Island is cooler than rural background average. 
...
LSTNightPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/LSTNightMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/LSTNightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTNightTempPopAvgRI) and Table \@ref(tab:tabLSTNightPopAvgRI) show population-weighted exposures for populations of concern relative to the average nighttime average LST of `r round(mean(ri_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`° for the state. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted nighttime average LST of `r round(max(LSTNightPop$LSTNightwMean),1)`°, approximately `r round(max(LSTNightPop$LSTNightwMean) - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[3,4]`) above the state nighttime average LST. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted nighttime average LST of `r LSTNightPop %>% filter(Group == "RI Minority") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, over `r round(LSTNightPop %>% filter(Group == "RI Minority") %>% summarize(max(LSTNightwMean)) %>% pull() - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[5,4]`) above the state nighttime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted nighttime average LSTs of `r LSTNightPopTab[8,2]`, approximately `r LSTNightPopTab[8,3]` below the state average.

```{r plotLSTNightTempPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to average nighttime Land Surfact Temperatures for populations of concern in Rhode Island relative to the state average."}
LSTNightPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 67.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 66.25, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 66.9, yend = 67.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(66,68))
```

\pagebreak
```{r tabLSTNightPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime LST for populations of concern in Rhode Island relative to the state average."}
LSTNightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
UHINightPopTab <- ri_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/UHINightMean - 1)*100,
         ...
         `RI Limited English HH` = (RI_ENGLISH/UHINightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHINightTempPopAvgRI) and Table \@ref(tab:tabUHINightPopAvgRI) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ri_blkgrpLST_sf$UHINight,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Rhode Island, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r round(max(UHINightPop$UHINightwMean),1)`° above the rural average. This value is more than `r round(max(UHINightPop$UHINightwMean) - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[3,4]`) above the state daytime average UHI. Similarly, minorities were exposed to a population-weighted daytime average UHI effect of `r UHINightPop %>% filter(Group == "RI Minority") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, approximately `r round(UHINightPop %>% filter(Group == "RI Minority") %>% summarize(max(UHINightwMean)) %>% pull() - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[5,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI of `r UHINightPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`° (`r UHINightPopTab[8,4]`) below the state average.

```{r plotUHINightTempPopAvgRI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Rhode Island."}
UHINightPop %>% 
...
```{r tabUHINightPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Rhode Island relative to the rural average."}
UHINightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
...
PopNight <- ri_blkgrpLST_sf %>% 
  ...
  filter(RI_ENGLISH == "E" & pctileNightLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskRILEH) and \@ref(fig:mapLSTNightHighRiskRIMIN) highlight Census Block Groups across Rhode Island which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and were also designated as Environmental Justice communities for limited English speaking households or minorities.[^Nightquintile] Approximately `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r MINNight %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHNightPct`% and `r MINNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLSTNightHighRiskRILEH) and \@ref(tab:tabLSTNightHighRiskRIMIN) show breakdowns by municipality of how many limited English speaking households and minorities these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLSTNightHighRiskLEH) to \@ref(fig:mapLSTNightHighRiskU5) and Tables \@ref(tab:tabLSTNightHighRiskLEH) to \@ref(tab:tabLSTNightHighRiskU5) in Appendix B.

```{r mapLSTNightHighRiskRILEH, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and 80th percentile of percentage of Limited English Speaking Households, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTNightEngLangRI5 <- ri_blkgrpLST_sf %>%
  filter(RI_ENGLISH == "E" & pctileNightLST >= 0.8)

bbox_new <- st_bbox(LSTNightEngLangRI5) # current bounding box
...
tmap_save(m, "DATA/LST/ri_LSTNightEngLangRI5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LSTNightEngLangRI5.png")
```

\pagebreak
```{r tabLSTNightHighRiskRILEH, fig.align = "center", fig.cap="Limited English Speaking Housheolds in Census Block Groups at or above 80th percentile of nighttime average Land Surface Temperature."}
LSTNightEngLangRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Limited English Speaking Households in Highest Quintile of Nighttime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Limited English Households*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
```{r mapLSTNightHighRiskRIMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and 80th percentile of percentage of Minorities, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
...
```
\pagebreak
```{r tabLSTNightHighRiskRIMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of Nighttime average Land Surface Temperature."}
LSTNightMinorityRI5 %>% 
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Minorities in Highest Quintile of Nighttime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Minorities*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3)) %>% 
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "Rhode Island Low Income threshold: 25% or more of households with median household incomes below 65% statewide median",
                                     "Rhode Island Language Isolation threshold: 25% or more people in households where all adults speak English less than \"very well\"",
                                     "Rhode Island Minority threshold: 25% or more are persons of Hispanic or Latino origin or persons who are not White",
...
LST24Minority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "Minorities in Highest Quintile of Day-Night Avg LST by Municipality", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day-Night LST (°F)" = 3))
```

```{r mapLST24hrHighRiskRILI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and Environmental Justice communities of Low Income persons, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LST24LowIncRI5 <- ri_blkgrpLST_sf %>%
  filter(RI_INCOME == "I" & pctile24hrLST >= 0.8)

bbox_new <- st_bbox(LST24LowIncRI5) # current bounding box
...
m <- tm_shape(LST24LowIncRI5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ri_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ri_LST24LowIncRI5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LST24LowIncRI5.png")
```
\pagebreak
```{r tabLST24hrHighRiskRILI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
LST24LowIncRI5 %>%
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Low Income Persons in Highest Quintile of Day-Night Avg LST", align = "r",
...
LSTDayMinority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "Minorities in Highest Quintile of Daytime Avg LST by Municipality", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day LST (°F)" = 3))
```

```{r mapLSTDayHighRiskRILI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and Environmental Justice communities of Low Income persons, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTDayLowIncRI5 <- ri_blkgrpLST_sf %>%
  filter(RI_INCOME == "I" & pctileDayLST >= 0.8)

bbox_new <- st_bbox(LSTDayLowIncRI5) # current bounding box
...
m <- tm_shape(LSTDayLowIncRI5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ri_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ri_LSTDayLowIncRI5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LSTDayLowIncRI5.png")
```
\pagebreak
```{r tabLSTDayHighRiskRILI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayLowIncRI5 %>%
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "RI Low Income Persons in Highest Quintile of Daytime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day LST (°F)" = 3)) %>% 
...
LSTNightEngLang5 <- ri_blkgrpLST_sf %>%
  filter(LSTNightEngLangScore == 5)
...
tmap_save(m, "DATA/LST/ri_LSTNightEngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ri_LSTNightEngLang5.png")
```
...
LSTNightMinority5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "Minorities in Highest Quintile of Nighttime Avg LST by Municipality", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3))
```

```{r mapLSTNightHighRiskRILI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and Environmental Justice communities of Low Income persons, as defined by Rhode Island Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
...
```
\pagebreak
```{r tabLSTNightHighRiskRILI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of nighttime average Land Surface Temperature."}
LSTNightLowIncRI5 %>%
  st_centroid(.) %>% 
  st_intersection(ri_towns_sf,.) %>% 
  as.data.frame() %>% 
...
```{r cormatrixLSTpopsRI, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and the proportions of populations of concern by Census Block Group."}
