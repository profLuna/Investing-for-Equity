...
ct_blkgrp_sf %>% 
  ...
  geom_text(aes(x = Group, y = Pct + 0.2 * sign(Pct), 
            ...
            hjust = 0.001, vjust = -.8, size = 3,
            color=rgb(100,100,100, maxColorValue=255)) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = .6, label = "Above state\naverage"),
            color = "gray48") +
...
There is a moderate to strong positive correlation between the proportions of limited English speaking households and minorities and ambient concentrations of Diesel Particulate Matter (see Figure \@ref(fig:cormatrixDPMPopAvg) in Appendix B).
...
In addition to variations in the general geography of lifetime cancer risk from inhalation of air toxics, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgCT) below shows population-weighted exposures for populations of concern relative to average cancer risk for the state. For example, limited English speaking households in Connecticut experience lifetime cancer risks from inhalation of air toxics that are 4.6% above the region as a whole. Similarly, minorities experience lifetime cancer risks of 4% above the state average. By contrast, persons over age 64 are, on average, exposed to lifetime cancer risks 1% below the state average. 
...
There is a moderately positive correlation between the proportions of minorities and limited English households and cancer risk (see Figure \@ref(fig:cormatrixATCPopAvg) in Appendix B).
...
In addition to variations in the general geography of the respiratory hazard index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgCT) below shows population-weighted exposures for populations of concern relative to average respiratory hazard index for the state. For example, limited English speaking households in Connecticut experience respiratory hazard indices that are 6.1% above the region as a whole. Similarly, minorities are experience hazard index values more than 5% above the regional average. By contrast, persons over age 64 are, on average, exposed to respiratory hazard indices over 1% below the regional average. 
...
There is a moderate positive correlation between the proportions of minorities, low income persons, limited English households, adults with less than a high school education, and traffic proximity and volume (see Figure \@ref(fig:cormatrixTPVPopAvg) in Appendix B).
