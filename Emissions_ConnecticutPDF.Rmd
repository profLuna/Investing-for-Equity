...
These PM~2.5~ levels vary significantly across the state, although the clusters of Block Groups with high PM~2.5~ values remains apparent. Figure \@ref(fig:boxplotPM25CT) is a boxplot of PM~2.5~ concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median PM~2.5~ concentration for all Block Groups, which in this case is `r round(median(ct_blkgrp_sf$PM25_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. Note that a large cluster of dots is concentrated on the far right, at the upper end of concentration values. Most of these are in southwest Connecticut. 
...
```{r slopegraphPM25CT, fig.align = "center", fig.cap="Change in population-weighted exposure to PM2.5 between 2011 and 2016 for Connecticut and New England."}
...
In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgCT) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, language isolated households in Connecticut are exposed to PM~2.5~ concentrations that are 2.8% above concentrations for the state as a whole. Similarly, minorities are exposed to concentrations over 2% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of PM~2.5~ at or below the state average. 

```{r barplotPM25PopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for populations of concern in Connecticut relative to the state average."}
...
These Ozone (O~3~) levels vary significantly across the state. Figure \@ref(fig:boxplotO3CT) is a boxplot of O~3~ concentrations by Block Group . The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median O~3~ concentration for all Block Groups, which in this case is `r round(median(ct_blkgrp_sf$OZONE_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group.   
...
```{r barplotO3PopAvgCT, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in Connecticut relative to the regional average."}
...
```{r slopegraphO3PopAvgCT, fig.align = "center", fig.cap="Change in population-weighted exposure to summer ozone for populations of concern between 2011 and 2016."}
...
On-road CO~2~ emissions closely follow major roadways arcoss the region (see Figure \@ref(fig:mapCO2CT) below). 
...
Significant hot spots, or clusters of high CO~2~ emissions, appear around Hartford, Danbury and Newtown, and along I-95 between New Haven and Fenwick (see Figure \@ref(fig:hotspotCO2CT) below).
...
tmap_save(m8, "DATA/EJSCREEN/ct_O3hotspot.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ct_O3hotspot.png")
```

CO~2~ emissions vary across the state with significant outliers (see Table \@ref(tab:statsCO2CT).   
...
ct_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total Connecticut ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```
...
Diesel Particulate Matter (DPM) emissions are concentrated along the highway traffic corridor from Stamford to Hartford along I-95 to I-91 (see Figure \@ref(fig:mapDPMCT) below). 
...
The Block Group with highest Diesel Particulate Matter emissions value in the state is found in `r ct_town_maxDPM` and the lowest is found in `r ct_town_minDPM`.
...
```{r boxplotDPMCT, fig.align = "center", fig.cap="Boxplot of 2017 Carbon Dioxoide on-road emissions in metric tons per square kilometer by state at Census Block Group level. 1 dot = 1 Block Group."}
...
The Block Group with highest cancer risk value in the state is found in `r ct_town_maxCancer` and the lowest is found in `r ct_town_minCaner`.

Figure \@ref(fig:boxplotATCNE) is a boxplot of cancer risk by Block Group . The box represents risk values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median lifetime cancer risk value for all Block Groups, which in this case is `r round(median(ct_blkgrp_sf$CANCER_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. The large dots represent outliers, or unusually high values.
...
Higher Respiratory Hazard Indices are concentrated along the I-91 to I-95 highway traffic corridor from Stamford to Hartford (see Figure \@ref(fig:mapRHICT) below). 

```{r mapRHICT, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Connecticut at Census Block Group level."}
...
There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics along the I-91 to I-95 highway traffic corridor from Stamford to Hartford (see Figure \@ref(fig:hotspotRHICT) below).
...
The Block Group with highest respiratory hazard index value in the state is found in `r ct_town_maxRESP` and the lowest is found in `r ct_town_minRESP`.

Figure \@ref(fig:boxplotRHICT) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(ct_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group.   

```{r boxplotRHICT, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Connecticut by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
ct_blkgrp_sf %>% 
  as.data.frame() %>% 
...
```{r mapTPVCT, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Connecticut at Census Block Group level."}
...
The Block Group with highest Traffic Proximity and Volume value in the state is found in `r ct_town_maxTraffic` and the lowest is found in `r ct_town_minTraffic`.

Figure \@ref(fig:boxplotTPVCT) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(ct_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPVCT, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Connecticut at Census Block Group level."}
