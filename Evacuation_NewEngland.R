# Evacuation risks analysis. Identify the number of people within areas that are at risk of flooding and thus potentially need evacuation in the event of an emergency. 
...
ri_HevacPops_df %>% 
  ...
  geom_text(aes(x = "Disabled", y = 11.6, label = "Below state avg"),
            color = "gray48") +
  geom_segment(aes(x = "No Car HH", xend = "No Car HH", y = 12.8, yend = 10.8), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  geom_text(aes(x = "Low Income", y = 14.5, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Under 18", xend = "Under 18", y = 13.3, yend = 15.3), 
               arrow = arrow(length = unit(0.3,"cm")))
...
# Intersect block groups with flood zones and allocate populations by proportion of area intersected
# ri_blkgrps_fhza_sf <- ri_blkgrps_nlcd %>%
#   mutate(OldArea = st_area(.)) %>%
#   st_intersection(RI_FHZA_2840,.) %>%
#   mutate(NewArea = st_area(.),
#          Proportion = NewArea/OldArea,
#          NewPop = as.integer(totlppE*Proportion),
#          NewMinority = as.integer(minrtyE*Proportion),
#          NewUnder5 = as.integer(under5E*Proportion),
#          NewOver64 = as.integer(over64E*Proportion),
#          NewUnder18 = as.integer(undr18E*Proportion),
#          NewEng_limit = as.integer(eng_lmE*Proportion),
#          NewPov = as.integer(num2pvE*Proportion),
#          NewLths = as.integer(lthsE*Proportion),
#          NewRI_LOWINC = as.integer(RI_LOWI*Proportion),
#          NewRI_MINORITIES = as.integer(RI_MINO*Proportion))
# 
# # Repeat for tracts
# ri_flood_tracts_sf <- ri_tracts_2840 %>% 
#   select(GEOID,
#          disabledOver18E,
#          HHnoCarE) %>% 
#   mutate(OldArea = st_area(.)) %>% 
...
# 
# # Compute total block group populations within flood zones
# ri_flood_blkgrp_df <- ri_flood_sf %>% 
#   as.data.frame() %>% 
...
# 
# # Compute total tract populations within flood zones
# ri_flood_tracts_df <- ri_flood_tracts_sf %>% 
#   as.data.frame() %>% 
#   summarize(`Disabled` = sum(NewDisabled),
#             `No Car HH` = sum(NewNoCar)) %>% 
#   gather(key = Group, value = FloodPop)
# # Compute total tract populations within the state for same groups
# ri_tract_pops_df <- ri_tracts_2840 %>% 
#   as.data.frame() %>% 
#   summarize(`Disabled` = sum(disabledOver18E),
#             `No Car HH` = sum(HHnoCarE)) %>% 
#   gather(key = Group, value = RIPop) %>% 
#   left_join(.,ri_flood_tracts_df, by = "Group")
# 
# # Compute populations for state, join with flood pops, and create plot
# ri_FloodPops_df <- ri_blkgrp_2840 %>% 
#   as.data.frame() %>% 
#   mutate(RI_LOWINC = if_else(RI_INCOME == "I",totalpopE,0)) %>% 
#   mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>% 
...
ri_FloodPops_df %>% 
  ...
  theme(panel.grid.major.y = element_blank(),
        ...
        axis.ticks.y = element_blank()) +
  geom_text(aes(x = Group, y = PctFlood + 0.2 * sign(PctFlood), 
                label = paste0(round(PctFlood,0),"%")), 
            hjust = 0.1, vjust = -0.5, size = 3,
            color=rgb(100,100,100, maxColorValue=255)) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) 

# Map Populations within flood zones
ri_flood_sf <- ri_flood_sf %>% 
  mutate(PopPerKm2 = NewPop/(as.numeric(NewArea)/10^6))

ri_flood_sf %>% 
  tm_shape(.) + tm_borders(col = "skyblue", lwd = 0.1, alpha = 0.3) +
  tm_shape(ri_flood_sf) + tm_fill("PopPerKm2", style = "quantile") +
  tm_shape(ne_states_sf_cb) + tm_borders() +
  tm_shape(ri_highways) + tm_lines(col = "yellow", lwd = 2) +
  tm_shape(I95roadSegment) + 
  tm_symbols(shape = I95, border.lwd = NA, size = 0.25) +
  tm_shape(ri_towns_sf_pts) + tm_dots() +
  tm_text("NAME", size = 0.5, col = "black", 
...
# Map Disabled Populations within flood zones
ri_flood_tracts_sf <- ri_flood_tracts_sf %>% 
  mutate(DisabledPerKm2 = NewDisabled/(as.numeric(NewArea)/10^6),
         NoCarPerKm2 = NewNoCar/(as.numeric(NewArea)/10^6))
# Create map with blue ocean background color
tm_layout(bg.color = "skyblue", saturation = .8) +
  tm_shape(ri_blkgrp_2840) + tm_fill(col = "white") +
  tm_shape(ne_states_sf_cb) + tm_fill(col="white") +
  tm_shape(ri_awater_sf) + tm_fill(col = "skyblue", alpha = 0.3) + 
  tm_shape(ri_lwater_sf) + tm_lines(col = "skyblue", alpha = 0.3) +
  tm_shape(ri_flood_tracts_sf) + 
  tm_borders(col = "skyblue", lwd = 0.1, alpha = 0.3) +
  tm_shape(ri_flood_tracts_sf) + 
  tm_fill("DisabledPerKm2", style = "quantile") + 
  tm_shape(ne_states_sf_cb) + tm_borders() +
  tm_shape(ri_highways) + tm_lines(col = "seashell4", lwd = 2) +
  tm_shape(I95roadSegment) + 
  tm_symbols(shape = I95, border.lwd = NA, size = 0.25) +
...
# # Extract bbox of RI for WFS request from FEMA
# ne_states_sf_cb %>% 
#   filter(NAME == "Rhode Island") %>% 
#   st_bbox()
# 
# # Extract smaller bbox
# ne_towns_sf %>% 
#   filter(STATE == "Rhode Island" & 
#            NAME == "Providence city, Providence County, Rhode Island") %>% 
#   st_bbox()
# 
# # Download WFS of flood hazard regions for RI
# RI_fhaz <- st_read("https://hazards.fema.gov/gis/nfhl/services/public/NFHL/MapServer/WFSServer?SERVICE=WFS&VERSION=1.1.0&REQUEST=GetFeature&TYPENAME=public_NFHL:Flood_Hazard_Zones&BBOX=41.77238,-71.36969,41.86182,-71.47267,urn:ogc:def:crs:EPSG:6.9:4269&MAXFEATURES=1000")
# 
# RI_fhaz <- st_read("https://hazards.fema.gov/gis/nfhl/services/public/NFHL/MapServer/WFSServer?SERVICE=WFS&REQUEST=GetFeature&TYPENAME=public_NFHL:Flood_Hazard_Zones&BBOX=41,-121.5,41.125,-121.375,urn:ogc:def:crs:EPSG::4269&MAXFEATURES=1000")
# 
# RI_fhaz <- st_read("http://ovc.catastro.meh.es/INSPIRE/wfsCP.aspx?service=WFS&request=GetFeature&Typenames=cp.cadastralparcel&SRSname=EPSG::25830&bbox=233673,4015968,233761,4016008&version=2.0.0&")
