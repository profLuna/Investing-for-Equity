...
me_FloodPops_df %>% 
  ...
  geom_text(aes(x = "No HS Dip", y = 2.9, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = 2.2, label = "Below state\naverage"),
            color = "gray48")
...
```{r tabHEVACMIN, fig.align = "center", fig.cap="Minority persons living within Hurricane Inundation Zones."}
me_blkgrps_hevac %>% 
  st_centroid(.) %>% 
  st_intersection(me_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "Minority Persons Living in Hurricane Inundation Zones", align = "r", col.names = c("City/Town", "People of Color", "Pct of POC")) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) 
```
