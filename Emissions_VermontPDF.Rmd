...
In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvg) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, adults with less than a high school diploma are exposed to PM~2.5~ concentrations that are 0.1% above concentrations for the state as a whole. All other groups are at or below the state average. 

```{r barplotPM25PopAvg, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for populations of concern in Vermont relative to the state average."}
...
vt_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "No HS Dip", y = 1, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -1, label = "Below state\naverage"),
            color = "gray48") +
...
On-road CO~2~ emissions closely follow major roadways across the state (see Figure \@ref(fig:mapCO2) below). 

```{r mapCO2, fig.align = "center", fig.cap="Map of 2017 Carbon Dioxide (CO2) annual on-road emissions in metric tons per square kilometer across Vermont at Census Block Group level."}
...
Significant hot spots, or clusters of high CO~2~ emissions, appear in Burlington (see Figure \@ref(fig:hotspotCO2) below).
...
# identify towns with 200% or more change
vt_town_maxCO2change <- vt_blkgrp_sf %>% 
  mutate(CO2_pctChange90_17 = (kgco2_2017 - kgco2_1990)/kgco2_1990*100,
         pctile = percent_rank(CO2_pctChange90_17)) %>%
  filter(CO2_pctChange90_17 >= 200) %>%
  st_centroid() %>% 
  st_intersection(vt_towns_sf,.) %>% 
  # select(NAME,CO2_pctChange90_17,pctile)
  # filter(row_number() == 1) %>%
  as.data.frame() %>% 
  distinct(NAME) %>% 
  pull(., NAME)
...
Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(vt_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(vt_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(vt_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2change) below). Declines have occurred in central and northern sections of the state. Increased emissions have occurred around in the south and central western areas of the state. Increases over 200% have occurred in `r paste(vt_town_maxCO2change, collapse = " and ")`.

```{r mapCO2change, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across Vermont between 1990 and 2017 at Census Block Group level."}
...
download.file("https://www2.census.gov/census_1990/CD90_1A_1/stf1a0vt.dbf",
              destfile = "DATA/stf1a0vt.dbf", method = "curl")
# reformat df to work in code below
ne_states_df90 <- read.dbf("DATA/stf1a0vt.dbf", as.is = TRUE) %>% 
...
```{r linegraphCO2, fig.align = "center", fig.cap="Total CO2 emissions 1990 to 2017 for Vermont."}
...
DPM emissions are concentrated along I-91 and around larger municipalities, such as Brattleboro, Rutland, Montpelier, and Burlington (see Figure \@ref(fig:mapDPM) below). 

```{r mapDPM, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Vermont at Census Block Group level."}
...
There are significant spatial clusters of high DPM concentrations in Burlington (see Figure \@ref(fig:hotspotDPM) below).
...
DPM concentrations vary across the state (see Table \@ref(tab:statsDPMRI) and Figure \@ref(fig:boxplotDPM) below).  

```{r statsDPM, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
Outlier <- quantile(vt_blkgrp_sf$DSLPM_19,.75, na.rm = TRUE) + 1.5*(quantile(vt_blkgrp_sf$DSLPM_19,.75, na.rm = TRUE) - quantile(vt_blkgrp_sf$DSLPM_19,.25, na.rm = TRUE))

# identify outliers
vt_town_outlier <- vt_blkgrp_sf %>% 
...
Figure \@ref(fig:boxplotDPM) is a boxplot of DPM concentrations by Block Group. The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median DPM concentration for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$DSLPM_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Larger black dots on right represent outliers, or Block Groups with unusually high values relative to the state. In this case, outliers would be represented by DPM values greater than `r Outlier`, which occur in `r glue::glue_collapse(vt_town_outlier,", ", last = ", and ")`. 
...
In addition to variations in the general geography of DPM concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvg) below shows population-weighted exposures for populations of concern relative to average DPM concentraions for the state. For example, limited English households in Vermont are exposed to  DPM concentrations that are more than 27% above concentrations for the state as a whole. Similarly, minorities are exposed to concentrations more than 22% above the state average. By contrast, persons over age 64 and under 5 are, on average, exposed to concentrations of DPM below the state average. 

```{r barplotDPMPopAvg, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of Diesel Particulate Matter across Vermont relative to the state average."}
...
vt_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 7, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -7, label = "Below state\naverage"),
            color = "gray48") +
...
There is a moderate positive correlation between the proportions of limited English speaking households and minorities and ambient concentrations of DPM (see Figure \@ref(fig:cormatrixDPMPopAvg) in Appendix B).
...
Cancer risks are concentrated in the northwest, in and around Burlington and also along the southern stretch of I-91 (see Figure \@ref(fig:mapATCNE) below).
...
There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics centered on Burlington (see Figure \@ref(fig:hotspotATC) below).
...
vt_town_minCancer <- vt_blkgrp_sf %>% 
...
# identify threshold for outliers using IQR method
Outlier <- quantile(vt_blkgrp_sf$CANCER_19,.75, na.rm = TRUE) + 1.5*(quantile(vt_blkgrp_sf$CANCER_19,.75, na.rm = TRUE) - quantile(vt_blkgrp_sf$CANCER_19,.25, na.rm = TRUE))
...
In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvg) below shows population-weighted exposures for populations of concern relative to average cancer risk for the state. For example, limited English speaking households in Vermont experience lifetime cancer risks from inhalation of air toxics that are more than 10% above the state as a whole. Similarly, minorities cancer risks more than 7% above the state average. In fact, all populations of concern experience risks above the state average. 

```{r barplotATCPopAvg, fig.align = "center", fig.cap="Population-weighted average lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Vermont at Census Block Group level relative to the state average."}
# Pop Weighted avg of DPM for all Groups in Vermont relative to NE average
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
  ...
  geom_text(aes(x = "Low Income", y = 4, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -2.5, label = "Below state\naverage"),
            color = "gray48") +
...
Higher respiratory hazard indices are concentrated along I-91 and US 7 from Rutland to the northern border, with the highest concentrations around Burlington and Montpelier (see Figure \@ref(fig:mapRHI) below). 

```{r mapRHI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level."}
...
There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics in Burlington (see Figure \@ref(fig:hotspotRHI) below).
...
Respiratory hazard indices from inhalation of air toxics vary across the state (see Table \@ref(tab:statsRHIRI) and Figure \@ref(fig:boxplotRHI) below).   

```{r statsRHI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
# identify threshold for outliers using IQR method
Outlier <- quantile(vt_blkgrp_sf$RESP_19,.75, na.rm = TRUE) + 1.5*(quantile(vt_blkgrp_sf$RESP_19,.75, na.rm = TRUE) - quantile(vt_blkgrp_sf$RESP_19,.25, na.rm = TRUE))
...
Figure \@ref(fig:boxplotRHI) is a boxplot of respiratory hazard indices by Block Group. The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots on the right represent outliers, or Block Groups with unusually high values relative to the state. In this case, outliers would be represented by ozone values greater than `r Outlier`, which occur in `r glue::glue_collapse(vt_town_outlier,", ", last = ", and ")`.  

```{r boxplotRHI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
In addition to variations in the general geography of the respitory hazard index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvg) below shows population-weighted exposures for populations of concern relative to average respiratory hazard index for the state. For example, limited English speaking households in Vermont experience respiratory hazard indices that are over 13% above the state as a whole. Similarly, minorities experience respiratory hazard indices more than 10% above the state average. In fact, all populations of concern are exposed to respiratory hazard indices above the state average. 

```{r barplotRHIPopAvg, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level relative to the state average."}
...
vt_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 4, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -2.5, label = "Below state\naverage"),
            color = "gray48") +
...
```{r mapTPV, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}
...
There appear to be significant spatial clusters of traffic proximity and volume in Burlington (see Figure \@ref(fig:hotspotTPV) below).
...
```{r statsTPV, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
Figure \@ref(fig:boxplotTPV) is a boxplot of traffic proximity and volume values by Block Group. The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPV, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}
...
vt_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "No HS Dip", y = 25, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -15, label = "Below state\naverage"),
            color = "gray48") +
