---
title: "Transportation Emissions in Vermont"
...
PM~2.5~ levels vary significantly across Vermont, with highest concentrations in southwest Vermont, with concentration declining from Stamford to Hartford (see Figure \@ref(fig:mapPM25RI) below). 
...
```{r mapPM25RI, fig.align = "center", fig.cap="Map of 2016 annual PM2.5 concentrations across Vermont at Census Block Group level."}
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {round(mean(vt_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {round(min(vt_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%, have been in the northern half of the state. 

```{r mapPM25RIchange, fig.align = "center", fig.cap="Map of percent change in annual PM2.5 concentrations across Vermont between 2011 and 2016 at Census Block Group level."}
...
Figure \@ref(fig:slopegraphPM25RI) below compares the average annual PM~2.5~ concentrations for the state and the region between 2011 and 2016. The region and the state both showed significant declines since 2011. Concentrations in Vermont remain below the rest of the region. 

```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted exposure to PM2.5 between 2011 and 2016 for Vermont and New England."}
...
### PM~2.5~ in Vermont and Populations of Concern

In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, adults with less than a high school diploma are exposed to PM~2.5~ concentrations that are 0.1% above concentrations for the state as a whole. All other groups are at or below the state average. 

```{r barplotPM25PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for populations of concern in Vermont relative to the state average."}
...
vt_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "No HS Dip", y = 1, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Limited English HH", xend = "Limited English HH", y = 0.1, yend = 1.1), 
...
Ozone (O~3~) levels vary significantly across Vermont, with highest concentrations in southwest Vermont, and declining toward the north and east (see Figure \@ref(fig:mapO3RI) below).[^NONATTAINMENT] 

```{r mapO3RI, fig.align = "center", fig.cap="Map of 2016 Ozone summer seasonal average of daily maximum 8-hour concentration in air in parts per billion across Vermont at Census Block Group level."}
...
Since 2011, Ozone (O~3~) levels have decreased slightly across the state, on average by `r {round(mean(vt_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)}`%. These have changes not been uniform (see Figure \@ref(fig:mapO3RIchange) below). The greatest declines, up to `r round(min(vt_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`%, have been in the northeast of the state. By contrast, increases in summer ozone concentrations of up to `r round(max(vt_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`% appear along the western border with New York.

```{r mapO3RIchange, fig.align = "center", fig.cap="Map of percent change in summer seasonal Ozone concentrations across Vermont between 2011 and 2016 at Census Block Group level."}
...
Figure \@ref(fig:slopegraphO3RI) below compares the average summer Ozone (O~3~) concentrations for the state and for the region between 2011 and 2016. Vermont's average concentrations have decreased and remain lower than the region. 

```{r slopegraphO3RI, fig.align = "center", fig.cap="Change in summer average ozone concentrations between 2011 and 2016 for Vermont and New England."}
...
### Ozone (O~3~) in Vermont and Populations of Concern

In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the region. For example, minorities and limited English households in Vermont are exposed to summer Ozone concentrations that are approximately 0.5% above concentrations for the region as a whole. By contrast, all other popuations of concern are, on average, exposed to concentrations of Ozone at or below the regional average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in Vermont relative to the regional average."}
...
vt_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Over 64", y = .3, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Low Income", xend = "Low Income", y = 0.1, yend = 0.4), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-0.5,1))
```

Like the region as a whole, these populations have also experienced changes in exposure since 2011. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvgRI). Unlike all other populations, minorities and limited English households have experienced a slight increase in population-weighted exposure to Ozone. 
...
\pagebreak
## Carbon Dioxide (CO~2~) in Vermont
...
```{r mapCO2RI, fig.align = "center", fig.cap="Map of 2017 Carbon Dioxide (CO2) annual on-road emissions in metric tons per square kilometer across Vermont at Census Block Group level."}

# Map of CO2 across Vermont
m7 <- vt_blkgrp_sf %>% 
...
tmap_save(m7, "DATA/EJSCREEN/vt_CO2.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_CO2.png")
```

Significant hot spots, or clusters of high CO~2~ emissions, appear in Burlington (see Figure \@ref(fig:hotspotCO2RI) below).
...
tmap_save(m8, "DATA/EJSCREEN/vt_CO2hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_CO2hotspot.png")
```
...
The Block Group with highest CO~2~ emissions value in the state is found in `r vt_town_maxCO2` and the lowest is found in `r vt_town_minCO2`.

Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(vt_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(vt_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(vt_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2RIchange) below). Declines have occurred in central and northern sections of the state. Increased emissions have occurred around in the south and central western areas of the state.

```{r mapCO2RIchange, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across Vermont between 1990 and 2017 at Census Block Group level."}
# Map of CO2 change across Vermont
m9 <- vt_blkgrp_sf %>% 
...
tmap_save(m9, "DATA/EJSCREEN/vt_CO2change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_CO2change.png")
```
...
vt_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total Vermont ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```

Vermont saw dramatic growth in CO~2~ emissions between 1990 and 1996 and again between 2001 and 2005. Since 2005, however, total emissions have declined significantly. This is reflected in the decline in per capita emissions since 1990, indicating that emissions growth has lagged population growth (see last three columns in Table \@ref(tab:changeCO2RI)).  


\pagebreak
## Diesel Particulate Matter in Vermont
...
Diesel Particulate Matter (DPM) emissions are concentrated along I-91 and around larger municipalities, such as Brattleboro, Rutland, Montpelier, and Burlington (see Figure \@ref(fig:mapDPMRI) below). 

```{r mapDPMRI, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Vermont at Census Block Group level."}

# Map of DPM across Vermont
m10 <- vt_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m10,"DATA/EJSCREEN/vt_DPM.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_DPM.png")
```

There are significant spatial clusters of high Diesel Particulate Matter concentrations in Burlington (see Figure \@ref(fig:hotspotDPMRI) below).
...
tmap_save(m11, "DATA/EJSCREEN/vt_DPMhotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_DPMhotspot.png")
```
...
```{r statsDPMRI, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
The Block Group with highest Diesel Particulate Matter emissions value in the state is found in `r vt_town_maxDPM` and the lowest is found in `r vt_town_minDPM`.
...
### Diesel Particulate Matter in Vermont and Populations of Concern

In addition to variations in the general geography of Diesel Particulate Matter concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Diesel Particulate Matter concentraions for the state. For example, limited English households in Vermont are exposed to  Diesel Particulate Matter concentrations that are more than 27% above concentrations for the state as a whole. Similarly, minorities are exposed to concentrations more than 22% above the state average. By contrast, persons over age 64 and under 5 are, on average, exposed to concentrations of Diesel Particulate Matter below the state average. 

```{r barplotDPMPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of Diesel Particulate Matter across Vermont relative to the regional average."}
...
vt_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 7, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Under 5", xend = "Under 5", y = 1, yend = 10), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-5,30))
```

There is a moderate positive correlation between the proportions of language isolated households and minorities and ambient concentrations of Diesel Particulate Matter (see Appendix).


\pagebreak
## Air Toxics Cancer Risk in Vermont
...
Air Toxics Cancer Risk are concentrated in the northwest, in and around Burliington and also along the southern stretch of I-91 (see Figure \@ref(fig:mapATCNE) below).

```{r mapATCNE, fig.align = "center", fig.cap="Map of 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Vermont at Census Block Group level."}
...
tmap_save(m12, "DATA/EJSCREEN/vt_Cancer.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_Cancer.png")
```

There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics centered on Burlington (see Figure \@ref(fig:hotspotATCRI) below).
...
tmap_save(m13, "DATA/EJSCREEN/vt_CancerHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_CancerHotspot.png")
```
\pagebreak
...
```{r statsATCNE, fig.align = "center", fig.cap="Annual 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
...
# extract block groups centroid with highest and lowest CO2 values, intersect with town polygons, and pull out town name
vt_town_maxCancer <- vt_blkgrp_sf %>% 
...
The Block Group with highest cancer risk value in the state is found in `r vt_town_maxCancer` and the lowest is found in `r vt_town_minCaner`.

Figure \@ref(fig:boxplotATCNE) is a boxplot of cancer risk by Block Group . The box represents risk values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median lifetime cancer risk value for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$CANCER_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots on the right represent outliers, or unusually high values. 

```{r boxplotATCNE, fig.align = "center", fig.cap="Boxplot of lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Vermont by state at Census Block Group level. 1 dot = 1 Block Group."}
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in Vermont and Populations of Concern

In addition to variations in the general geography of Lifetime Cancer Risk from Inhalation of Air Toxics, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Lifetime Cancer Risk from Inhalation of Air Toxics for the state. For example, limited English households in Vermont experience lifetime cancer risks from inhalation of air toxics that are more than 10% above the state as a whole. Similarly, minorities experience lifetime cancer risks more than 7% above the state average. In fact, all populations of concern experience risks above the state average. 

```{r barplotATCPopAvgRI, fig.align = "center", fig.cap="Population-weighted average lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across Vermont at Census Block Group level relative to the state average."}
# Pop Weighted avg of DPM for all Groups in Vermont relative to NE average
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 4, label = "Above state avg"),
            color = "gray48") +
...
\pagebreak
## Respiratory Hazard Index in Vermont
...
Higher Respiratory Hazard Indices are concentrated along I-91 and US 7 from Rutland to the northern border, with the highest concentrations around Burlington (see Figure \@ref(fig:mapRHIRI) below). 

```{r mapRHIRI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level."}

# Map of RHI across Vermont
m14 <- vt_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m14, "DATA/EJSCREEN/vt_RespHzrd.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_RespHzrd.png")
```

There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics in Burlington (see Figure \@ref(fig:hotspotRHIRI) below).
...
tmap_save(m15, "DATA/EJSCREEN/vt_RespHzrdHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_RespHzrdHotspot.png")
```
...
```{r statsRHIRI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
The Block Group with highest respiratory hazard index value in the state is found in `r vt_town_maxRESP` and the lowest is found in `r vt_town_minRESP`.

Figure \@ref(fig:boxplotRHIRI) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots on the right represent outliers, or Block Groups with unusually high values relative to the state.   

```{r boxplotRHIRI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
### Respiratory Hazard Index from Inhalation of Air Toxics in Vermont and Populations of Concern

In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Respiratory Hazard Index from Inhalation of Air Toxics for the state. For example, limited English households in Vermont experience respiratory hazard indices from inhalation of air toxics that are over 13% above the state as a whole. Similarly, minorities experience lifetime hazard index values more than 10% above the state average. In fact, all populations of concern are exposed to respiratory hazard indices above the state average. 

```{r barplotRHIPopAvgRI, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across Vermont at Census Block Group level relative to the regional average."}
# Pop Weighted avg of RHI for all Groups in Vermont relative to NE average
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 4, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Over 64", xend = "Over 64", y = 1, yend = 5), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-1,15))
```

There is a moderate to strong positive correlation between the proportions of minorities and respiratory hazard index (see Appendix).


\pagebreak
## Traffic Proximity and Volume in Vermont
...
```{r mapTPVRI, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}

# Map of TPV across Vermont
m16 <- vt_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m16, "DATA/EJSCREEN/vt_Traffic.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_Traffic.png")
```

There appear to be significant spatial clusters of traffic proximity and volume in Burlington (see Figure \@ref(fig:hotspotTPVRI) below).
...
tmap_save(m17, "DATA/EJSCREEN/vt_TrafficHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/vt_TrafficHotspot.png")
```
...
```{r statsTPVRI, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}
vt_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
# extract block groups centroid with highest and lowest values, intersect with town polygons, and pull out town name
vt_town_maxTraffic <- vt_blkgrp_sf %>% 
...
vt_town_minTraffic <- vt_blkgrp_sf %>% 
...
The Block Group with highest Traffic Proximity and Volume value in the state is found in `r vt_town_maxTraffic` and the lowest is found in `r vt_town_minTraffic`.

Figure \@ref(fig:boxplotTPVRI) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(vt_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPVRI, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level."}
...
### Traffic Proximity and Volume Exposure in Vermont and Populations of Concern

In addition to variations in the general geography of the Traffic Proximity and Volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for populations of concern relative to average Traffic Proximity and Volume for the state. For example, minorities are exposed to Traffic Proximity and Volume more than 79% above the state average. By contrast, persons over age 64 are, on average, are exposed to Traffic Proximity and Volume of more than 13% below the state average. 

```{r barplotTPVPopAvgNE, fig.align = "center", fig.cap="Population-weighted average Traffic Proximity and Volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across Vermont at Census Block Group level relative to the state average."}
# Pop Weighted avg of TPV for all Groups in Vermont relative to NE average
vt_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  geom_text(aes(x = "No HS Dip", y = 25, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Under 5", xend = "Under 5", y = 5, yend = 30), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-20,80))
```

There is a moderate positive correlation between the proportions of minorities and traffic proximity and volume, and a moderate positive correlation with low income persons (see Appendix).
...
[^EPAPM10risk]: See EPA SUMMARY OF PROPOSAL TO RETAIN THE AIR QUALITY STANDARDS FOR PARTICLE POLLUTION. https://www.epa.gov/sites/production/files/2020-04/documents/favt_sheet_pm_naaqs_proposal.pdf
...
[^NONATTAINMENT]: Note that the EJSCREEN values do not directly indicate nonattainment of the NAAQS standard because the EJSCREEN data is based on estimates from a combination of modeling and monitoring for a single year, while nonattainment is determined for a large area (often a county) based on three years of monitoring data. For example, five counties in Vermont have been designated as "nonattainment" status for NAAQS ozone standards as of March 2020. For a list of nonattainment counties see EPA 8-Hour Ozone Designated Area State/Area/County Report. https://www3.epa.gov/airquality/greenbook/jbcs.html#RI 
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "Vermont Low Income threshold: 30% or more people in households where the household income is less than or equal to twice the federal poverty level",
                                     ...
                                     "Vermont Low Income threshold: highest 15% of block groups with people in households where the household income is less than or equal to twice the federal poverty level",
                                     "Vermont Minority threshold: highest 15% of block groups with persons of Hispanic or Latino origin or persons who are not White"),
...
ggcorrplot(corrPollutants, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Pollutant Correlation Matrix for Vermont", 
...
ggcorrplot(corrPM25, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "PM2.5 Correlation Matrix for Vermont", 
           legend.title = "Spearman's \nCorrelation\nCoefficient")
```
...
ggcorrplot(corrO3, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Ozone Correlation Matrix for Vermont", 
           legend.title = "Spearman's \nCorrelation\nCoefficient")
```
...
ggcorrplot(corrDPM, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "DPM Correlation Matrix for Vermont", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
...
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Cancer Risk Correlation Matrix for Vermont", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           ggtheme = ggplot2::theme_minimal())
```
...
# corrPM25
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Respiratory Hazard Index \nCorrelation Matrix for Vermont", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           ggtheme = ggplot2::theme_minimal())
```
...
ne_towns_sf %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    caption = "Average Pollutant and Transportation Burden Values by Municipality in Vermont", align = "r") %>% 
