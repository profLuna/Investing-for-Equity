...
```{r hotspotCO2, fig.align = "center", fig.cap="Hot spot map of 2017 on-road CO2 emissions at Census Block Group level."}
# Get rid of empty geometries and NAs, and convert to spdf
empty_geo <- st_is_empty(vt_blkgrp_sf)
ne_blkgrp_sp_DemoEJ <- vt_blkgrp_sf[!empty_geo,] %>% 
  mutate(onroad_2017 = kgco2_2017/1000/(bg_area_m2/1000000)) %>%
...
```{r statsCO2, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Annual 2017 on-road CO2 emissions (mtons) at Census Block Group level and by state."}
vt_blkgrp_sf %>% 
...
```{r mapCO2change, fig.align = "center", fig.cap="Map of percent change in annual on-road CO2 emissions across Vermont between 1990 and 2017 at Census Block Group level."}
...
```{r changeCO2, fig.align = "center", fig.cap="Annual 2017 On-road CO2 emissions (mtons) and Percent Change by state at Census Block Group level."}
library(tidycensus)
...
```{r linegraphCO2, fig.align = "center", fig.cap="Total On-road CO2 emissions 1990 to 2017 for Vermont."}
...
vt_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total Vermont On-road", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```
...
### Diesel Particulate Matter in Vermont and Priority Populations

In addition to variations in the general geography of DPM concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvg) below shows population-weighted exposures for priority populations relative to average DPM concentrations for the state. For example, limited English households in Vermont are exposed to  DPM concentrations that are more than 32% above concentrations for the state as a whole. Similarly, People of Color are exposed to concentrations more than 22% above the state average. By contrast, persons over age 64 and under 5 are, on average, exposed to concentrations of DPM below the state average. 
...
# ggsave("images/VT_DPM_graph.png")
```

There is a moderate positive correlation between the proportions of limited English speaking households and People of Color and ambient concentrations of DPM (see Figure \@ref(fig:cormatrixDPMPopAvg) in Appendix B).
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in Vermont and Priority Populations

In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvg) below shows population-weighted exposures for priority populations relative to average cancer risk for the state. For example, limited English speaking households in Vermont experience lifetime cancer risks from inhalation of air toxics that are more than 11% above the state as a whole. Similarly, People of Color cancer risks more than 7% above the state average. In fact, all priority populations experience risks above the state average. 
...
# ggsave("images/VT_Cancer_graph.png")
```
...
### Respiratory Hazard Index from Inhalation of Air Toxics in Vermont and Priority Populations

In addition to variations in the general geography of the respitory hazard index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvg) below shows population-weighted exposures for priority populations relative to average respiratory hazard index for the state. For example, limited English speaking households in Vermont experience respiratory hazard indices that are over 14% above the state as a whole. Similarly, People of Color experience respiratory hazard indices more than 10% above the state average. In fact, all priority populations are exposed to respiratory hazard indices above the state average. 
...
# ggsave("images/VT_Resp_graph.png")
```
...
### Traffic Proximity and Volume Exposure in Vermont and Priority Populations

In addition to variations in the general geography of the traffic proximity and volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for priority populations relative to average traffic proximity and volume for the state. For example, People of Color are exposed to traffic proximity and volume more than 79% above the state average. By contrast, persons over age 64 are, on average, are exposed to traffic proximity and volume of more than 13% below the state average. 
...
# ggsave("images/VT_Traffic_graph.png")
```

There is a moderate positive correlation between the proportions of People of Color and traffic proximity and volume, and a moderate positive correlation with low income persons (see Figure \@ref(fig:cormatrixTPVPopAvg) in Appendix B).
...
The total or sum of the products (i.e., Minority pop x PM~2.5~ concentrations) is then divided by the sum of the weights (i.e., total Minority), so that 324.5/45 = `r round(324.5/45,2)`. The result is a weighted average PM~2.5~ concentration that is influenced by the number of People of Color. 
