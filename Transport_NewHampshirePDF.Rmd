...
# Analysis of walkability and transportation cost burden in New Hampshire

This is an analysis of walkability and transportation cost burden in New Hampshire.
...
# nh_busBuff1mile <- st_buffer(nh_stops_sf_Bus,dist = 1609.34) %>% 
#   st_union() %>% 
#   st_as_sf()
...
# nh_prBuff10mile <- nh_parknride_sf %>% 
#   st_buffer(., dist = (10*1609.34)) %>% 
#   st_union() %>% 
#   st_as_sf()
# # identify block groups that do not intersect with 10 mile buffer
# nh_blkgrps_sf_noPR <- nh_blkgrps_developed %>% 
#   filter(st_disjoint(.,nh_prBuff10mile,sparse = FALSE))
# # create simplified version of block groups for faster mapping
# nh_blkgrps_sf_noPR_simple <- nh_blkgrps_sf_noPR %>% 
...
# nh_tracts_sf_noPR <- nh_tracts_developed %>% 
#   filter(st_disjoint(.,nh_prBuff10mile,sparse = FALSE))
# # create simplified version of tracts for faster mapping
# nh_tracts_sf_noPR_simple <- nh_tracts_sf_noPR %>% 
...
# nh_transitBuffAll <- rbind(nh_busBuff1mile,
#                              nh_prBuff10mile) %>% 
#   st_union() %>% 
#   st_as_sf() %>% 
#   tmaptools::crop_shape(.,nh_state_sf_cb,polygon = TRUE)
# 
# # identify block groups that do not intersect with any transit buffer
# nh_blkgrps_sf_noTransit <- nh_blkgrps_developed %>% 
#   filter(st_disjoint(.,nh_transitBuffAll,sparse = FALSE))
# # identify tracts that do not intersect with 5 mile buffer
# nh_tracts_sf_noTransit <- nh_tracts_developed %>% 
#   filter(st_disjoint(.,nh_transitBuffAll,sparse = FALSE))
# # create simplified versions for faster mapping
# nh_blkgrps_sf_noTransit_simple <- nh_blkgrps_sf_noTransit %>% 
#   st_simplify(., dTolerance = 100) %>% 
#   filter(!st_is_empty(.)) %>% 
#   st_make_valid()
# nh_tracts_sf_noTransit_simple <- nh_tracts_sf_noTransit %>% 
#   st_simplify(., dTolerance = 100) %>% 
#   filter(!st_is_empty(.)) %>% 
#   st_make_valid()
```
