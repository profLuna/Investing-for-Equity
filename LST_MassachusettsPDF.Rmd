...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLST24hrTempPopAvgMA) and Table \@ref(tab:tabLST24hrPopAvgMA) show population-weighted exposures for populations of concern relative to the day-night average LST for the state of `r round(mean(ma_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average LST of `r round(max(LST24hrPop$LST24hrwMean),1)`°, approximately `r round(max(LST24hrPop$LST24hrwMean) - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[1,4]`) above the state day-night average LST. Similarly, minorities, as defined by the state's environmental justice policy,  were exposed to a population-weighted day-night average LST of `r LST24hrPop %>% filter(Group == "MA Minority") %>% summarize(round(max(LST24hrwMean),1)) %>% pull()`°, over `r round(LST24hrPop %>% filter(Group == "MA Minority") %>% summarize(max(LST24hrwMean)) %>% pull() - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[3,4]`) above the state day-night average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted day-night average LSTs of `r LST24hrPopTab[8,3]` below the state average.

```{r plotLST24hrTempPopAvgMA, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Massachusetts relative to the state average."}
LST24hrPop %>% 
...
```
\pagebreak
```{r tabLST24hrPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average LST for populations of concern in Massachusetts relative to the state average."}
LST24hrPopTab %>% 
  arrange(Group) %>% 
...
UHI24hrPopTab <- ma_blkgrpLST_sf %>% 
  ...
  transmute(Minority = (minorityE/UHI24hrMean - 1)*100,
         ...
         `MA Low Income` = (MA_LOWINC/UHI24hrMean - 1)*100,
         `MA Minority` = (MA_MINORITY/UHI24hrMean - 1)*100,
         `MA Limited English HH` = (MA_ENGLISH/UHI24hrMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHI24hrTempPopAvgMA) and Table \@ref(tab:tabUHI24hrPopAvgMA) show population-weighted exposures for populations of concern relative to the day-night average Urban Heat Island (UHI) effect for Massachusetts. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across Massachusetts is approximately `r round(mean(ma_blkgrpLST_sf$UHI24avg,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average UHI effect of `r round(max(UHI24hrPop$UHI24hrwMean),1)`° above the rural average. This value is more than `r round(max(UHI24hrPop$UHI24hrwMean) - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[1,4]`) above the state day-night average UHI. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted day-night average UHI effect of `r UHI24hrPop %>% filter(Group == "MA Minority") %>% summarize(round(max(UHI24hrwMean),1)) %>% pull()`°, approximately `r round(UHI24hrPop %>% filter(Group == "MA Minority") %>% summarize(max(UHI24hrwMean)) %>% pull() - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[5,4]`) above the state day-night average UHI. By contrast, persons over age 64 were, on average, were exposed to a population-weighted day-night average UHI `r UHI24hrPopTab[8,3]` (`r UHI24hrPopTab[8,4]`) below the state average.

```{r plotUHI24hrTempPopAvgMA, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Massachusetts."}
UHI24hrPop %>% 
...
```{r tabUHI24hrPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Massachusetts relative to the rural average."}
UHI24hrPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average day-night UHI effect is ", round(mean(ma_blkgrpLST_sf$UHI24avg,na.rm=TRUE),2), "°"))
```

```{r LST24hrHighRiskStats, include=FALSE}
Pop24hr <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(MA_ENGLISH == "E" & pctile24hrLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))

Pop24hrPct <- round(Pop24hr/statepopsdf$TotalPop*100,1) %>% 
  pull()

HH24hr <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(MA_ENGLISH == "E" & pctile24hrLST >= 0.8) %>% 
...
MIN24hr <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(MA_MINORITY == "M" & pctile24hrLST >= 0.8) %>% 
...
m <- tm_shape(LST24EngLangMA5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ma_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ma_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ma_LST24EngLangMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24EngLangMA5.png")
```

\pagebreak
```{r tabLST24hrHighRiskMALEH, fig.align = "center", fig.cap="Limited English Housheolds in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
LST24EngLangMA5 %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    ...
                    digits = 1,
                    caption = "MA Limited English Speaking Households in Highest Quintile of Day-Night Avg LST", align = "r",
  ...
  kableExtra::footnote(., symbol = "As defined by Massachusetts Environmental Justice Policy")
```

```{r mapLST24hrHighRiskMAMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and 80th percentile of percentage of Minorities, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
# LST24Minority5 <- ma_blkgrpLST_sf %>%
#   filter(LST24MinorScore == 5)

LST24MinorityMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_MINORITY == "M" & pctile24hrLST >= 0.8)
...
m <- tm_shape(LST24MinorityMA5, unit = "mi", bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ma_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ma_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ma_LST24MinorityMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24MinorityMA5.png")
```

\pagebreak
```{r tabLST24hrHighRiskMAMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
...
LST24MinorityMA5 %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Minorities in Highest Quintile of Day-Night Avg LST by Municipality", align = "r",
...
Daytime Average Land Surface Temperatures (LST) represent LST values collected during the day (11:48am - 2pm) over eight days, from July 28 to August 4, 2019. Daytime average LST for the state was `r round(mean(ma_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. Daytime Land Surface Temperatures (LST) varied significantly across Massachusetts, with highest temperatures apparent in the most urbanized areas of the state, particularly within Route 128/I-95, including the Boston area, and along I-91 from Springfield to Holyoke. Other areas of high temperature appear around Worcester, Lawrence, Lowell, Brockton, Fall River, and New Bedford (see Figure \@ref(fig:mapLSTdayMA) below). Across Massachusetts, approximately `r ma_blkgrpLST_sf %>% as.data.frame() %>% filter(pctileDayLST >= 0.8) %>% group_by() %>% summarize(sum(totalpopE, na.rm = TRUE)/10^6) %>% pull() %>% round(.,2)` million people live in Block Groups that exceeded the 80th percentile (temperatures over `r round(quantile(ma_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),1)`°) for daytime average temperatures.  

```{r mapLSTdayMA, fig.align = "center", fig.cap="Map of July-August 2019 average daytime Land Surface Temperatures (LST) across Massachusetts at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Massachusetts
m <- tm_shape(ma_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ma_DayLST.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_DayLST.png")
```

```{r LSTDayoutliers, include=FALSE}
# Identify municipalities with high heat outliers
ma_LSTDayoutliers <- ma_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTdayMA) is a boxplot of average daytime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ma_blkgrpLST_sf$meanDayLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ma_LSTDayoutliers`.

```{r boxplotLSTdayMA, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime average Land Surface Temperatures (LST) across Massachusetts at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(meanDayLST) %>% 
...
The UHI effect for daytime average temperatures in Massachusetts are most pronounced in the more densely urbanized areas (see Figure \@ref(fig:mapUHIdayMA)), particularly in the Boston area, along Route 128/I-95, and the cities of Brockton, Lawrence, Lowell, and Worcester. Temperatures in these areas are up to `r round(max(ma_blkgrpLST_sf$UHIDay,na.rm = TRUE),1)`° warmer than the rural background average.  

```{r mapUHIdayMA, fig.align = "center", fig.cap="Map of July-Aug 2019 daytime Urban Heat Island (UHI) effect across Massachusetts at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Massachusetts
m <- tm_shape(ma_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ma_DayUHI.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_DayUHI.png")
```

```{r LSTDayUHIoutliers, include=FALSE}
ma_LSTDayUHIoutliers <- ma_blkgrpLST_sf %>% 
  arrange(desc(UHIDay)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotUHIDayMA) is a boxplot of average daytime Urban Heat Island (UHI) effect values by Block Group. The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median value for all Block Groups, which in this case is `r round(median(ma_blkgrpLST_sf$UHIDay, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ma_LSTDayUHIoutliers`. 

```{r boxplotUHIDayMA, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime Urban Heat Island (UHI) across Massachusetts at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHIDay) %>% 
...
# Pop Weighted avg of LST day for all Groups in Massachusetts relative to MA average
LSTDayPopTab <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
  ...
  transmute(Minority = (minorityE/LSTDayMean - 1)*100,
         ...
         `MA Low Income` = (MA_LOWINC/LSTDayMean - 1)*100,
         `MA Minority` = (MA_MINORITY/LSTDayMean - 1)*100,
         `MA Limited English HH` = (MA_ENGLISH/LSTDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTDayTempPopAvgMA) and Table \@ref(tab:tabLSTDayPopAvgMA) show population-weighted exposures for populations of concern relative to the average daytime average LST for the state, which is `r round(mean(ma_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average LST of `r round(max(LSTDayPop$LSTDaywMean),1)`°, approximately `r round(max(LSTDayPop$LSTDaywMean) - max(LSTDayPop$LSTDayMean),1)`° (`r LSTDayPopTab[3,4]`) above the state daytime average LST. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average LST of approximately `r LSTDayPop %>% filter(Group == "MA Minority") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, `r round(LSTDayPop %>% filter(Group == "MA Minority") %>% summarize(max(LSTDaywMean)) %>% pull() - LSTDayPop %>% summarize(max(LSTDayMean)) %>% pull(),1)`° (`r LSTDayPopTab[5,4]`) above the state daytime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average LSTs of `r LSTDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, approximately `r LSTDayPopTab[8,3]` below the state average.

```{r plotLSTDayTempPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to average daytime Land Surface Temperatures for populations of concern in Massachusetts relative to the state average."}
LSTDayPop %>% 
...
\pagebreak
```{r tabLSTDayPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to Daytime LST for populations of concern in Massachusetts relative to the state average."}
LSTDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime LST is ", round(mean(ma_blkgrpLST_sf$meanDayLST,na.rm=TRUE),2), "°"))
```

```{r UHIDayPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Massachusetts relative to MA average
UHIDayPop <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
...
# Pop Weighted avg of UHI for all Groups in Massachusetts relative to MA average
UHIDayPopTab <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
  ...
  transmute(Minority = (minorityE/UHIDayMean - 1)*100,
         ...
         `MA Low Income` = (MA_LOWINC/UHIDayMean - 1)*100,
         `MA Minority` = (MA_MINORITY/UHIDayMean - 1)*100,
         `MA Limited English HH` = (MA_ENGLISH/UHIDayMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHIDayTempPopAvgMA) and Table \@ref(tab:tabUHIDayPopAvgMA) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ma_blkgrpLST_sf$UHIDay,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r round(max(UHIDayPop$UHIDaywMean),1)`° above the rural average. This value is more than `r round(max(UHIDayPop$UHIDaywMean) - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[3,4]`) above the state daytime average UHI. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r UHIDayPop %>% filter(Group == "MA Minority") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`°, approximately `r round(UHIDayPop %>% filter(Group == "MA Minority") %>% summarize(max(UHIDaywMean)) %>% pull() - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[5,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI `r UHIDayPopTab[8,3]`° (`r UHIDayPopTab[8,4]`) below the state average.

```{r plotUHIDayTempPopAvgMA, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Massachusetts."}
UHIDayPop %>% 
...
```{r tabUHIDayPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Massachusetts relative to the rural average."}
UHIDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime UHI effect is ", round(mean(ma_blkgrpLST_sf$UHIDay,na.rm=TRUE),2), "°"))
```

```{r LSTDayHighRiskStats, include=FALSE}
PopDay <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTDayEngLangScore == 5) %>% 
  filter(MA_ENGLISH == "E" & pctileDayLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))

PopDayPct <- round(PopDay/statepopsdf$TotalPop*100,1) %>% 
  pull()

HHDay <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTDayEngLangScore == 5) %>% 
  filter(MA_ENGLISH == "E" & pctileDayLST >= 0.8) %>% 
  summarize(sum(householdsE,na.rm = TRUE))
...
MINDay <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTDayMinorScore == 5) %>% 
  filter(MA_MINORITY == "M" & pctileDayLST >= 0.8) %>% 
  summarize(sum(minorityE,na.rm = TRUE))
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ma_LSTDayEngLangMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayEngLangMA5.png")
```

\pagebreak
```{r tabLSTDayHighRiskMALEH, fig.align = "center", fig.cap="Limited English Housheolds in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayEngLangMA5 %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Limited English Speaking Households in Highest Quintile of Daytime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Limited English Households*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
```{r mapLSTDayHighRiskMAMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and 80th percentile of percentage of Minorities, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTDayMinorityMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_MINORITY == "M" & pctileDayLST >= 0.8)

bbox_new <- st_bbox(LSTDayMinorityMA5) # current bounding box
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ma_LSTDayMinorityMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayMinorityMA5.png")
```
\pagebreak
```{r tabLSTDayHighRiskMAMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayMinorityMA5 %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Minorities in Highest Quintile of Daytime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Minorities*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Daytime LST (°F)" = 3)) %>% 
...
Nighttime Land Surface Temperatures (LST) represent LST values collected during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. Nighttime average LST for the state was `r round(mean(ma_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`°. Nighttime Land Surface Temperatures (LST) varied significantly across Massachusetts, with highest temperatures apparent in or near major cities and in the eastern half of the state (see Figure \@ref(fig:mapLSTnightMA) below). In contrast to daytime LSTs, nighttime values are also high in areas outside of the urban cores, especially the southeast, including the Cape.

```{r mapLSTnightMA, fig.align = "center", fig.cap="Map of July-August 2019 average nighttime Land Surface Temperatures (LST) across Massachusetts at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Massachusetts
m <- tm_shape(ma_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ma_NightLST.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_NightLST.png")
```

```{r LSTNightoutliers, include=FALSE}
ma_LSTNightoutliers <- ma_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTnightMA) is a boxplot of average nighttime temperatures by Block Group. The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ma_blkgrpLST_sf$meanNightLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large dots on either end represents outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ma_LSTNightoutliers`. 

```{r boxplotLSTnightMA, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Land Surface Temperatures (LST) across Massachusetts at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
The UHI effect for nighttime average temperatures in Massachusetts are most pronounced in areas immediately north and west of Boston, as well as in Springfield, Lawrence, Lowell, Buzzards Bay, especially around Fall River and New Bedford, and on the Cape (see Figure \@ref(fig:mapUHInightMA)). Temperatures in these areas are up to `r round(max(ma_blkgrpLST_sf$UHINight,na.rm = TRUE),1)`° warmer than the rural background average. By contrast, most of western Massachusetts is cooler than rural background average. 

```{r mapUHInightMA, fig.align = "center", fig.cap="Map of July-Aug 2019 nighttime Urban Heat Island (UHI) effect across Massachusetts at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Massachusetts
m <- tm_shape(ma_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/ma_NightUHI.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_NightUHI.png")
```
...
Figure \@ref(fig:boxplotUHINightMA) is a boxplot of nighttime Urban Heat Island (UHI) effects by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(ma_blkgrpLST_sf$UHINight, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large black dots represent outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r ma_LSTNightoutliers`.

```{r boxplotUHINightMA, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Urban Heat Island (UHI) across Massachusetts at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHINight) %>% 
...
# Pop Weighted avg of LST night for all Groups in Massachusetts relative to MA average
LSTNightPopTab <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
  ...
  transmute(Minority = (minorityE/LSTNightMean - 1)*100,
         ...
         `MA Low Income` = (MA_LOWINC/LSTNightMean - 1)*100,
         `MA Minority` = (MA_MINORITY/LSTNightMean - 1)*100,
         `MA Limited English HH` = (MA_ENGLISH/LSTNightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTNightTempPopAvgMA) and Table \@ref(tab:tabLSTNightPopAvgMA) show population-weighted exposures for populations of concern relative to the average nighttime average LST of `r round(mean(ma_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`° for the state. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted nighttime average LST of `r round(max(LSTNightPop$LSTNightwMean),1)`°, approximately `r round(max(LSTNightPop$LSTNightwMean) - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[3,4]`) above the state nighttime average LST. Similarly, minorities, as defined by the state's environmental justice policy, were exposed to a population-weighted nighttime average LST of `r LSTNightPop %>% filter(Group == "MA Minority") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, over `r round(LSTNightPop %>% filter(Group == "MA Minority") %>% summarize(max(LSTNightwMean)) %>% pull() - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[5,4]`) above the state nighttime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted nighttime average LSTs of `r LSTNightPopTab[8,2]`, approximately `r LSTNightPopTab[8,3]` below the state average.

```{r plotLSTNightTempPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to average nighttime Land Surfact Temperatures for populations of concern in Massachusetts relative to the state average."}
LSTNightPop %>% 
...
\pagebreak
```{r tabLSTNightPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime LST for populations of concern in Massachusetts relative to the state average."}
LSTNightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime LST is ", round(mean(ma_blkgrpLST_sf$meanNightLST,na.rm=TRUE),2), "°"))
```

```{r UHINightPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Massachusetts relative to MA average
UHINightPop <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
...
# Pop Weighted avg of PM2.5 for all Groups in Massachusetts relative to MA average
UHINightPopTab <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  mutate(MA_LOWINC = if_else(MA_INCOME == "I", totalpopE,0)) %>% 
  mutate(MA_LOWINC = replace_na(MA_LOWINC,0)) %>% 
  mutate(MA_MINORITY = if_else(MA_MINORITY == "M", totalpopE,0)) %>%
  ...
  transmute(Minority = (minorityE/UHINightMean - 1)*100,
         ...
         `MA Low Income` = (MA_LOWINC/UHINightMean - 1)*100,
         `MA Minority` = (MA_MINORITY/UHINightMean - 1)*100,
         `MA Limited English HH` = (MA_ENGLISH/UHINightMean - 1)*100) %>%
  gather(key = Group, value = Pct) %>% 
...
Figure \@ref(fig:plotUHINightTempPopAvgMA) and Table \@ref(tab:tabUHINightPopAvgMA) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(ma_blkgrpLST_sf$UHINight,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Massachusetts, as defined by the state's environmental justice policy, were exposed to a population-weighted daytime average UHI effect of `r round(max(UHINightPop$UHINightwMean),1)`° above the rural average. This value is more than `r round(max(UHINightPop$UHINightwMean) - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[3,4]`) above the state daytime average UHI. Similarly, minorities were exposed to a population-weighted daytime average UHI effect of `r UHINightPop %>% filter(Group == "MA Minority") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, approximately `r round(UHINightPop %>% filter(Group == "MA Minority") %>% summarize(max(UHINightwMean)) %>% pull() - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[5,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI of `r UHINightPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`° (`r UHINightPopTab[8,4]`) below the state average.

```{r plotUHINightTempPopAvgMA, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Massachusetts."}
UHINightPop %>% 
...
```{r tabUHINightPopAvgMA, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Massachusetts relative to the rural average."}
UHINightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime UHI effect is ", round(mean(ma_blkgrpLST_sf$UHINight,na.rm=TRUE),2), "°"))
```

```{r LSTNightHighRiskStats, include=FALSE}
PopNight <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTNightEngLangScore == 5) %>% 
  filter(MA_ENGLISH == "E" & pctileNightLST >= 0.8) %>% 
  summarize(sum(totalpopE,na.rm = TRUE))

PopNightPct <- round(PopNight/statepopsdf$TotalPop*100,1) %>% 
  pull()

HHNight <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTNightEngLangScore == 5) %>% 
  filter(MA_ENGLISH == "E" & pctileNightLST >= 0.8) %>%
  summarize(sum(householdsE,na.rm = TRUE))
...
MINNight <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(LSTNightMinorScore == 5) %>% 
  filter(MA_MINORITY == "M" & pctileNightLST >= 0.8) %>% 
  summarize(sum(minorityE,na.rm = TRUE))
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskMALEH) and \@ref(fig:mapLSTNightHighRiskMAMIN) highlight Census Block Groups across Massachusetts which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and were also designated as Environmental Justice communities for limited English speaking households or minorities.[^Nightquintile] Approximately `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r MINNight %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHNightPct`% and `r MINNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLSTNightHighRiskMALEH) and \@ref(tab:tabLSTNightHighRiskMAMIN) show breakdowns by municipality of how many limited English speaking households and minorities these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLSTNightHighRiskLEH) to \@ref(fig:mapLSTNightHighRiskU5) and Tables \@ref(tab:tabLSTNightHighRiskLEH) to \@ref(tab:tabLSTNightHighRiskU5) in Appendix B.

```{r mapLSTNightHighRiskMALEH, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and 80th percentile of percentage of Limited English Speaking Households, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTNightEngLangMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_ENGLISH == "E" & pctileNightLST >= 0.8)

bbox_new <- st_bbox(LSTNightEngLangMA5) # current bounding box
...
tmap_save(m, "DATA/LST/ma_LSTNightEngLangMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightEngLangMA5.png")
```

\pagebreak
```{r tabLSTNightHighRiskMALEH, fig.align = "center", fig.cap="Limited English Speaking Housheolds in Census Block Groups at or above 80th percentile of nighttime average Land Surface Temperature."}
LSTNightEngLangMA5 %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Limited English Speaking Households in Highest Quintile of Nighttime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Limited English Households*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
...
```{r mapLSTNightHighRiskMAMIN, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and 80th percentile of percentage of Minorities, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTNightMinorityMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_MINORITY == "M" & pctileNightLST >= 0.8)

bbox_new <- st_bbox(LSTNightMinorityMA5) # current bounding box
...
tmap_save(m, "DATA/LST/ma_LSTNightMinorityMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightMinorityMA5.png")
```
\pagebreak
```{r tabLSTNightHighRiskMAMIN, fig.align = "center", fig.cap="Minorities in Census Block Groups at or above 80th percentile of Nighttime average Land Surface Temperature."}
LSTNightMinorityMA5 %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Minorities in Highest Quintile of Nighttime Avg LST by Municipality", align = "r",
                    col.names = c(names(.)[1:2],"Minorities*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3)) %>% 
...
[^LST24quintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of day-night average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ma_blkgrpLST_sf$meanAvgLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ma_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 

[^Dayquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ma_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ma_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 
...
[^Nightquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(ma_blkgrpLST_sf$meanNightLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(ma_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region.
...
The LST rasters were then mosaiced to create a single scene for each time period and clipped to Massachusetts. 
...
isd_ma_DayNight_sf <- readRDS("DATA/LST/isd_ne_DayNight_sf.Rds") %>% 
  st_transform(., crs = st_crs(ma_state_sf_cb)) %>% 
  st_intersection(ma_state_sf_cb,.) 
```

In order to validate land surface temperature (LST) values, MODIS LST data was compared to ground-based air temperature measurements for the same time periods across Massachusetts. Weather data was acquired from NOAA's Integrated Surface Data ('ISD') files from the NOAA National Centers for Environmental Information (https://www.ncdc.noaa.gov/isd/data-access) using the Commmon Access tool (https://www.ncei.noaa.gov/access/search/data-search/global-hourly). This yielded `r nrow(isd_ma_DayNight_sf)` weather stations across Massachusetts with air temperature data available for the same period as the MODIS LST data (i.e., July 28 - August 4, 2019; see Figure \@ref(fig:ISDstations)). 

```{r ISDstations, fig.align = "center", fig.cap="NOAA Integrated Surface Data Weather Stations across Massachusetts."}
tm_shape(ma_state_sf_cb, unit = "mi") + tm_borders(lwd = 0.2) +
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(isd_ma_DayNight_sf) + tm_dots(shape = 4, size = 0.08) +
...
cor_day <- cor.test(x = isd_ma_DayNight_sf$gridDay, y = isd_ma_DayNight_sf$tempFday, use = "pairwise.complete.obs", method = "spearman")
...
cor_night <- cor.test(x = isd_ma_DayNight_sf$gridNight, y = isd_ma_DayNight_sf$tempFnight, use = "pairwise.complete.obs", method = "spearman")
...
cor_daynight <- cor.test(x = isd_ma_DayNight_sf$gridDayNight, y = isd_ma_DayNight_sf$tempFdaynight, use = "pairwise.complete.obs", method = "spearman")
...
# Compare day average LST to weather station air temp data for same time period
lm_day <- lm(tempFday ~ gridDay, data = isd_ma_DayNight_sf)
RMSE_day <- sqrt(mean(lm_day$residuals^2))

# Compare night average LST to weather station air temp data for same time period
lm_night <- lm(tempFnight ~ gridNight, data = isd_ma_DayNight_sf)
RMSE_night <- sqrt(mean(lm_night$residuals^2))

# Compare day-night average LST to weather station air temp data for same time period
lm_daynight <- lm(tempFdaynight ~ gridDayNight, data = isd_ma_DayNight_sf)
...
```{r LSTvalidation, warning=FALSE, message=FALSE, fig.align = "center", fig.cap="Comparison of MODIS LST values and air temperatures for same time periods from NOAA Integrated Surface Data Weather Stations across Massachusetts."}

day <- isd_ma_DayNight_sf %>% 
...
g <- arrangeGrob(day,night,daynight,ncol=1,
                 ...
                 widths = unit(5,"inches"))
ggsave(g, filename = "DATA/LST/ma_validate.png", 
       width = 5, height = 8, units = "in")
knitr::include_graphics("DATA/LST/ma_validate.png")
```


### Urban Heat Island effects {-}

Urban Heat Island (UHI) effects are measured as the difference between LST raster values across Massachusetts for each time period and the average LST for rural states for the same time periods. It is represented by the following formula:

*UHI effect = LST - Mean Rural LST*

For example, to calculate the UHI effects for daytime temperatures, daytime LST pixels falling within the rural states of Massachusetts were all averaged to provide a mean rural LST benchmark value. This mean rural LST value was then subtracted from the daytime LST values of all pixels across the state. The resulting value for each pixel indicates the magnitude or size of the difference, and the sign (+/-) indicates whether it is warmer or cooler than the mean rural LST.

Rural states of Massachusetts were identified according to the U.S. Census Bureau's urban-rural classification. The Census Bureau identifies two types of urban areas:
...
Urban areas TIGER/Line spatial files were downloaded from the Census Bureau via API using the `tigris` package in R. Areas of Massachusetts not within the urban areas polygons were then extracted to represent rural states. LST raster pixels falling within these non-urban or rual polygons were extracted and a mean LST value calculated for each time period (i.e., day, night, and day-night average) to represent the mean rural LST benchmark for calculating the UHI effect.
...
tmap_save(m, "DATA/LST/ma_LST24EngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24EngLang5.png")
```
...
tmap_save(m, "DATA/LST/ma_LST24Minority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24Minority5.png")
```
...
```{r mapLST24hrHighRiskMALI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of day-night average LST and Environmental Justice communities of Low Income persons, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LST24LowIncMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_INCOME == "I" & pctile24hrLST >= 0.8)

bbox_new <- st_bbox(LST24LowIncMA5) # current bounding box
...
m <- tm_shape(LST24LowIncMA5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ma_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ma_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ma_LST24LowIncMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24LowIncMA5.png")
```
\pagebreak
```{r tabLST24hrHighRiskMALI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of day-night average Land Surface Temperature."}
LST24LowIncMA5 %>%
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Low Income Persons in Highest Quintile of Day-Night Avg LST", align = "r",
...
tmap_save(m, "DATA/LST/ma_LST24LowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24LowInc5.png")
```
\pagebreak
...
tmap_save(m, "DATA/LST/ma_LST24NoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24NoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/ma_LST24Over645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24Over645.png")
```
...
tmap_save(m, "DATA/LST/ma_LST24Under55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LST24Under55.png")
```
...
LSTDayEngLang5 <- ma_blkgrpLST_sf %>%
  filter(LSTDayEngLangScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ma_LSTDayEngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayEngLang5.png")
```
...
LSTDayMinority5 <- ma_blkgrpLST_sf %>%
  filter(LSTDayMinorScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/ma_LSTDayMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayMinority5.png")
```
\pagebreak
...
```{r mapLSTDayHighRiskMALI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of daytime average LST and Environmental Justice communities of Low Income persons, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTDayLowIncMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_INCOME == "I" & pctileDayLST >= 0.8)

bbox_new <- st_bbox(LSTDayLowIncMA5) # current bounding box
...
m <- tm_shape(LSTDayLowIncMA5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ma_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ma_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ma_LSTDayLowIncMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayLowIncMA5.png")
```
\pagebreak
```{r tabLSTDayHighRiskMALI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of daytime average Land Surface Temperature."}
LSTDayLowIncMA5 %>%
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Low Income Persons in Highest Quintile of Daytime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Day LST (°F)" = 3)) %>% 
...
tmap_save(m, "DATA/LST/ma_LSTDayLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayLowInc5.png")
```
...
LSTDayNoHSDip5 <- ma_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ma_LSTDayNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayNoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/ma_LSTDayOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayOver645.png")
```
...
tmap_save(m, "DATA/LST/ma_LSTDayUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTDayUnder55.png")
```
...
LSTNightEngLang5 <- ma_blkgrpLST_sf %>%
  filter(LSTNightEngLangScore == 5)
...
tmap_save(m, "DATA/LST/ma_LSTNightEngLang5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightEngLang5.png")
```
...
LSTNightMinority5 <- ma_blkgrpLST_sf %>%
  filter(LSTNightMinorScore == 5)
...
tmap_save(m, "DATA/LST/ma_LSTNightMinority5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightMinority5.png")
```
\pagebreak
...
```{r mapLSTNightHighRiskMALI, fig.align = "center", fig.cap="Map of Block Groups in the highest quintile (80th percentile) of nighttime average LST and Environmental Justice communities of Low Income persons, as defined by Massachusetts Environmental Justice Policy, by Census Block Group, July-Aug 2019."}
LSTNightLowIncMA5 <- ma_blkgrpLST_sf %>%
  filter(MA_INCOME == "I" & pctileNightLST >= 0.8)

bbox_new <- st_bbox(LSTNightLowIncMA5) # current bounding box
...
m <- tm_shape(LSTNightLowIncMA5, unit = "mi" , bbox = bbox_new) + tm_fill(col = "red", title = "80th Percentile", legend.is.portrait = TRUE) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) + 
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ma_highways) + tm_lines(col = "seashell4", lwd = 1) +
  tm_shape(ma_highways2nd) + tm_lines(col = "seashell4", lwd = 1) +
...
tmap_save(m, "DATA/LST/ma_LSTNightLowIncMA5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightLowIncMA5.png")
```
\pagebreak
```{r tabLSTNightHighRiskMALI, fig.align = "center", fig.cap="Low Income Persons in Census Block Groups at or above 80th percentile of nighttime average Land Surface Temperature."}
LSTNightLowIncMA5 %>%
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Low Income Persons in Highest Quintile of Nighttime Avg LST", align = "r",
                    col.names = c(names(.)[1:2],"Low Income*",names(.)[4:7])) %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped")) %>% 
  kableExtra::column_spec(2:4, width = "3cm") %>%
  kableExtra::add_header_above(c(" " = 4, "Night LST (°F)" = 3)) %>% 
...
LSTNightLowInc5 <- ma_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ma_LSTNightLowInc5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightLowInc5.png")
```
...
LSTNightNoHSDip5 <- ma_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ma_LSTNightNoHSDip5.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightNoHSDip5.png")
```
...
LSTNightOver645 <- ma_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ma_LSTNightOver645.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightOver645.png")
```
...
LSTNightUnder55 <- ma_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/ma_LSTNightUnder55.png",
          height = 5, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/ma_LSTNightUnder55.png")
```
...
```{r cormatrixLSTpopsMA, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and the proportions of populations of concern by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTpops <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
ggcorrplot(corrLSTpops, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Correlation Matrix of LST \nand Populations for Massachusetts", 
...
```{r cormatrixLSTAvgMApollutants, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and pollution by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTemissions <- ma_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  dplyr::transmute(`Day-Night Avg LST` = meanNightLST,
                   `Day LST` = meanDayLST,
                   `Night LST` = meanNightLST,
...
ggcorrplot(corrLSTemissions, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Correlation Matrix of LST \nand Air Pollution for Massachusetts",
           legend.title = "Spearman's \nCorrelation\nCoefficient", 
           lab_size = 2)
```
...
# 
# # convert to sp
# ma_blkgrpLST_sp <- as_Spatial(ma_blkgrpLST_sf)
# 
# #calculate kernel bandwidth
# GWRbandwidth <- gwr.sel(ma_blkgrpLST_sp$PM25_19 ~ ma_blkgrpLST_sp$meanAvgLST, data=ma_blkgrpLST_sp, adapt=T)
