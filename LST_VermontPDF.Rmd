...
# vt_bmap <- ne_states_sf_cb %>% 
#   st_transform(., crs = 4326) %>% 
#   st_bbox(.) %>% 
#   read_osm(., type = "stamen-watercolor")
# 
# # map basemap
# tm_shape(vt_bmap) + tm_rgb() + tm_shape(ne_states_sf_cb) + tm_borders()
...
# 
# # create urban area polygon for Vermont
# vt_urban_sf <- us_urban_sf %>%
#   st_centroid(.) %>%
#   st_transform(., crs = 2852) %>%
#   st_join(., vt_state_sf_cb, join = st_within, left = FALSE) %>%
#   as.data.frame() %>%
...
# vt_rural_sf <- vt_state_sf_cb %>%
#   group_by() %>%
#   summarize() %>%
#   st_difference(., vt_urban_sf) %>%
#   st_make_valid()
# 
# # # select blkgrps that do not intersect wtih urban polygons - rural blkgrps
# # vt_blkgrpLST_rural <- st_disjoint(vt_blkgrp_sf,
# #                                   vt_urban_sf,sparse = FALSE) %>%
# #   apply(., 1, all) %>% # convert dense matrix to logical vector
# #   vt_blkgrpLST_sf[.,] # subset blkgrps where there is NO intersection
# #
# # # select blkgrps that intersect with urban polygons - urban blkgrps
# # vt_blkgrpLST_urban <- st_intersects(vt_blkgrp_sf,
# #                                     vt_urban_sf,sparse = FALSE) %>%
# #   apply(., 1, any) %>% # convert dense matrix to logical vector
# #   vt_blkgrpLST_sf[.,] # subset blkgrps where there is ANY intersection
# #
# # vt_blkgrp_urban_simple <- vt_blkgrpLST_urban %>%
# #   group_by() %>%
# #   summarize()
# #
# # vt_blkgrp_rural_simple <- vt_blkgrpLST_rural %>%
# #   group_by() %>%
# #   summarize()
# 
# # crop LST to urban
...
# 
# meanLSTDayNightDiff_df <- vt_blkgrp_sf %>%
#   dplyr::select(GEOID) %>%
#   as_Spatial() %>%
...
# # meanLSTavgRural_df <- vt_blkgrp_rural_simple %>%
# #   as_Spatial() %>%
...
# 
# save(vt_blkgrpLST_sf, vt_urban_sf, vt_rural_sf, file = "DATA/LST/vt_blkgrpLST_sf.rds")
# 
# # clean up rasters
...
Day-Night Average Land Surface Temperatures (LST) represent a simple average of LST values collected during the day (11:48am - 2pm) and during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. These day-night average temperatures varied significantly across Vermont, with highest temperatures apparent along the shore of Lake Champlain and in the southeast along the I-91 corridor (see Figure \@ref(fig:mapLSTavgVT)). 

```{r mapLSTavgVT, fig.align = "center", fig.cap="Map of July-Aug 2019 Day-Night Average Land Surface Temperatures (LST) across Vermont at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_24hrLST.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_24hrLST.png")
```

```{r LST24outliers, include=FALSE}
# Identify municipalities with high heat outliers in Vermont
vt_LST24outliers <- vt_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTavgVT) is a boxplot of average day-night temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$meanAvgLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LST24outliers`. 

```{r boxplotLSTavgVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 day-night average Land Surface Temperatures (LST) across Vermont at Census Block Group level."}
...
The UHI effect analyzed here is defined as the difference in temperature between urbanized areas and rural areas in Vermont.[^urban] Specifically, the average temperature of the rural areas of Vermont is substracted from the average temperatures of each Census Block Group. The resulting value shows how much warmer or cooler each Census Block Group is when compared to the rural background reference. Positive values indicate that a Census Block Group is warmer than the rural background average and may be experiencing the UHI effect. The larger the value, the stronger the effect. 

The UHI effect for day-night average temperatures in Vermont are most pronounced in the inner core of urbanized areas, (see Figure \@ref(fig:mapUHIavgVT)), particularly Burlington. Other areas with elevated temperatures include Addison, St. Albans and Swanton. Temperatures in these areas are up to `r round(max(vt_blkgrpLST_sf$UHI24avg, na.rm = TRUE),1)`° warmer than the rural background average. By contrast, most of the central part of the state is closer to the rural average. 

```{r mapUHIavgVT, fig.align = "center", fig.cap="Map of July-Aug 2019 day-night average Urban Heat Island (UHI) effect across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_24hrUHI.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_24hrUHI.png")
```

```{r LST24UHIoutliers, include=FALSE}
vt_LST24UHIoutliers <- vt_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotUHIavgVT) is a boxplot of day-night average Urban Heat Island (UHI) effect by Block Group . The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median UHI for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$UHI24avg, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LST24UHIoutliers`. 

```{r boxplotUHIavgVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 day-night average Urban Heat Island (UHI) across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHI24avg) %>% 
...
# Pop Weighted avg of 24hr LST exposure for all Groups in Vermont relative to VT average
...
# Pop Weighted avg of LST24hr for all Groups in Vermont relative to VT average
LST24hrPopTab <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLST24hrTempPopAvgVT) and Table \@ref(tab:tabLST24hrPopAvgVT) show population-weighted exposures for populations of concern relative to the day-night average LST for the state of `r round(mean(vt_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),1)`°. For example, limited English speaking households in Vermont were exposed to a population-weighted day-night average LST of `r round(max(LST24hrPop$LST24hrwMean),1)`°, approximately `r round(max(LST24hrPop$LST24hrwMean) - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[1,4]`) above the state day-night average LST. Similarly, minorities were exposed to a population-weighted day-night average LST of `r LST24hrPop %>% filter(Group == "Minority") %>% summarize(round(max(LST24hrwMean),1)) %>% pull()`°, over `r round(LST24hrPop %>% filter(Group == "Minority") %>% summarize(max(LST24hrwMean)) %>% pull() - max(LST24hrPop$LST24hrMean),1)`° (`r LST24hrPopTab[3,4]`) above the state day-night average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted day-night average LSTs of `r LST24hrPopTab[5,3]` below the state average.

```{r plotLST24hrTempPopAvgVT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Vermont relative to the state average."}
LST24hrPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 71.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 69, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 81, yend = 82), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(68.5,72.5))
```


```{r tabLST24hrPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average LST for populations of concern in Vermont relative to the state average."}
LST24hrPopTab %>% 
  arrange(Group) %>% 
  ...
  kableExtra::footnote(., symbol = paste0("State average day-night LST is ", round(mean(vt_blkgrpLST_sf$meanAvgLST,na.rm=TRUE),2), "°"))
```
...
Figure \@ref(fig:plotUHI24hrTempPopAvgVT) and Table \@ref(tab:tabUHI24hrPopAvgVT) show population-weighted exposures for populations of concern relative to the day-night average Urban Heat Island (UHI) effect for Vermont. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across Vermont is approximately `r round(mean(vt_blkgrpLST_sf$UHI24avg,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Vermont were exposed to a population-weighted day-night average UHI effect of `r round(max(UHI24hrPop$UHI24hrwMean),1)`° above the rural average. This value is more than `r round(max(UHI24hrPop$UHI24hrwMean) - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[1,4]`) above the state day-night average UHI. Similarly, minorities were exposed to a population-weighted day-night average UHI effect of `r UHI24hrPop %>% filter(Group == "Minority") %>% summarize(round(max(UHI24hrwMean),1)) %>% pull()`°, approximately `r round(UHI24hrPop %>% filter(Group == "Minority") %>% summarize(max(UHI24hrwMean)) %>% pull() - max(UHI24hrPop$UHI24hrMean),1)`° (`r UHI24hrPopTab[3,4]`) above the state day-night average UHI. By contrast, persons over age 64 were, on average, were exposed to a population-weighted day-night average UHI of `r UHI24hrPopTab[5,3]` (`r UHI24hrPopTab[5,4]`) below the state average.

```{r plotUHI24hrTempPopAvgVT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Vermont."}
UHI24hrPop %>% 
...
```{r tabUHI24hrPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to day-night average Urban Heat Island effect for populations of concern in Vermont relative to the rural average."}
UHI24hrPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average day-night UHI effect is ", round(mean(vt_blkgrpLST_sf$UHI24avg,na.rm=TRUE),2), "°"))
```

```{r LST24hrHighRiskStats, include=FALSE}
Pop24hr <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLST24hrHighRiskLEH) and \@ref(fig:mapLST24hrHighRiskMIN) highlight Census Block Groups across Vermont which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and also contained high percentages of limited English speaking households or minorities in the top quintile for the state.[^LST24quintile] Approximately `r LEH24hr %>% format(., big.mark = ",") %>% pull()` limited English speaking households and `r MIN24hr %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEH24hrPct`% and `r MIN24hrPct`%, respectively, of those populations in the state. By comparison, `r HH24hrPct`% of all households, and `r Pop24hrPct`% of the general population were resident in these same Block Groups. 

Tables \@ref(tab:tabLST24hrHighRiskLEH) and \@ref(tab:tabLST24hrHighRiskMIN) show breakdowns by municipality of how many limited English speaking households and minorities these represent, and the day-night average LST values for those same Census Block Groups. Maps and tables for other populations of concern can be found in Figures \@ref(fig:mapLST24hrHighRiskLI) to \@ref(fig:mapLST24hrHighRiskU5) and Tables \@ref(tab:tabLST24hrHighRiskLI) to \@ref(tab:tabLST24hrHighRiskU5) in Appendix B.
...
tmap_save(m, "DATA/LST/vt_LST24EngLang5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24EngLang5.png")
```
...
LST24Minority5 <- vt_blkgrpLST_sf %>%
  filter(LST24MinorScore == 5)
...
tmap_save(m, "DATA/LST/vt_LST24Minority5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24Minority5.png")
```
...
Daytime Average Land Surface Temperatures (LST) represent LST values collected during the day (11:48am - 2pm) over eight days, from July 28 to August 4, 2019. Daytime average LST for the state was `r round(mean(vt_blkgrpLST_sf$meanDayLST,na.rm=TRUE),1)`°. Daytime Land Surface Temperatures (LST) varied significantly across Vermont, with highest temperatures apparent in the most urbanized areas of the state, particularly in and around Burlington and south along the shore of Lake Champlain. Other areas of high temperature appear around Brattleboro, Bennington, Berlin, and Manchester (see Figure \@ref(fig:mapLSTdayVT) below). Across Vermont, approximately `r vt_blkgrpLST_sf %>% as.data.frame() %>% filter(pctileDayLST >= 0.8) %>% group_by() %>% summarize(sum(totalpopE, na.rm = TRUE)/10^3) %>% pull() %>% round(.,0)` thousand people live in Block Groups that exceeded the 80th percentile (temperatures over `r round(quantile(vt_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),1)`°) for daytime average temperatures.  

```{r mapLSTdayVT, fig.align = "center", fig.cap="Map of July-August 2019 average daytime Land Surface Temperatures (LST) across Vermont at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_DayLST.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_DayLST.png")
```

```{r LSTDayoutliers, include=FALSE}
# Identify municipalities with high heat outliers
vt_LSTDayoutliers <- vt_blkgrpLST_sf %>% 
  arrange(desc(meanAvgLST)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(vt_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotLSTdayVT) is a boxplot of average daytime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$meanDayLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LSTDayoutliers`.

```{r boxplotLSTdayVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime average Land Surface Temperatures (LST) across Vermont at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(meanDayLST) %>% 
...
The UHI effect for daytime average temperatures in Vermont are most pronounced in the more densely urbanized areas (see Figure \@ref(fig:mapUHIdayVT)), particularly in the Burlington area, and in the cores of Berlin, Bennington, Rutland, and St. Albans. Temperatures in these areas are up to `r round(max(vt_blkgrpLST_sf$UHIDay,na.rm = TRUE),1)`° warmer than the rural background average.  

```{r mapUHIdayVT, fig.align = "center", fig.cap="Map of July-Aug 2019 daytime Urban Heat Island (UHI) effect across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_DayUHI.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_DayUHI.png")
```

```{r LSTDayUHIoutliers, include=FALSE}
vt_LSTDayUHIoutliers <- vt_blkgrpLST_sf %>% 
  arrange(desc(UHIDay)) %>% 
  slice(1:10) %>% 
  st_centroid(.) %>% 
  st_intersection(vt_towns_sf,.) %>% 
...
Figure \@ref(fig:boxplotUHIDayVT) is a boxplot of average daytime Urban Heat Island (UHI) effect values by Block Group . The box represents UHI values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median value for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$UHIDay, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LSTDayUHIoutliers`. 

```{r boxplotUHIDayVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 daytime Urban Heat Island (UHI) across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHIDay) %>% 
...
# Pop Weighted avg of LST day for all Groups in Vermont relative to VT average
LSTDayPopTab <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTDayTempPopAvgVT) and Table \@ref(tab:tabLSTDayPopAvgVT) show population-weighted exposures for populations of concern relative to the average daytime average LST for the state. For example, limited English speaking households in Vermont were exposed to a population-weighted daytime average LST of `r round(max(LSTDayPop$LSTDaywMean),1)`°, approximately `r round(max(LSTDayPop$LSTDaywMean) - max(LSTDayPop$LSTDayMean),1)`° (`r LSTDayPopTab[1,4]`) above the state daytime average LST. Similarly, minorities were exposed to a population-weighted daytime average LST of approximately `r LSTDayPop %>% filter(Group == "Minority") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, `r round(LSTDayPop %>% filter(Group == "Minority") %>% summarize(max(LSTDaywMean)) %>% pull() - LSTDayPop %>% summarize(max(LSTDayMean)) %>% pull(),1)`° (`r LSTDayPopTab[3,4]`) above the state daytime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average LSTs of `r LSTDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(LSTDaywMean),1)) %>% pull()`°, approximately `r LSTDayPopTab[5,3]` below the state average.

```{r plotLSTDayTempPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to average daytime Land Surface Temperatures for populations of concern in Vermont relative to the state average."}
LSTDayPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 82, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 80.3, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 94, yend = 97), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(80,83.5))

ggsave("DATA/LST/vt_DayLST_lolli.png")
```


```{r tabLSTDayPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to Daytime LST for populations of concern in Vermont relative to the state average."}
LSTDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime LST is ", round(mean(vt_blkgrpLST_sf$meanDayLST,na.rm=TRUE),2), "°"))
```

```{r UHIDayPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Vermont relative to VT average
UHIDayPop <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
Figure \@ref(fig:plotUHIDayTempPopAvgVT) and Table \@ref(tab:tabUHIDayPopAvgVT) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(vt_blkgrpLST_sf$UHIDay,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Vermont were exposed to a population-weighted daytime average UHI effect of `r round(max(UHIDayPop$UHIDaywMean),1)`° above the rural average. This value is more than `r round(max(UHIDayPop$UHIDaywMean) - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[1,4]`) above the state daytime average UHI. Similarly, minorities were exposed to a population-weighted daytime average UHI effect of `r UHIDayPop %>% filter(Group == "Minority") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`°, approximately `r round(UHIDayPop %>% filter(Group == "Minority") %>% summarize(max(UHIDaywMean)) %>% pull() - max(UHIDayPop$UHIDayMean),1)`° (`r UHIDayPopTab[3,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI of `r UHIDayPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHIDaywMean),1)) %>% pull()`° (`r UHIDayPopTab[5,4]`) below the state average.

```{r plotUHIDayTempPopAvgVT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Vermont."}
UHIDayPop %>% 
...
```{r tabUHIDayPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to daytime Urban Heat Island effect for populations of concern in Vermont relative to the rural average."}
UHIDayPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average daytime UHI effect is ", round(mean(vt_blkgrpLST_sf$UHIDay,na.rm=TRUE),2), "°"))
```

```{r LSTDayHighRiskStats, include=FALSE}
PopDay <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTDayHighRiskLEH) and \@ref(fig:mapLSTDayHighRiskMIN) highlight Census Block Groups across Vermont which exhibited daytime average LSTs in the top quintile (i.e., 80th percentile) and also contained high percentages of limited English speaking households or minorities in the top quintile for the state.[^Dayquintile] Approximately `r LEHDay %>% format(., big.mark = ",") %>% pull()` limited English households and `r MINDay %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHDayPct`% and `r MINDayPct`%, respectively, of those populations in the state. By comparison, `r HHDayPct`% of all households, and `r PopDayPct`% of the general population were resident in these same Block Groups.  
...
LSTDayEngLang5 <- vt_blkgrpLST_sf %>%
  filter(LSTDayEngLangScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/vt_LSTDayEngLang5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayEngLang5.png")
```
...
LSTDayMinority5 <- vt_blkgrpLST_sf %>%
  filter(LSTDayMinorScore == 5)
...
bbox_new <- bbox_new %>%  # take the bounding box ...
 ...
 tmap_save(m, "DATA/LST/vt_LSTDayMinority5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayMinority5.png")
```
\pagebreak
...
Nighttime Land Surface Temperatures (LST) represent LST values collected during the night (12am - 3:06am) over eight days, from July 28 to August 4, 2019. Nighttime average LST for the state was `r round(mean(vt_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`°. Nighttime Land Surface Temperatures (LST) varied significantly across Vermont, with highest temperatures apparent in along the shore of Lake Champlain and in the south around Bennington and Brattleboro (see Figure \@ref(fig:mapLSTnightVT) below).

```{r mapLSTnightVT, fig.align = "center", fig.cap="Map of July-August 2019 average nighttime Land Surface Temperatures (LST) across Vermont at Census Block Group level.", strip.white=TRUE}
# Map of average LST across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_NightLST.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_NightLST.png")
```

```{r LSTNightoutliers, include=FALSE}
vt_LSTNightoutliers <- vt_blkgrpLST_sf %>% 
...
Figure \@ref(fig:boxplotLSTnightVT) is a boxplot of average nighttime temperatures by Block Group . The box represents temperatures ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$meanNightLST, na.rm = TRUE),2)`°. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large dots on either end represents outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LSTNightoutliers`. 

```{r boxplotLSTnightVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Land Surface Temperatures (LST) across Vermont at Census Block Group level."}
# boxplot of PM2.5 by state for 2016
vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
The UHI effect for nighttime average temperatures in Vermont are most pronounced in and around Burlington and St. Albans (see Figure \@ref(fig:mapUHInightVT)). Temperatures in these areas are up to `r round(max(vt_blkgrpLST_sf$UHINight,na.rm = TRUE),1)`° warmer than the rural background average. By contrast, most of the rest of the state is close to or cooler than rural background average. 

```{r mapUHInightVT, fig.align = "center", fig.cap="Map of July-Aug 2019 nighttime Urban Heat Island (UHI) effect across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas.", strip.white=TRUE}
# Map of average LST UHI across Vermont
m <- tm_shape(vt_blkgrpLST_sf, unit = "mi") + 
...
tmap_save(m, "DATA/LST/vt_NightUHI.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_NightUHI.png")
```
...
Figure \@ref(fig:boxplotUHINightVT) is a boxplot of nighttime Urban Heat Island (UHI) effects by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median temperature for all Block Groups, which in this case is `r round(median(vt_blkgrpLST_sf$UHINight, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Large black dots represent outliers, or unusually high or low values. The top ten highest outlier Block Groups (far right) were found in the following municipalities: `r vt_LSTNightoutliers`.

```{r boxplotUHINightVT, fig.align = "center", fig.cap="Boxplot of July-Aug 2019 nighttime Urban Heat Island (UHI) across Vermont at Census Block Group level. UHI is the difference between Block Group average temperature and average temperature of rural areas."}
# boxplot of PM2.5 by state for 2016
vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  drop_na(UHINight) %>% 
...
# Pop Weighted avg of PM2.5 for all Groups in Vermont relative to VT average
LSTNightPopTab <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
In addition to variations in the general geography of Land Surface Temperatures (LST), temperature exposure also varies demographically. Figure \@ref(fig:plotLSTNightTempPopAvgVT) and Table \@ref(tab:tabLSTNightPopAvgVT) show population-weighted exposures for populations of concern relative to the average nighttime average LST of `r round(mean(vt_blkgrpLST_sf$meanNightLST,na.rm=TRUE),1)`° for the state. For example, limited English speaking households in Vermont were exposed to a population-weighted nighttime average LST of `r round(max(LSTNightPop$LSTNightwMean),1)`°, approximately `r round(max(LSTNightPop$LSTNightwMean) - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[1,4]`) above the state nighttime average LST. Similarly, minorities were exposed to a population-weighted nighttime average LST of `r LSTNightPop %>% filter(Group == "Minority") %>% summarize(round(max(LSTNightwMean),1)) %>% pull()`°, over `r round(LSTNightPop %>% filter(Group == "Minority") %>% summarize(max(LSTNightwMean)) %>% pull() - max(LSTNightPop$LSTNightMean),1)`° (`r LSTNightPopTab[3,4]`) above the state nighttime average LST. By contrast, persons over age 64 were, on average, exposed to population-weighted nighttime average LSTs `r LSTNightPopTab[5,3]` below the state average.

```{r plotLSTNightTempPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to average nighttime Land Surfact Temperatures for populations of concern in Vermont relative to the state average."}
LSTNightPop %>% 
  ...
  geom_text(aes(x = "Low Income", y = 60.5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = 58.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 66.9, yend = 67.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(58,62))
```


```{r tabLSTNightPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime LST for populations of concern in Vermont relative to the state average."}
LSTNightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime LST is ", round(mean(vt_blkgrpLST_sf$meanNightLST,na.rm=TRUE),2), "°"))
```

```{r UHINightPop, include=FALSE}
# Pop Weighted avg of 24hr LST exposure for all Groups in Vermont relative to VT average
UHINightPop <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
# Pop Weighted avg of PM2.5 for all Groups in Vermont relative to VT average
UHINightPopTab <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Vermont") %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
...
Figure \@ref(fig:plotUHINightTempPopAvgVT) and Table \@ref(tab:tabUHINightPopAvgVT) show population-weighted exposures for populations of concern relative to the average daytime average Urban Heat Island (UHI) effect for the state. All populations of concern are exposed to the UHI effect on average. Indeed, the average UHI effect across the state is approximately `r round(mean(vt_blkgrpLST_sf$UHINight,na.rm=TRUE),1)`°. However, some populations experience significantly higher UHI effects. For example, limited English speaking households in Vermont were exposed to a population-weighted daytime average UHI effect of `r round(max(UHINightPop$UHINightwMean),1)`° above the rural average. This value is more than `r round(max(UHINightPop$UHINightwMean) - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[1,4]`) above the state daytime average UHI. Similarly, minorities were exposed to a population-weighted daytime average UHI effect of `r UHINightPop %>% filter(Group == "Minority") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`°, approximately `r round(UHINightPop %>% filter(Group == "Minority") %>% summarize(max(UHINightwMean)) %>% pull() - max(UHINightPop$UHINightMean),1)`° (`r UHINightPopTab[3,4]`) above the state daytime average UHI. By contrast, persons over age 64 were, on average, exposed to population-weighted daytime average UHI of `r UHINightPop %>% filter(Group == "Over 64") %>% summarize(round(max(UHINightwMean),1)) %>% pull()`° (`r UHINightPopTab[5,4]`) below the state average.

```{r plotUHINightTempPopAvgVT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Vermont."}
UHINightPop %>% 
...
```{r tabUHINightPopAvgVT, fig.align = "center", fig.cap="Population-weighted average exposures to nighttime Urban Heat Island effect for populations of concern in Vermont relative to the rural average."}
UHINightPopTab %>% 
  arrange(Group) %>% 
  kableExtra::kable(longtable = T, booktabs = T, digits = 2, 
  ...
  kableExtra::column_spec(2:4, width = "4cm") %>%
  kableExtra::kable_styling(latex_options = c("repeat_header")) %>% 
  kableExtra::footnote(., symbol = paste0("State average nighttime UHI effect is ", round(mean(vt_blkgrpLST_sf$UHINight,na.rm=TRUE),2), "°"))
```

```{r LSTNightHighRiskStats, include=FALSE}
PopNight <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
Because the risk of heat-related illness is a combination of both exposure and vulnerability, it is important to consider where these factors come together. Figures \@ref(fig:mapLSTNightHighRiskLEH) and \@ref(fig:mapLSTNightHighRiskMIN) highlight Census Block Groups across Vermont which exhibited day-night average LSTs in the top quintile (i.e., 80th percentile) and also contained high percentages of limited English speaking households or minorities in the top quintile for the state.[^Nightquintile] Approximately `r LEHNight %>% format(., big.mark = ",") %>% pull()` limited English households and `r MINNight %>% format(., big.mark = ",") %>% pull()` minorities were resident in these highest quintile Block Groups. These numbers represent `r LEHNightPct`% and `r MINNightPct`%, respectively, of those populations in the state. By comparison, `r HHNightPct`% of all households, and `r PopNightPct`% of the general population were resident in these same Block Groups. 
...
LSTNightEngLang5 <- vt_blkgrpLST_sf %>%
  filter(LSTNightEngLangScore == 5)
...
tmap_save(m, "DATA/LST/vt_LSTNightEngLang5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightEngLang5.png")
```
...
LSTNightMinority5 <- vt_blkgrpLST_sf %>%
  filter(LSTNightMinorScore == 5)
...
tmap_save(m, "DATA/LST/vt_LSTNightMinority5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightMinority5.png")
```
\pagebreak
...
[^Williams2020]: See Augusta A. Williams, Joseph G. Allen, Paul J. Catalano, and John D. Spengler. 2020. "The Role of Individual and Small-Area Social and Environmental Factors on Heat Vulnerability to Mortality Within and Outside of the Home in Boston, VT." *Climate* 8(2): 29. https://doi.org/10.3390/cli8020029
...
[^LST24quintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of day-night average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(vt_blkgrpLST_sf$meanAvgLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(vt_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 

[^Dayquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(vt_blkgrpLST_sf$meanDayLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(vt_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region. 
...
[^Nightquintile]: The top quintile, or 80th percentile, represents the value that is greater than 80% of all values in the data set. In the case of daytime average Land Surface Temperatures (LSTs) for New England, a temperature of `r round(quantile(vt_blkgrpLST_sf$meanNightLST,0.8,na.rm=TRUE),2)`° is greater than 80% of all LST values in the region. Similarly for the percentage of limited English speaking households, a Census Block Group with `r round(quantile(vt_blkgrpLST_sf$eng_limit_pctE,0.8,na.rm=TRUE),2)`% of households classified as limited English speaking respresents a percentage that is higher than 80% of all other Block Groups in the region.
...
The LST rasters were then mosaiced to create a single scene for each time period and clipped to Vermont. 
...
isd_vt_DayNight_sf <- readRDS("DATA/LST/isd_ne_DayNight_sf.Rds") %>% 
  st_transform(., crs = st_crs(vt_state_sf_cb)) %>% 
  st_intersection(vt_state_sf_cb,.) 
```

In order to validate land surface temperature (LST) values, MODIS LST data was compared to ground-based air temperature measurements for the same time periods across Vermont. Weather data was acquired from NOAA's Integrated Surface Data ('ISD') files from the NOAA National Centers for Environmental Information (https://www.ncdc.noaa.gov/isd/data-access) using the Commmon Access tool (https://www.ncei.noaa.gov/access/search/data-search/global-hourly). This yielded `r nrow(isd_vt_DayNight_sf)` weather stations across Vermont with air temperature data available for the same period as the MODIS LST data (i.e., July 28 - August 4, 2019; see Figure \@ref(fig:ISDstations)). 

```{r ISDstations, fig.align = "center", fig.cap="NOAA Integrated Surface Data Weather Stations across Vermont."}
tm_shape(vt_state_sf_cb, unit = "mi") + tm_borders(lwd = 0.2) +
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(isd_vt_DayNight_sf) + tm_dots(shape = 4, size = 0.08) +
   tm_scale_bar(breaks = c(0, 25, 50), text.size = 0.5, 
               position = c(0.35,0.005)) +
...
The validation analysis shows that LST values are positively correlated with weather station air temperatures, although nighttime values are not statistically significant (see Table \@ref(tab:tabCorrelations)). 

```{r tabCorrelations}
# Compare day average LST to weather station air temp data for same time period
cor_day <- cor.test(x = isd_vt_DayNight_sf$gridDay, y = isd_vt_DayNight_sf$tempFday, use = "pairwise.complete.obs", method = "spearman")
...
cor_night <- cor.test(x = isd_vt_DayNight_sf$gridNight, y = isd_vt_DayNight_sf$tempFnight, use = "pairwise.complete.obs", method = "spearman")
...
cor_daynight <- cor.test(x = isd_vt_DayNight_sf$gridDayNight, y = isd_vt_DayNight_sf$tempFdaynight, use = "pairwise.complete.obs", method = "spearman")
...
# Compare day average LST to weather station air temp data for same time period
lm_day <- lm(tempFday ~ gridDay, data = isd_vt_DayNight_sf)
RMSE_day <- sqrt(mean(lm_day$residuals^2))

# Compare night average LST to weather station air temp data for same time period
lm_night <- lm(tempFnight ~ gridNight, data = isd_vt_DayNight_sf)
RMSE_night <- sqrt(mean(lm_night$residuals^2))

# Compare day-night average LST to weather station air temp data for same time period
lm_daynight <- lm(tempFdaynight ~ gridDayNight, data = isd_vt_DayNight_sf)
RMSE_daynight <- sqrt(mean(lm_daynight$residuals^2))
```

Linear models are statistically significant predictors of air temperature data for day and day-night values (see Figure \@ref(fig:LSTvalidation)). The Root Mean Square Error (RMSE) of day values, `r round(RMSE_day,2)`°, and Day-Night Avg LST, `r round(RMSE_daynight,2)`°, indicate modest deviation of predicted air temperatures from observed values. These prediction error levels are consistent with published research on the use of MODIS LST as a proxy for air temperature.  

```{r LSTvalidation, warning=FALSE, message=FALSE, fig.align = "center", fig.cap="Comparison of MODIS LST values and air temperatures for same time periods from NOAA Integrated Surface Data Weather Stations across Vermont."}

day <- isd_vt_DayNight_sf %>% 
...
g <- arrangeGrob(day,night,daynight,ncol=1,
                 ...
                 widths = unit(5,"inches"))
ggsave(g, filename = "DATA/LST/vt_validate.png", 
       width = 5, height = 8, units = "in")
knitr::include_graphics("DATA/LST/vt_validate.png")
```


### Urban Heat Island effects {-}

Urban Heat Island (UHI) effects are measured as the difference between LST raster values across Vermont for each time period and the average LST for rural states for the same time periods. It is represented by the following formula:

*UHI effect = LST - Mean Rural LST*

For example, to calculate the UHI effects for daytime temperatures, daytime LST pixels falling within the rural states of Vermont were all averaged to provide a mean rural LST benchmark value. This mean rural LST value was then subtracted from the daytime LST values of all pixels across the state. The resulting value for each pixel indicates the magnitude or size of the difference, and the sign (+/-) indicates whether it is warmer or cooler than the mean rural LST.

Rural states of Vermont were identified according to the U.S. Census Bureau's urban-rural classification. The Census Bureau identifies two types of urban areas:
...
Urban areas TIGER/Line spatial files were downloaded from the Census Bureau via API using the `tigris` package in R. Areas of Vermont not within the urban areas polygons were then extracted to represent rural states. LST raster pixels falling within these non-urban or rual polygons were extracted and a mean LST value calculated for each time period (i.e., day, night, and day-night average) to represent the mean rural LST benchmark for calculating the UHI effect.
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "Vermont Low Income threshold: 25% or more of households with median household incomes below 65% statewide median",
                                     "Vermont Language Isolation threshold: 25% or more people in households where all adults speak English less than \"very well\"",
                                     "Vermont Minority threshold: 25% or more are persons of Hispanic or Latino origin or persons who are not White",
...
tmap_save(m, "DATA/LST/vt_LST24LowInc5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24LowInc5.png")
```
...
tmap_save(m, "DATA/LST/vt_LST24NoHSDip5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24NoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/vt_LST24Over645.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24Over645.png")
```
...
tmap_save(m, "DATA/LST/vt_LST24Under55.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LST24Under55.png")
```
...
tmap_save(m, "DATA/LST/vt_LSTDayLowInc5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayLowInc5.png")
```
...
LSTDayNoHSDip5 <- vt_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/vt_LSTDayNoHSDip5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayNoHSDip5.png")
```
...
tmap_save(m, "DATA/LST/vt_LSTDayOver645.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayOver645.png")
```
...
tmap_save(m, "DATA/LST/vt_LSTDayUnder55.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTDayUnder55.png")
```
...
LSTNightLowInc5 <- vt_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/vt_LSTNightLowInc5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightLowInc5.png")
```
...
LSTNightNoHSDip5 <- vt_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/vt_LSTNightNoHSDip5.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightNoHSDip5.png")
```
...
LSTNightOver645 <- vt_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/vt_LSTNightOver645.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightOver645.png")
```
...
LSTNightUnder55 <- vt_blkgrpLST_sf %>%
...
tmap_save(m, "DATA/LST/vt_LSTNightUnder55.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/LST/vt_LSTNightUnder55.png")
```
...
```{r cormatrixLSTpopsVT, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and the proportions of populations of concern by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTpops <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
...
```{r cormatrixLSTAvgVTpollutants, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of temperature and pollution by Census Block Group."}
# Create a correlation matrix of PM25 and populations of concern
corrLSTemissions <- vt_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  dplyr::transmute(`Day-Night Avg LST` = meanAvgLST,
                   `Day LST` = meanDayLST,
                   `Night LST` = meanNightLST,
...
# 
# # convert to sp
# vt_blkgrpLST_sp <- as_Spatial(vt_blkgrpLST_sf)
# 
# #calculate kernel bandwidth
# GWRbandwidth <- gwr.sel(vt_blkgrpLST_sp$PM25_19 ~ vt_blkgrpLST_sp$meanAvgLST, data=vt_blkgrpLST_sp, adapt=T)
