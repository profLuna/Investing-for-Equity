...
### PM~2.5~ in New Hampshire and Priority Populations

In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for priority populations relative to average PM~2.5~ concentrations for the state. For example, limited English households are exposed to PM~2.5~ concentrations that are over 2% above concentrations for the state as a whole. By contrast, low income persons are exposed to concentrations below the state average. 

```{r barplotPM25PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for priority populations in New Hampshire relative to the state average."}
...
nh_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Under 5", y = -0.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Diploma", xend = "No HS Diploma", y = 0.2, yend = 1.1), 
...
nh_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Under 5", y = -0.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Diploma", xend = "No HS Diploma", y = 0.35, yend = 1.5), arrow = arrow(length = unit(0.3,"cm"))) +
...
nh_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total New Hampshire On-road ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```
...
### Diesel Particulate Matter in New Hampshire and Priority Populations

In addition to variations in the general geography of DPM concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvg) below shows population-weighted exposures for priority populations relative to average DPM concentraions for the state. For example, limited English speaking households in New Hampshire are exposed to  DPM concentrations that are more than 47% above concentrations for the state as a whole. Similarly, People of Color are exposed to concentrations more than 28% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of DPM 4.8% below the state average. 
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in New Hampshire and Priority Populations

In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvg) below shows population-weighted exposures for priority populations relative to average cancer risk for the state. For example, limited English speaking households in New Hampshire experience  cancer risks from inhalation of air toxics that are more than 14% above the state as a whole. Similarly, People of Color experience  cancer risks more than 8% above the state average. By contrast, persons over age 64 are exposed to risks 1.2% below the state average. 
...
### Respiratory Hazard Index from Inhalation of Air Toxics in New Hampshire and Priority Populations

In addition to variations in the general geography of the respiratory hazard index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvg) below shows population-weighted exposures for priority populations relative to average respiratory hazard index for the state. For example, limited English speaking households in New Hampshire experience respiratory hazard indices that are over 15% above the state as a whole. Similarly, People of Color experience respiratory hazard index values more than 9% above the state average. By contrast, persons over age 64 are exposed to an average hazard index below the state average. 
...
### Traffic Proximity and Volume Exposure in New Hampshire and Priority Populations

In addition to variations in the general geography of the traffic proximity and volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for priority populations relative to average traffic proximity and volume for the state. For example, limited English speaking households are exposed to traffic proximity and volume more than 139% above the state average. By contrast, persons over age 64 are, on average, are exposed to traffic proximity and volume of more than 22% below the state average. 
...
nh_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Low Income", y = -20, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Diploma", xend = "No HS Diploma", y = 30, yend = 60), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
...
```{r cormatrixTPVPopAvg, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Spearman's correlation matrix of traffice proximity and volume and the proportions of priority populations by Census Block Group."}
# Create a correlation matrix of cancer risk and priority populations
