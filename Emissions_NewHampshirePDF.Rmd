---
title: "Transportation Burdens and Emissions in New Hampshire"
...
```
\pagebreak
# Analysis of transportation-related emissions in New Hampshire

This is an analysis of transportation-related emissions and related externalities in New Hampshire.

## PM~2.5~ in New Hampshire
...
PM~2.5~ levels vary significantly across New Hampshire, with highest concentrations in the southeast and southwest corners of New Hampshire (see Figure \@ref(fig:mapPM25RI) below). 
...
```{r mapPM25RI, fig.align = "center", fig.cap="Map of 2016 annual PM2.5 concentrations across New Hampshire at Census Block Group level."}
# Map of PM2.5 across New Hampshire
m1 <- tm_shape(nh_blkgrp_sf, unit = "mi") + 
...
tmap_save(m1, "DATA/EJSCREEN/nh_pm25.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_pm25.png")
```
...
ne_blkgrp_sp_DemoEJ <- nh_blkgrp_sf[!empty_geo,] %>% 
...
tmap_save(m2, "DATA/EJSCREEN/nh_pm25hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_pm25hotspot.png")
```

There is a statistically significant hot spot of PM~2.5~ in the southeastern corner of the state around Nashua and Salem. Warm clusters extend across the southeastern corner of the state from Nashua to Farmington. 
...
nh_town_min <- nh_blkgrp_sf %>% 
  ...
  pull(., NAME)
```
The Block Group with highest PM~2.5~ concentration value in the state is found in `r nh_town_max` and the lowest is found in `r nh_town_min`. See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.

Figure \@ref(fig:boxplotPM) is a boxplot of PM~2.5~ concentrations by Block Group. The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median PM~2.5~ concentration for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$PM25_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.  
...
nh_blkgrp_sf %>% 
  ...
  ggtitle(expression(atop(paste(PM[2.5], " Annual Concentrations in New Hampshire,"), "2016"))) +
  theme_minimal() +
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {abs(round(mean(nh_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1))}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {abs(round(min(nh_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1))}`%, have been in the southwestern corner of the state, particularly around Manchester. 

```{r mapPM25RIchange, fig.align = "center", fig.cap="Map of percent change in annual PM2.5 concentrations across New Hampshire between 2011 and 2016 at Census Block Group level."}
# Map of PM2.5 change across New Hampshire
m3 <- tm_shape(nh_blkgrp_sf, unit = "mi") + 
...
tmap_save(m3, "DATA/EJSCREEN/nh_pm25change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_pm25change.png")
```

Figure \@ref(fig:slopegraphPM25RI) below compares the average annual PM~2.5~ concentrations for the state and the region between 2011 and 2016. The region and the state both showed significant declines since 2011. Concentrations in New Hampshire remain below the rest of the region. 

```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted concentration of PM2.5 between 2011 and 2016 for New Hampshire and New England."}
# Slope graph of PM2.5 by state and region between 2011 and 2016
# Create df of 2019 actual values
PM19wSTAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
PM15wSTAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
newggslopegraph(dataframe = PM11_16wSTAvg_actual, 
                ...
                LineColor = c("NEW ENGLAND" = "#000000",
                              "New Hampshire" = "#E69F00",
                              "Massachusetts" = "#56B4E9",
                              "New Hampshire" = "#009E73",
...
### PM~2.5~ in New Hampshire and Populations of Concern

In addition to variations in the general geography of PM~2.5~ concentrations, exposure to these pollutants also varies demographically. Figure \@ref(fig:barplotPM25PopAvgRI) below shows population-weighted exposures for populations of concern relative to average PM~2.5~ concentrations for the state. For example, limited English households are exposed to PM~2.5~ concentrations that are over 2% above concentrations for the state as a whole. By contrast, low income persons are exposed to concentrations below the state average. 

```{r barplotPM25PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to PM2.5 for populations of concern in New Hampshire relative to the state average."}
# Pop Weighted avg of PM2.5 for all Groups in New Hampshire relative to state average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  ...
  coord_flip() + 
  labs(x = "", y = "", title = expression(atop(paste("Population-Weighted ", PM[2.5], " Exposure"), "(relative to New Hampshire average)"))) + 
  theme_light() +
  ...
  geom_text(aes(x = "Under 5", y = 1.2, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -0.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Dip", xend = "No HS Dip", y = 0.2, yend = 1.1), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-1,2.5))

ggsave("images/NH_PM25_graph.png")
```

Like the region as a whole, these populations have also experienced a decline in exposure since 2011. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphPM25PopAvgNE). Note however that all populations of concern now experience exposures greater than the state average, with People of Color and limited English speaking households leading on this measure. 
...
# Pop Weighted avg of PM2.5 EXPOSURE CHANGE 2011 - 2016 for all Groups in New Hampshire relative to NE average. 
# Create df of 2019 actual values
PM19wAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>%
  drop_na(PM25_19) %>% 
...
PM15wAvgs_actual <- nh_blkgrp_sf %>% 
  ...
  mutate(Year = 2011)
# Create regional average benchmark
pm15NHavg <- mean(nh_blkgrp_sf$pm_15, na.rm = TRUE)
pm19NHavg <- mean(nh_blkgrp_sf$PM25_19, na.rm = TRUE)
pmRegionalAvg_actual <- data.frame(
  Group = c("STATE AVG", "STATE AVG"),
  PM25wMean = c(pm15NHavg,pm19NHavg),
  Year = c(2011,2016)
)
#rbind tables
...
ggsave("images/NH_PM25_slope.png")
```
...
Ozone (O~3~) levels vary significantly across New Hampshire, with highest concentrations in around Nashua, and declining toward the north (see Figure \@ref(fig:mapO3RI) below).[^NONATTAINMENT] 

```{r mapO3RI, fig.align = "center", fig.cap="Map of 2016 Ozone summer seasonal average of daily maximum 8-hour concentration in air in parts per billion across New Hampshire at Census Block Group level."}

# Map of O3 across New Hampshire
m4 <- tm_shape(nh_blkgrp_sf, unit = "mi") + 
...
tmap_save(m4, "DATA/EJSCREEN/nh_O3.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_O3.png")
```
...
tmap_save(m5, "DATA/EJSCREEN/nh_O3hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_O3hotspot.png")
```

There is a statistically significant hot spot of Ozone (O~3~) centered on Nashua. A warm cluster extends northwest to Keene and northeast to Portsmouth. 

These O~3~ levels vary significantly across the state (see Table \@ref(tab:statsO3) and Figure \@ref(fig:boxplotO3)). See Table \@ref(tab:PollutantByTown) in Appendix B for concentrations by municipality.
...
Since 2011, Ozone (O~3~) levels have decreased slightly across the state, on average by `r {round(mean(nh_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)}`%. These have changes not been uniform (see Figure \@ref(fig:mapO3RIchange) below). The greatest declines, up to `r round(min(nh_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`%, have been in the north central part of the state around Franconia. By contrast, increases in summer ozone concentrations of up to `r round(max(nh_blkgrp_sf$OZONE_pctChange, na.rm = TRUE),1)`% appear around Nashua and Manchester.

```{r mapO3RIchange, fig.align = "center", fig.cap="Map of percent change in summer seasonal Ozone concentrations across New Hampshire between 2011 and 2016 at Census Block Group level."}
# Map of O3 change across New Hampshire
m6 <- tm_shape(nh_blkgrp_sf, unit = "mi") + 
...
tmap_save(m6, "DATA/EJSCREEN/nh_O3change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_O3change.png")
```

Figure \@ref(fig:slopegraphO3) below compares the average summer Ozone (O~3~) concentrations for the state and for the region between 2011 and 2016. New Hampshire's average concentrations have decreased and remain lower than the region. 

```{r slopegraphO3, fig.align = "center", fig.cap="Change in summer average ozone concentrations between 2011 and 2016 for New Hampshire and New England."}
# Slope graph of O3 by state and region between 2011 and 2016
# Create df of 2019 actual values
O319wSTAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  ...
  mutate(Year = 2016)
# Create df of 2015 values
O315wSTAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
### Ozone (O~3~) in New Hampshire and Populations of Concern

In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvg) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the region. For example, limited English households in New Hampshire are exposed to summer Ozone concentrations that are approximately 2.7% above concentrations for the state as a whole. By contrast, low income persons are, on average, exposed to concentrations of Ozone slightly below the state average. 

```{r barplotO3PopAvg, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in New Hampshire relative to the state average."}
# Pop Weighted avg of O3 for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>%
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  geom_point(color = "blue", size = 4, alpha = 0.8) +
  coord_flip() + 
  labs(x = "", y = "", title = expression(atop(paste("Population-Weighted Ozone (", O[3], ") Exposure"), "(relative to New Hampshire average)"))) + 
  theme_light() +
  theme(panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()) +
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 1.8, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -0.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Dip", xend = "No HS Dip", y = 0.35, yend = 1.5), arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-1,3.5))

ggsave("images/NH_O3_graph.png")
```

Like the state as a whole, these populations have also experienced declines in exposure since 2011, except for limited English speaking households. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvg).  
...
# Pop Weighted avg of O3 EXPOSURE CHANGE 2011 - 2016 for all Groups in New Hampshire relative to NE average. 
# Create df of 2019 actual values
O319wAvgs_actual <- nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(minorityE,
                num2povE,
...
O315wAvgs_actual <- nh_blkgrp_sf %>% 
  ...
  mutate(Year = 2011)
# Create regional average benchmark
O315NHavg <- mean(nh_blkgrp_sf$o3_15, na.rm = TRUE)
O319NHavg <- mean(nh_blkgrp_sf$OZONE_19, na.rm = TRUE)
O3RegionalAvg_actual <- data.frame(
  Group = c("STATE AVG", "STATE AVG"),
  O3wMean = c(O315NHavg,O319NHavg),
  Year = c(2011,2016)
)
#rbind tables
...
ggsave("images/NH_O3_slope.png")
```


\pagebreak
## Carbon Dioxide (CO~2~) in New Hampshire
...
On-road CO~2~ emissions closely follow major roadways across the region, especially along I-95, I-93, and I-89 (see Figure \@ref(fig:mapCO2RI) below). 

```{r mapCO2RI, fig.align = "center", fig.cap="Map of 2017 Carbon Dioxide (CO2) annual on-road emissions in metric tons per square kilometer across New Hampshire at Census Block Group level."}

# Map of CO2 across New Hampshire
m7 <- nh_blkgrp_sf %>% 
...
tmap_save(m7, "DATA/EJSCREEN/nh_CO2.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_CO2.png")
```

Significant hot spots, or clusters of high CO~2~ emissions, appear in Manchester (see Figure \@ref(fig:hotspotCO2RI) below).
...
tmap_save(m8, "DATA/EJSCREEN/nh_CO2hotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_CO2hotspot.png")
```
...
The Block Group with highest CO~2~ emissions value in the state is found in `r nh_town_maxCO2` and the lowest is found in `r nh_town_minCO2`.

Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(nh_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(nh_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(nh_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2change) below). Declines have occurred in the northeast and southwest corner of the state. Increased emissions have occurred in the southeast along the I-93 and I-95 corridors.

```{r mapCO2change, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across New Hampshire between 1990 and 2017 at Census Block Group level."}
# Map of CO2 change across New Hampshire
m9 <- nh_blkgrp_sf %>% 
...
tmap_save(m9, "DATA/EJSCREEN/nh_CO2change.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_CO2change.png")
```
...
download.file("https://www2.census.gov/census_1990/CD90_1A_1/stf1a0nh.dbf",
              destfile = "DATA/stf1a0nh.dbf", method = "curl")
# reformat df to work in code below
ne_states_df90 <- read.dbf("DATA/stf1a0nh.dbf", as.is = TRUE) %>% 
...
nh_blkgrp_sf %>% 
  ...
  ggtitle(expression(paste("Total New Hampshire ", CO[2], " Emissions by Year, 1990 - 2017", sep = ""))) +
  theme_minimal()
```

New Hampshire saw dramatic growth in CO~2~ emissions between 1990 and 2004. Since 2004, total emissions have declined slightly. However, emissions growth has exceeded population growth. This is reflected in the increase in per capita emissions since 1990 (see last three columns in Table \@ref(tab:changeCO2)).  


\pagebreak
## Diesel Particulate Matter in New Hampshire
...
DPM emissions are concentrated in the southeast corner of the state, particularly between Nashua and Concord, and also between Seabrook and Portsmouth (see Figure \@ref(fig:mapDPM) below). 

```{r mapDPM, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across New Hampshire at Census Block Group level."}

# Map of DPM across New Hampshire
m10 <- nh_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m10,"DATA/EJSCREEN/nh_DPM.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_DPM.png")
```

There are significant spatial clusters of high Diesel Particulate Matter concentrations in Portsmouth (see Figure \@ref(fig:hotspotDPM) below).
...
tmap_save(m11, "DATA/EJSCREEN/nh_DPMhotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_DPMhotspot.png")
```

DPM concentrations vary across the state (see Table \@ref(tab:statsDPM) and Figure \@ref(fig:boxplotDPM) below).  

```{r statsDPM, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
Outlier <- quantile(nh_blkgrp_sf$DSLPM_19,.75, na.rm = TRUE) + 1.5*(quantile(nh_blkgrp_sf$DSLPM_19,.75, na.rm = TRUE) - quantile(nh_blkgrp_sf$DSLPM_19,.25, na.rm = TRUE))

# identify outliers
nh_town_outlier <- nh_blkgrp_sf %>% 
...
The Block Group with highest Diesel Particulate Matter emissions value in the state is found in `r nh_town_maxDPM` and the lowest is found in `r nh_town_minDPM`.

Figure \@ref(fig:boxplotDPM) is a boxplot of DPM concentrations by Block Group. The box represents concentration values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median DPM concentration for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$DSLPM_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. Larger black dots on right represent outliers, or Block Groups with unusually high values relative to the state. In this case, outliers would be represented by DPM values greater than `r Outlier`, which all occur in `r nh_town_outlier`.

```{r boxplotDPM, fig.align = "center", fig.cap="Boxplot of Diesel Particulate Matter (DPM) by state at Census Block Group level. 1 dot = 1 Block Group."}
...
### Diesel Particulate Matter in New Hampshire and Populations of Concern

In addition to variations in the general geography of DPM concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvg) below shows population-weighted exposures for populations of concern relative to average DPM concentraions for the state. For example, limited English speaking households in New Hampshire are exposed to  DPM concentrations that are more than 47% above concentrations for the state as a whole. Similarly, People of Color are exposed to concentrations more than 28% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of DPM 4.8% below the state average. 

```{r barplotDPMPopAvg, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of Diesel Particulate Matter across New Hampshire relative to the state average."}
# Pop Weighted avg of DPM for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  geom_text(aes(x = "Low Income", y = 15, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -10, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 5, yend = 15), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-15,50))

ggsave("images/NH_DPM_graph.png")
```

There is a moderate positive correlation between the proportions of People of Color and ambient concentrations of Diesel Particulate Matter (see Figure \@ref(fig:cormatrixDPMPopAvg) in Appendix B).


\pagebreak
## Air Toxics Cancer Risk in New Hampshire
...
Cancer risks are concentrated in the southeast, between Nashua and Lebanon, and between Seabrook and Portsmouth (see Figure \@ref(fig:mapATCNE) below).

```{r mapATCNE, fig.align = "center", fig.cap="Map of 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across New Hampshire at Census Block Group level."}
...
tmap_save(m12, "DATA/EJSCREEN/nh_Cancer.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_Cancer.png")
```

There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics centered on Nashua and Manchester (see Figure \@ref(fig:hotspotATC) below).
...
tmap_save(m13, "DATA/EJSCREEN/nh_CancerHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_CancerHotspot.png")
```
\pagebreak
...
```{r statsATCNE, fig.align = "center", fig.cap="Annual 2014 lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
...
# extract block groups centroid with highest and lowest CO2 values, intersect with town polygons, and pull out town name
nh_town_maxCancer <- nh_blkgrp_sf %>% 
...
nh_town_minCancer <- nh_blkgrp_sf %>% 
...
The Block Group with highest cancer risk value in the state is found in `r nh_town_maxCancer` and the lowest is found in `r nh_town_minCancer`.

Figure \@ref(fig:boxplotATCNE) is a boxplot of cancer risk by Block Group. The box represents risk values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median lifetime cancer risk value for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$CANCER_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.  

```{r boxplotATCNE, fig.align = "center", fig.cap="Boxplot of lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across New Hampshire by state at Census Block Group level. 1 dot = 1 Block Group."}
...
### Lifetime Cancer Risk from Inhalation of Air Toxics in New Hampshire and Populations of Concern

In addition to variations in the general geography of cancer risk, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvg) below shows population-weighted exposures for populations of concern relative to average cancer risk for the state. For example, limited English speaking households in New Hampshire experience  cancer risks from inhalation of air toxics that are more than 14% above the state as a whole. Similarly, People of Color experience  cancer risks more than 8% above the state average. By contrast, persons over age 64 are exposed to risks 1.2% below the state average. 

```{r barplotATCPopAvg, fig.align = "center", fig.cap="Population-weighted average lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across New Hampshire at Census Block Group level relative to the state average."}
# Pop Weighted avg of DPM for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
  ...
  geom_text(aes(x = "Under 5", y = 5, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -2.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 1, yend = 5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-5,15))

ggsave("images/NH_Cancer_graph.png")
```

There is a moderately positive correlation between the proportions of People of Color and cancer risk (see Figure \@ref(fig:cormatrixATCPopAvg) in Appendix B).


\pagebreak
## Respiratory Hazard Index in New Hampshire
...
Higher respiratory hazard indices are concentrated in the southeast, between Nashua and Concord, and between Seabrook and Portsmouth (see Figure \@ref(fig:mapRHI) below). 

```{r mapRHI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level."}

# Map of RHI across New Hampshire
m14 <- nh_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m14, "DATA/EJSCREEN/nh_RespHzrd.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_RespHzrd.png")
```

There appear to be significant spatial clusters of high indices of respiratory hazard index from inhalation of air toxics between Nashua and Concord, and around Portsmouth (see Figure \@ref(fig:hotspotRHIRI) below).
...
tmap_save(m15, "DATA/EJSCREEN/nh_RespHzrdHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_RespHzrdHotspot.png")
```
...
```{r statsRHI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
The Block Group with highest respiratory hazard index value in the state is found in `r nh_town_maxRESP` and the lowest is found in `r nh_town_minRESP`.

Figure \@ref(fig:boxplotRHI) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.    

```{r boxplotRHI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
### Respiratory Hazard Index from Inhalation of Air Toxics in New Hampshire and Populations of Concern

In addition to variations in the general geography of the respiratory hazard index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvg) below shows population-weighted exposures for populations of concern relative to average respiratory hazard index for the state. For example, limited English speaking households in New Hampshire experience respiratory hazard indices that are over 15% above the state as a whole. Similarly, People of Color experience respiratory hazard index values more than 9% above the state average. By contrast, persons over age 64 are exposed to an average hazard index below the state average. 

```{r barplotRHIPopAvg, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level relative to the state average."}
# Pop Weighted avg of RHI for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 1, yend = 5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-5,16))

ggsave("images/NH_Resp_graph.png")
```

There is a moderate positive correlation between the proportions of People of Color and respiratory hazard index (see Figure \@ref(fig:cormatrixRHIPopAvg) in Appendix B).


\pagebreak
## Traffic Proximity and Volume in New Hampshire
...
```{r mapTPV, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}

# Map of TPV across New Hampshire
m16 <- nh_blkgrp_sf %>% 
  tm_shape(., unit = "mi") + 
...
tmap_save(m16, "DATA/EJSCREEN/nh_Traffic.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_Traffic.png")
```

There appear to be significant spatial clusters of traffic proximity and volume in Nashua and Manchester (see Figure \@ref(fig:hotspotTPV) below).
...
tmap_save(m17, "DATA/EJSCREEN/nh_TrafficHotspot.png",
          height = 7, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/nh_TrafficHotspot.png")
```
...
```{r statsTPV, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
# extract block groups centroid with highest and lowest values, intersect with town polygons, and pull out town name
nh_town_maxTraffic <- nh_blkgrp_sf %>% 
...
nh_town_minTraffic <- nh_blkgrp_sf %>% 
...
The Block Group with highest Traffic Proximity and Volume value in the state is found in `r nh_town_maxTraffic` and the lowest is found in `r nh_town_minTraffic`.

Figure \@ref(fig:boxplotTPV) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into two parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPV, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}
...
### Traffic Proximity and Volume Exposure in New Hampshire and Populations of Concern

In addition to variations in the general geography of the traffic proximity and volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for populations of concern relative to average traffic proximity and volume for the state. For example, limited English speaking households are exposed to traffic proximity and volume more than 139% above the state average. By contrast, persons over age 64 are, on average, are exposed to traffic proximity and volume of more than 22% below the state average. 

```{r barplotTPVPopAvgNE, fig.align = "center", fig.cap="Population-weighted average Traffic Proximity and Volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level relative to the state average."}
# Pop Weighted avg of TPV for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
                nhwhitepopE,
  ...
  geom_text(aes(x = "Low Income", y = -20, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "No HS Dip", xend = "No HS Dip", y = 30, yend = 60), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-30,150))

ggsave("images/NH_Traffic_graph.png")
```

There is a moderate positive correlation between the proportions of People of Color and traffic proximity and volume (see Figure \@ref(fig:cormatrixTPVPopAvg) in Appendix B).
...
[^EPAPM10risk]: See EPA SUMMARY OF PROPOSAL TO RETAIN THE AIR QUALITY STANDARDS FOR PARTICLE POLLUTION. https://www.epa.gov/sites/production/files/2020-04/documents/fanh_sheet_pm_naaqs_proposal.pdf
...
[^NONATTAINMENT]: Note that the EJSCREEN values do not directly indicate nonattainment of the NAAQS standard because the EJSCREEN data is based on estimates from a combination of modeling and monitoring for a single year, while nonattainment is determined for a large area (often a county) based on three years of monitoring data. For example, five counties in New Hampshire have been designated as "nonattainment" status for NAAQS ozone standards as of March 2020. For a list of nonattainment counties see EPA 8-Hour Ozone Designated Area State/Area/County Report. https://www3.epa.gov/airquality/greenbook/jbcs.html#RI 
...
ggcorrplot(corrPollutants, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Pollutant Correlation Matrix for New Hampshire", 
...
ggcorrplot(corrPM25, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "PM2.5 Correlation Matrix for New Hampshire", 
           legend.title = "Spearman's \nCorrelation\nCoefficient")
```
...
ggcorrplot(corrO3, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Ozone Correlation Matrix for New Hampshire", 
           legend.title = "Spearman's \nCorrelation\nCoefficient")
```
...
ggcorrplot(corrDPM, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "DPM Correlation Matrix for New Hampshire", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
...
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Cancer Risk Correlation Matrix for New Hampshire", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           ggtheme = ggplot2::theme_minimal())
```
...
# corrPM25
ggcorrplot(corrARI, hc.order = TRUE, type = "lower", lab = TRUE, 
           title = "Respiratory Hazard Index \nCorrelation Matrix for New Hampshire", 
           legend.title = "Spearman's \nCorrelation\nCoefficient",
           ggtheme = ggplot2::theme_minimal())
```
...
ne_towns_sf %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    caption = "Average Pollutant and Transportation Burden Values by Municipality in New Hampshire", align = "r") %>% 
