---
title: "Transportation Emissions in New Hampshire"
...
There is a statistically significant hot spot of PM~2.5~ around Nashua. Warm clusters extend across the southeastern corner of the state. 
...
Since 2011, PM~2.5~ levels have declined across the state, on average by `r {round(mean(nh_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%. Unsurprisingly, this decline has not been uniform (see Figure \@ref(fig:mapPM25RIchange) below). The greatest declines, up to `r {round(min(nh_blkgrp_sf$PM25_pctChange, na.rm = TRUE),1)}`%, have been in the southwestern corner of the state, particularly around Manchester. 
...
```{r slopegraphPM25RI, fig.align = "center", fig.cap="Change in population-weighted exposure to PM2.5 between 2011 and 2016 for New Hampshire and New England."}
...
nh_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Under 5", y = 1.2, label = "Above state avg"),
            color = "gray48") +
...
There is a statistically significant hot spot of Ozone (O~3~) centered on Nashua. A warm cluster extends northwest to Keene and northwest to Portsmouth. 
...
Figure \@ref(fig:slopegraphO3RI) below compares the average summer Ozone (O~3~) concentrations for the state and for the region between 2011 and 2016. New Hampshire's average concentrations have decreased and remain lower than the region. 

```{r slopegraphO3RI, fig.align = "center", fig.cap="Change in summer average ozone concentrations between 2011 and 2016 for New Hampshire and New England."}
...
In addition to variations in the general geography of Ozone (O~3~) concentrations, exposure to this pollutant also varies demographically. Figure \@ref(fig:barplotO3PopAvgRI) below shows population-weighted exposures for populations of concern relative to average Ozone concentrations for the region. For example, limited English households in New Hampshire are exposed to summer Ozone concentrations that are approximately 2.7% above concentrations for the state as a whole. By contrast, low income persons are, on average, exposed to concentrations of Ozone at or below the state average. 

```{r barplotO3PopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to Ozone for populations of concern in New Hampshire relative to the regional average."}
...
nh_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 1.8, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "No HS Dip", xend = "No HS Dip", y = 0.35, yend = 1.5), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-0.5,3.5))
```

Like the region as a whole, these populations have also experienced changes in exposure since 2011, except for limited English households. The comparison between exposure for these groups since 2011 is displayed below in Figure \@ref(fig:slopegraphO3PopAvgRI).  
...
Since 1990, CO~2~ emissions increased across the state, on average by `r {round((sum(nh_blkgrp_sf$kgco2_2017, na.rm = TRUE)-sum(nh_blkgrp_sf$kgco2_1990, na.rm = TRUE))/sum(nh_blkgrp_sf$kgco2_1990, na.rm = TRUE)*100,1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2RIchange) below). Declines have occurred in the northeast and southwest corner of the state. Increased emissions have occurred in the southeast along the I-93 and I-95 corridors.

```{r mapCO2RIchange, fig.align = "center", fig.cap="Map of percent change in annual CO2 emissions across New Hampshire between 1990 and 2017 at Census Block Group level."}
...
```{r linegraphCO2RI, fig.align = "center", fig.cap="Total CO2 emissions 1990 to 2017 for New Hampshire."}
...
New Hampshire saw dramatic growth in CO~2~ emissions between 1990 and 2004. Since 2004, total emissions have declined slightly. However, emissions growth has exceeded population growth. This is reflected in the increase in per capita emissions since 1990 (see last three columns in Table \@ref(tab:changeCO2RI)).  
...
Diesel Particulate Matter (DPM) emissions are concentrated in the southeast corner of the state, particuarly between Nashua and Concord, and also between Seabrook and Portsmouth (see Figure \@ref(fig:mapDPMRI) below). 

```{r mapDPMRI, fig.align = "center", fig.cap="Map of 2014 annual average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across New Hampshire at Census Block Group level."}
...
There are significant spatial clusters of high Diesel Particulate Matter concentrations in Portsmouth (see Figure \@ref(fig:hotspotDPMRI) below).
...
```{r statsDPMRI, fig.align = "center", fig.cap="Annual 2014 average ambient concentrations of Diesel Particulate Matter in micrograms per cubic meter across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
In addition to variations in the general geography of Diesel Particulate Matter concentrations, exposure to these concentrations also varies demographically. Figure \@ref(fig:barplotDPMPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Diesel Particulate Matter concentraions for the state. For example, limited English households in New Hampshire are exposed to  Diesel Particulate Matter concentrations that are more than 47% above concentrations for the state as a whole. Similarly, minorities are exposed to concentrations more than 28% above the state average. By contrast, persons over age 64 are, on average, exposed to concentrations of Diesel Particulate Matter below the state average. 

```{r barplotDPMPopAvgRI, fig.align = "center", fig.cap="Population-weighted average exposures to annual average ambient concentrations of Diesel Particulate Matter across New Hampshire relative to the regional average."}
...
nh_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 15, label = "Above state avg"),
            color = "gray48") +
...
There is a moderate positive correlation between the proportions of minorities and ambient concentrations of Diesel Particulate Matter (see Appendix).
...
Air Toxics Cancer Risk are concentrated in the southeast, between Nashua and Lebanon, and between Seabrook and Portsmouth (see Figure \@ref(fig:mapATCNE) below).
...
There appear to be significant spatial clusters of high lifetime cancer risk from inhalation of air toxics centered on Nashua and Manchester (see Figure \@ref(fig:hotspotATCRI) below).
...
The Block Group with highest cancer risk value in the state is found in `r nh_town_maxCancer` and the lowest is found in `r nh_town_minCaner`.
...
In addition to variations in the general geography of Lifetime Cancer Risk from Inhalation of Air Toxics, exposure to these risks also varies demographically. Figure \@ref(fig:barplotATCPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Lifetime Cancer Risk from Inhalation of Air Toxics for the state. For example, limited English households in New Hampshire experience lifetime cancer risks from inhalation of air toxics that are more than 13% above the state as a whole. Similarly, minorities experience lifetime cancer risks more than 8% above the state average. By contrast, persons over age 64 are exposed to concentrations below the state average. 

```{r barplotATCPopAvgRI, fig.align = "center", fig.cap="Population-weighted average lifetime cancer risk from inhalation of air toxics (expressed as risk in-1 million) across New Hampshire at Census Block Group level relative to the state average."}
# Pop Weighted avg of DPM for all Groups in New Hampshire relative to NE average
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  dplyr::select(totalpopE,
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 5, label = "Above state avg"),
            color = "gray48") +
  geom_segment(aes(x = "Low Income", xend = "Low Income", y = 1, yend = 5), 
               arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-3,15))
```

There is a moderately positive correlation between the proportions of minorities and cancer risk (see Appendix).
...
Higher Respiratory Hazard Indices are concentrated in the southeast, between Nashua and Concoard, and between Seabrook and Portsmouth (see Figure \@ref(fig:mapRHIRI) below). 

```{r mapRHIRI, fig.align = "center", fig.cap="Map of 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level."}
...
```{r statsRHIRI, fig.align = "center", fig.cap="Annual 2014 respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  # group_by(STATE) %>% 
...
Figure \@ref(fig:boxplotRHIRI) is a boxplot of respiratory hazard indices by Block Group . The box represents index values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median respiratory hazard index value for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$RESP_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group.    

```{r boxplotRHIRI, fig.align = "center", fig.cap="Boxplot of respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire by state at Census Block Group level. 1 dot = 1 Block Group."}
# boxplot of lifetime cancer risk by state for 2014
nh_blkgrp_sf %>% 
  as.data.frame() %>% 
  drop_na(RESP_19) %>% 
...
In addition to variations in the general geography of the Respitory Hazard Index, exposure to these risks also varies demographically. Figure \@ref(fig:barplotRHIPopAvgRI) below shows population-weighted exposures for populations of concern relative to average Respiratory Hazard Index from Inhalation of Air Toxics for the state. For example, limited English households in New Hampshire experience respiratory hazard indices from inhalation of air toxics that are over 15% above the state as a whole. Similarly, minorities experience lifetime hazard index values more than 9% above the state average. By contrast, persons over age 64 are exposed to an avearge hazard index below the state average. 

```{r barplotRHIPopAvgRI, fig.align = "center", fig.cap="Population-weighted average respiratory hazard index from inhalation of air toxics (expressed as ratio of exposure concentration to health-based reference concentration) across New Hampshire at Census Block Group level relative to the regional average."}
...
There is a moderate positive correlation between the proportions of minorities and respiratory hazard index (see Appendix).
...
```{r mapTPVRI, fig.align = "center", fig.cap="Map of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}
...
There appear to be significant spatial clusters of traffic proximity and volume in Nashua and Manchester (see Figure \@ref(fig:hotspotTPVRI) below).
...
```{r statsTPVRI, fig.align = "center", fig.cap="Annual 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}
nh_blkgrp_sf %>%
  as.data.frame() %>% 
  drop_na(PTRAF_19) %>% 
  # group_by(STATE) %>% 
...
Figure \@ref(fig:boxplotTPVRI) is a boxplot of Traffic Proximity and Volume values by Block Group . The box represents values ranging between the 25th and 75th percentiles. The line that divides the box into 2 parts represents the median Traffic Proximity and Volume value for all Block Groups, which in this case is `r round(median(nh_blkgrp_sf$PTRAF_19, na.rm = TRUE),2)`. Half of the state's Block Groups are below the median and half are above the median. Each dot represents an individual Block Group. The large black dots represent outliers, or unusually high values.  

```{r boxplotTPVRI, fig.align = "center", fig.cap="Boxplot of 2017 traffic proximity and volume (calculated as a count of vehicles (average annual daily traffic) at major roads within 500 meters, divided by distance in kilometers (km)) across New Hampshire at Census Block Group level."}
...
In addition to variations in the general geography of the Traffic Proximity and Volume exposure, this exposure also varies demographically. Figure \@ref(fig:barplotTPVPopAvgNE) below shows population-weighted exposures for populations of concern relative to average Traffic Proximity and Volume for the state. For example, minorities are exposed to Traffic Proximity and Volume more than 137% above the state average. By contrast, persons over age 64 are, on average, are exposed to Traffic Proximity and Volume of more than 22% below the state average. 
...
There is a moderate positive correlation between the proportions of minorities and traffic proximity and volume (see Appendix).
...
ACSvDF <- data.frame(Variable = c("Total Population",
                     ...
                     Description = c("Total population",
                                     ...
                                     "New Hampshire Low Income threshold: 30% or more people in households where the household income is less than or equal to twice the federal poverty level",
                                     ...
                                     "New Hampshire Low Income threshold: highest 15% of block groups with people in households where the household income is less than or equal to twice the federal poverty level",
                                     "New Hampshire Minority threshold: highest 15% of block groups with persons of Hispanic or Latino origin or persons who are not White"),
