...
ma_totalpop_pts <- ma_blkgrp_2805 %>% 
  select(totalpopE) %>% 
  filter(totalpopE >= 1000) %>% 
  st_sample(., size = round(.$totalpopE/1000)) %>% # create 1 random point for every 1000 people
  st_sf(.) %>% 
  group_by() %>% 
