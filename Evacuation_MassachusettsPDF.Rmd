...
# save(ma_blkgrps_nfhza,ma_tracts_nfhza,
...
```{r mapNFHZAPop, fig.align = "center", fig.cap="FEMA flood zones and population distribution across Massachusetts at Census Block Group level."}
# Create a dot density map of total populations and overlay on flood zones
...
The maps below (Figures \@ref(fig:mapNFHZAPopConcern1) and \@ref(fig:mapNFHZAPopConcern2)) show those populations disproportionately exposed to flood risk. Maps for other populations of concern can be found in Figures \@ref(fig:mapNFHZAPopConcern3) to \@ref(fig:mapNFHZAPopConcern12) and breakdowns by municipality in Tables \@ref(tab:tabNFHZAMALEH) to \@ref(tab:tabNFHZAU18) in Appendix B.
...
# save data for later analysis
save(ma_blkgrps_hevac, ma_tracts_hevac, 
...
tmap_save(m2, "DATA/FEMA/MA/ma_hevac.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/ma_hevac.png")
```
...
```{r tabHEVACpop, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Massachusetts Populations Living within Hurricane Evacuation Zones."}
...
ma_HevacPops_df %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','),
                    caption = "Massachusetts Populations Living within Hurricane Evacuation Zones",
                    digits = 0, align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header"))
```

Some groups are disproportionately exposed to these hurricane evacuation hazards. While approximately 14.1% of the general population are living within hurricane evacuation zones, people in limited English speaking households, households without access to a car, minorities, people in low income households, adults with less than a high school education, and other populations of concern exceed the population average living within hurricane evacuation zones. These populations of concern are disproportionately exposed to hurricane evacuation risk (see Figure \@ref(fig:chartHEVACpop)).

```{r chartHEVACpop, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Massachusetts Populations Living within Hurricane Evacuation Zones."}
# Create lollipop plot of pops within flood zones
ma_HevacPops_df %>% 
  ...
  geom_point(color = "blue", size = 4, alpha = 0.8) +
  coord_flip() + xlab("") + ylab("") + 
  ggtitle("Massachusetts Populations Living within\nHurricane Evacuation Zones") + theme_light() +
  theme(panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()) +
  ...
  geom_text(aes(x = "MA Low Income", y = 24, label = "Above state\naverage"),
            color = "gray48") +
  geom_text(aes(x = "MA Low Income", y = 8.5, label = "Below state\naverage"),
            color = "gray48") +
  # geom_segment(aes(x = "Total Pop", xend = "Total Pop", y = 15, yend = 22), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(6,40))

ggsave("DATA/FEMA/MA/ma_hevac_lolli.png")
```

The maps below (Figures \@ref(fig:mapHEVACPopConcern1) and \@ref(fig:mapHEVACPopConcern2)) show limited English speaking households and households without a car which are disproportionately exposed to hurricane evacuation risk. Maps for other populations of concern can be found in Figures \@ref(fig:mapHEVACPopConcern3) to \@ref(fig:mapHEVACPopConcern12) as well as breakdowns by municipality in Tables \@ref(tab:tabHEVACMALEH) to \@ref(tab:tabHEVACU18) in Appendix B.
...
# 
# Disabled_hevac_pts <- pop2points(ma_tracts_hevac,NewDisabled,"Disabled")
# 
# Under5_hevac_pts <- pop2points(ma_blkgrps_hevac,NewUnder5,"Under 5")
# 
# Over64_hevac_pts <- pop2points(ma_blkgrps_hevac,NewOver64,"Over 64")
# 
# Under18_hevac_pts <- pop2points(ma_blkgrps_hevac,NewUnder18,"Under 18")
# 
# # Bring them together
...
```{r mapHEVACPopConcern1, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Limited English Speaking Households, as Defined by the Massachusetts Environmental Justice Policy, Living within Hurricane Evacuation Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "MA Limited English HH")
...
tmap_save(m2, "DATA/FEMA/MA/MA_hevac_vulnerable1.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable1.png")
```

```{r mapHEVACPopConcern2, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Households without a Car Living within Hurricane Evacuation Zones."}
# # Create a dot density map of over-represented populations in hurricane evacuation zones
...
tmap_save(m2, "DATA/FEMA/MA/MA_hevac_vulnerable2.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable2.png")
```

```{r testOSM, include=FALSE, eval=FALSE}
# # create OSM basemap. see help on openmap for type options.
ma_hevac_bmap <- ma_hevac_vulnerable %>%
  st_transform(., crs = 4326) %>%
  st_bbox(.) %>%
  read_osm(., type = "esri-topo")

# map basemap
# tm_layout(bg.color = "#e6f3f7") + 
  tm_shape(ma_hevac_bmap) + tm_rgb() + 
  # tm_shape(ma_awater_sf_simple) + tm_fill(col = "#e6f3f7") +
  tm_shape(ma_hevac_vulnerable) + tm_dots(col = "springgreen3",
          labels = "1 dot = 500 people",
          alpha = 0.2)
```
...
```{r tabNFHZAMALEH, fig.align = "center", fig.cap="Limited English Speaking Housheolds, as Defined by the Massachusetts Environmental Justice Policy, living within FEMA Flood Zones."}
ma_blkgrps_nfhza %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  group_by(NAME) %>%
  summarize(`MA Limited English Households` = sum(NewMA_NOENGLISH,na.rm = TRUE),
            `Pct of MA Limited English Households` = round(sum(NewMA_NOENGLISH)/max(TotalMA_ENGLISH)*100,1)) %>% 
  rename("City/Town" = NAME) %>% 
  mutate(`MA Limited English Households` = as.integer(`MA Limited English Households`),
         `Pct of MA Limited English Households` = paste0(`Pct of MA Limited English Households`,"%")) %>% 
  filter(`MA Limited English Households` > 0) %>% 
  arrange(desc(`MA Limited English Households`)) %>% 
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Limited English Speaking Households Living in FEMA Flood Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r tabNFHZAMANoCar, fig.align = "center", fig.cap="Households without a car living within FEMA Flood Zones."}
ma_tracts_nfhza %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable3.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable3.png")
```
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable4.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable4.png")
```
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable5.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable5.png")
```
...
```{r mapNFHZAPopConcern6, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Disabled Persons Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable6.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable6.png")
```
...
```{r mapNFHZAPopConcern7, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Minority Persons, as Defined by the Massachusetts Environmental Justice Policy, Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
  filter(Group == "MA Minority")
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable7.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable7.png")
```

```{r tabNFHZAMAMIN, fig.align = "center", fig.cap="Minority Persons, as Defined by the Massachusetts Environmental Justice Policy, living within FEMA Flood Zones."}
ma_blkgrps_nfhza %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  left_join(., townpopsdf,by="NAME") %>%
  group_by(NAME) %>%
  summarize(`MA Minority` = sum(NewMA_MINORITY,na.rm = TRUE),
            `Pct of MA Minority` = round(sum(NewMA_MINORITY)/max(TotalMA_MINORITY)*100,1)) %>% 
  rename("City/Town" = NAME) %>% 
  mutate(`MA Minority` = as.integer(`MA Minority`),
         `Pct of MA Minority` = paste0(`Pct of MA Minority`,"%")) %>% 
  filter(`MA Minority` > 0) %>% 
  arrange(desc(`MA Minority`)) %>% 
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Minority Persons Living in FEMA Flood Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r mapNFHZAPopConcern8, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Low Income Persons, as Defined by the Massachusetts Environmental Justice Policy, Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
  filter(Group == "MA Low Income")
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable8.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable8.png")
```

```{r tabNFHZAMALINC, fig.align = "center", fig.cap="Low Income Persons, as Defined by the Massachusetts Environmental Justice Policy, living within FEMA Flood Zones."}
ma_blkgrps_nfhza %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  left_join(., townpopsdf,by="NAME") %>%
  group_by(NAME) %>%
  summarize(`MA Low Income` = sum(NewMA_LOWINC,na.rm = TRUE),
            `Pct of MA Low Income` = round(sum(NewMA_LOWINC)/max(TotalMA_LOWINC)*100,1)) %>% 
  rename("City/Town" = NAME) %>% 
  mutate(`MA Low Income` = as.integer(`MA Low Income`),
         `Pct of MA Low Income` = paste0(`Pct of MA Low Income`,"%")) %>% 
  filter(`MA Low Income` > 0) %>% 
  arrange(desc(`MA Low Income`)) %>% 
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Low Income Persons Living in FEMA Flood Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r mapNFHZAPopConcern9, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Minority Persons Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable9.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable9.png")
```
...
```{r mapNFHZAPopConcern10, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Children Under 5 Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable10.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable10.png")
```
...
```{r mapNFHZAPopConcern11, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Persons Over 64 Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable11.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable11.png")
```
...
```{r mapNFHZAPopConcern12, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Persons Under 18 Living within Flood Zones."}
# Create data subset
nfhza_sub <- ma_nfhza_vulnerable %>% 
...
tmap_save(m, "DATA/FEMA/MA/MA_nhfza_vulnerable12.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_nhfza_vulnerable12.png")
```
...
```{r tabHEVACMALEH, fig.align = "center", fig.cap="Limited English Speaking Housheolds, as Defined by the Massachusetts Environmental Justice Policy, living within Hurricane Evacuation Zones."}
ma_blkgrps_hevac %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Limited English Speaking Households Living in Hurricane Evacuation Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r tabHEVACMANoCar, fig.align = "center", fig.cap="Households without a car living within Hurricane Evacuation Zones."}
ma_tracts_hevac %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
...
```{r mapHEVACPopConcern3, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Limited English Speaking Households Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Limited English HH")
...
m <- tm_layout(bg.color = "#e6f3f7") +
  ...
  tm_fill(col = "EVACZONE",
          palette = c("deeppink", "purple3"),
          labels = c("A: Category 1 - 2", "B: Category 3 - 4"),
          title = "Evacuation Zone &\nHurricane Category",
          alpha = 0.6,
          border.alpha = 0) +
  tm_shape(ne_states_sf_cb) + tm_borders(lwd = 0.2, alpha = 0.8) +
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable3.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable3.png")
```
...
```{r mapHEVACPopConcern4, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Adults without a High School Diploma Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "No HS Dip")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable4.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable4.png")
```
...
```{r mapHEVACPopConcern5, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Low Income Persons Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Low Income")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable5.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable5.png")
```
...
```{r mapHEVACPopConcern6, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Disabled Persons Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Disabled")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable6.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable6.png")
```
...
```{r mapHEVACPopConcern7, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Minority Persons, as Defined by the Massachusetts Environmental Justice Policy, Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "MA Minority")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable7.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable7.png")
```

```{r tabHEVACMAMIN, fig.align = "center", fig.cap="Minority Persons, as Defined by the Massachusetts Environmental Justice Policy, living within Hurricane Evacuation Zones."}
ma_blkgrps_hevac %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Minority Persons Living in Hurricane Evacuation Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r mapHEVACPopConcern8, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Low Income Persons, as Defined by the Massachusetts Environmental Justice Policy, Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "MA Low Income")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable8.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable8.png")
```

```{r tabHEVACMALINC, fig.align = "center", fig.cap="Low Income Persons, as Defined by the Massachusetts Environmental Justice Policy, living within Hurricane Evacuation Zones."}
ma_blkgrps_hevac %>% 
  st_centroid(.) %>% 
  st_intersection(ma_towns_sf,.) %>% 
  as.data.frame() %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    format.args = list(big.mark = ','), 
                    digits = 1,
                    caption = "MA Low Income Persons Living in Hurricane Evacuation Zones", align = "r") %>% 
  kableExtra::kable_styling(latex_options = c("repeat_header","striped"))
```

```{r mapHEVACPopConcern9, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Minority Persons Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Minority")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable9.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable9.png")
```
...
```{r mapHEVACPopConcern10, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Children Under 5 Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Under 5")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable10.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable10.png")
```
...
```{r mapHEVACPopConcern11, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Persons Over 64 Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Over 64")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable11.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable11.png")
```
...
```{r mapHEVACPopConcern12, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Persons Under 18 Living within Flood Zones."}
# Create data subset
hevac_sub <- ma_hevac_vulnerable %>% 
  filter(Group == "Under 18")
...
tmap_save(m, "DATA/FEMA/MA/MA_hevac_vulnerable12.png",
          height = 4, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/FEMA/MA/MA_hevac_vulnerable12.png")
```
