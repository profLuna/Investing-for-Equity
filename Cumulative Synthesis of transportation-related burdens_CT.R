...
town_pops <- get_acs(geography = "county subdivision", 
                     ...
                     state = "CT", output = "wide", year = 2018) %>% 
...
m <- burdens_town_df %>% 
  filter(`3+ Burdens` >= 1) %>%
  right_join(ct_towns_sf,., by = c("NAME" = "City/Town")) %>% 
  ...
  tm_text("STUSPS", size = 0.7, remove.overlap = TRUE, col = "gray") +
  tm_shape(ct_highways) + tm_lines(col = "seashell4", alpha = 0.7, lwd = 0.5) +
  tm_shape(I95roadSegment) +
  tm_symbols(shape = I95, border.lwd = NA, size = 0.1) +
  tm_shape(I95roadSegment2) +
  tm_symbols(shape = I95, border.lwd = NA, size = 0.1) +
...
tmap_save(m, "images/CT_CUM_BURDEN_TOWN_map2.png",
          height = 4, width = 8, units = "in", dpi = 600)
...
senate_names_pops <- ct_blkgrps_sf %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
...
burdens_senate_df <- ct_blkgrps_sf %>%
  ...
  mutate(NewArea = as.numeric(st_area(.)),
         ...
         NewPop = totalpopE*Proportion) %>% 
  st_drop_geometry() %>%
  group_by(NAMELSAD10, BurdenCount) %>% 
  summarize(Pop = sum(NewPop)) %>% 
  pivot_wider(id_cols = NAMELSAD10, names_from = BurdenCount, 
              values_from = Pop) %>% 
  as.data.frame() %>% # change from rowwise_df back to regular df
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
  transmute(`Senate District` = NAMELSAD10, `3 Burdens` = `3`, 
            `4 Burdens` = `4`) %>% 
  rowwise() %>% 
  mutate(`3+ Burdens` = sum(c_across(`3 Burdens`:`4 Burdens`))) %>% 
  left_join(., senate_names_pops, by = c("Senate District" = "NAMELSAD10")) %>% 
  mutate(`Pct 3 Burdens` = `3 Burdens`/totalpopE*100, .after = `3 Burdens`) %>% 
  mutate(`Pct 4 Burdens` = `4 Burdens`/totalpopE*100, .after = `4 Burdens`) %>% 
  mutate(`Pct 3+ Burdens` = `3+ Burdens`/totalpopE*100, 
         .after = `3+ Burdens`) %>% 
...
burdens_senate_df <- senate_districts %>% 
  as.data.frame() %>% 
  anti_join(., burdens_senate_df, by = c("NAMELSAD10" = "Senate District")) %>% 
...
burdensCnt_senate_df <- ct_blkgrps_sf %>%
  transmute(BurdenCount = as.character(BurdenCount)) %>% 
  st_centroid(.) %>% 
  st_intersection(.,senate_districts) %>% 
  as.data.frame() %>% 
...
burdensCnt_senate_df %>% kable(longtable = T, booktabs = T,
  ...
  kable_styling(latex_options = c("repeat_header","striped")) 
# save as csv
write_csv(burdensCnt_senate_df, "tables/CT_burdensCnt_senate.csv")
...
# create pop totals for each district
house_names_pops <- ct_blkgrps_sf %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
...
burdens_house_df <- ct_blkgrps_sf %>%
  ...
  as.data.frame() %>% # change from rowwise_df back to regular df
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
  transmute(`House District` = NAMELSAD10, `3 Burdens` = `3`, 
            `4 Burdens` = `4`) %>% 
  rowwise() %>% 
  mutate(`3+ Burdens` = sum(c_across(`3 Burdens`:`4 Burdens`))) %>% 
  left_join(., house_names_pops, by = c("House District" = "NAMELSAD10")) %>% 
  mutate(`Pct 3 Burdens` = `3 Burdens`/totalpopE*100, .after = `3 Burdens`) %>% 
  mutate(`Pct 4 Burdens` = `4 Burdens`/totalpopE*100, .after = `4 Burdens`) %>% 
  mutate(`Pct 3+ Burdens` = `3+ Burdens`/totalpopE*100, 
         .after = `3+ Burdens`) %>% 
...
burdens_house_df <- house_districts %>% 
  as.data.frame() %>% 
  anti_join(., burdens_house_df, by = c("NAMELSAD10" = "House District")) %>% 
...
# create a table of counts of block gruops by jurisdiction for 3+ burdens
burdensCnt_house_df <- ct_blkgrps_sf %>%
  transmute(BurdenCount = as.character(BurdenCount)) %>% 
  st_centroid(.) %>% 
  st_intersection(.,house_districts) %>% 
  as.data.frame() %>% 
...
burdensCnt_house_df %>% kable(longtable = T, booktabs = T,
  ...
  kable_styling(latex_options = c("repeat_header","striped")) 
# save as csv
write_csv(burdensCnt_house_df, "tables/CT_burdensCnt_house.csv")
