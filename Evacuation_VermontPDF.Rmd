...
vt_totalpop_pts <- vt_blkgrp_2852 %>% 
  select(totalpopE) %>% 
...
# vt_hea_sf <- st_read(dsn = "DATA/FEMA/VT",layer = "vt_hevac2") %>% 
...
# 
# # read in processed vt_blkgrps and vt_tracts
# vt_blkgrps_hevac <- st_read(dsn = "DATA/FEMA/VT", 
#                             layer = "vt_blkgrps_hevac") %>% 
#   left_join(., as.data.frame(vt_blkgrp_2852), by = "GEOID") %>%
#   st_transform(., crs = 2852) %>% 
#   mutate(NewArea = st_area(.)) %>%
#   st_make_valid()
# 
# vt_tracts_hevac <- st_read(dsn = "DATA/FEMA/VT", 
#                            layer = "vt_tracts_hevac") %>% 
#   left_join(., as.data.frame(vt_tracts_2852), by = "GEOID") %>%
#   st_transform(., crs = 2852) %>% 
#   mutate(NewArea = st_area(.)) %>% 
#   st_make_valid()
# 
# # Apportion populations based on geographic proportion of intersect
# vt_blkgrps_hevac <- vt_blkgrps_hevac %>%
#   mutate(VT_LOWINC = if_else(VT_INCOME == "I",totalpopE,0)) %>%
#   mutate(VT_LOWINC = replace_na(VT_LOWINC,0)) %>%
#   mutate(VT_MINORITIES = if_else(VT_MINORITY == "M", totalpopE,0)) %>%
#   mutate(VT_MINORITIES = replace_na(VT_MINORITIES,0)) %>% 
#   mutate(VT_NOENGLISH = if_else(VT_ENGLISH == "E", totalpopE,0)) %>%
#   mutate(VT_NOENGLISH = replace_na(VT_NOENGLISH,0)) %>%
#   mutate(Proportion = as.numeric(NewArea/OldArea),
#          NewPop = totalpopE*Proportion,
#          NewMinority = minorityE*Proportion,
#          NewUnder5 = under5E*Proportion,
...
#          NewPov = num2povE*Proportion,
#          NewLths = lthsE*Proportion,
#          NewVT_LOWINC = VT_LOWINC*Proportion,
#          NewVT_MINORITY = VT_MINORITIES*Proportion,
#          NewVT_NOENGLISH = VT_NOENGLISH*Proportion)
# 
# vt_tracts_hevac <- vt_tracts_hevac %>%
#   mutate(Proportion = as.numeric(NewArea/OldArea),
#          NewDisabled = disabledOver18E*Proportion,
#          NewNoCar = HHnoCarE*Proportion)
# 
# # Compute total populations within hurricane evac zones
# vt_hevac_blkgrp_df <- vt_blkgrps_hevac %>%
#   as.data.frame() %>%
#   summarize(`Total Pop` = as.integer(sum(NewPop)),
#             Minority = as.integer(sum(NewMinority)),
#             `Under 5` = as.integer(sum(NewUnder5)),
...
#             `Low Income` = as.integer(sum(NewPov)),
#             `No HS Dip` = as.integer(sum(NewLths)),
#             `VT Low Income` = as.integer(sum(NewVT_LOWINC)),
#             `VT Minority` = as.integer(sum(NewVT_MINORITY)),
#             `VT Limited English HH` = as.integer(sum(NewVT_NOENGLISH))) %>%
#   gather(key = Group, value = HevacPop)
# 
# vt_hevac_tracts_df <- vt_tracts_hevac %>%
#   as.data.frame() %>%
#   summarize(`Disabled` = as.integer(sum(NewDisabled)),
#             `No Car HH` = as.integer(sum(NewNoCar))) %>%
#   gather(key = Group, value = HevacPop)
# 
# # Compute total tract populations within the state for same groups
# vt_travt_hevac_pops_df <- vt_tracts_2852 %>%
#   as.data.frame() %>%
#   summarize(`Disabled` = sum(disabledOver18E),
#             `No Car HH` = sum(HHnoCarE)) %>%
#   gather(key = Group, value = VTPop) %>%
#   left_join(.,vt_hevac_tracts_df, by = "Group")
# 
# # Compute populations for state,  and join with hurricane evac pops
# vt_HevacPops_df <- vt_blkgrp_2852 %>%
#   as.data.frame() %>%
#   mutate(VT_LOWINC = if_else(VT_INCOME == "I",totalpopE,0)) %>%
#   mutate(VT_LOWINC = replace_na(VT_LOWINC,0)) %>%
#   mutate(VT_MINORITIES = if_else(VT_MINORITY == "M", totalpopE,0)) %>%
...
#             `Low Income` = sum(num2povE),
#             `No HS Dip` = sum(lthsE),
#             `VT Low Income` = sum(VT_LOWINC, na.rm = TRUE),
#             `VT Minority` = sum(VT_MINORITIES, na.rm = TRUE),
#             `VT Limited English HH` = sum(VT_NOENGLISH, na.rm = TRUE)) %>%
#   gather(key = Group, value = VTPop) %>%
#   left_join(., vt_hevac_blkgrp_df, by = "Group") %>%
#   rbind(.,vt_travt_hevac_pops_df) %>%
#   mutate(PctHevac = HevacPop/VTPop*100)
# ```
# 
# Vermont is subject to significant hurricane risk. Since 1900, Vermont has been struck by hurricanes 8 times, and 3 of those were major hurricanes (i.e., Category 3 or higher). The most recent hurricane to hit Vermont directly was [Hurricane Bob in 1991](https://www.mass.gov/service-details/the-worst-Vermont-hurricanes-of-the-20th-century), a Category 2 storm when it struck the Cape and Islands and south shore of Vermont. The latter caused over $39 million in damage in Vermont alone and left 60% of the state without power. Hurricane Sandy in 2012 did not hit Vermont directly, but nevertheless resulted in flooding of Vermont coastal communities. In general, the Cape and Islands of Vermont are subject to a [hurricane return period of approximately 13 - 16 years](https://www.nhc.noaa.gov/climo/#returns), a level of risk comparable to the Gulf Coast states including Texas, Louisiana, and Florida. Boston Harbor is subject to a hurricane return period of approximately 30 years. 
# 
...
# 
# ```{r mapHEVACPop, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="FEMA flood zones and population distribution across Vermont at Census Block Group level."}
# # Create a simplified version of hea polygons to speed up mapping
# empty_geo <- st_is_empty(vt_hea_sf)
# vt_hea_sf <- vt_hea_sf[!empty_geo,]
# 
# vt_hea_sf_simple <- vt_hea_sf %>% 
#   select(EVACZONE) %>% 
#   # st_buffer(., dist = 0) %>%
...
# tm_layout(bg.color = "#e6f3f7") +
#   tm_shape(vt_hea_sf_simple, unit = "mi") + tm_fill(col = "white") +
#   tm_shape(vt_blkgrp_2852, unit = "mi") + tm_fill(col = "white") +
#   tm_shape(ne_states_sf_cb) + tm_fill(col="white") +
#   tm_shape(ny_state_sf) + tm_fill(col="white") +
#   tm_shape(vt_awater_sf_simple) + tm_fill(col = "#e6f3f7") + 
#   tm_shape(ne_states_sf_cb) + tm_borders() +
#   tm_shape(ny_state_sf) + tm_borders() +
#   tm_shape(vt_totalpop_pts) + 
#   tm_dots(col = "darkgoldenrod3",
#           labels = "1 dot = 500 people",
#           alpha = 0.6) +
#   tm_shape(vt_hea_sf_simple) + 
...
# ```
# 
# Approximately 6.4% of Vermont's land area falls within hurricane evacuation zones. The breakdown by hurricane evacuation zone is presented in Table \@ref(tab:tabHEVACarea) below.
# 
# ```{r tabHEVACarea, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Vermont Populations within Hurricane Evacuation Zones."}
...
#   kableExtra::kable(longtable = T, booktabs = T,
#                     format.args = list(big.mark = ','),
#                     caption = "Vermont Land Area within Hurricane Evacuation Zones",
#                     digits = 0, align = "r") %>% 
#   kableExtra::kable_styling(latex_options = c("repeat_header"))
# ```
# 
...
# \pagebreak
# ```{r tabHEVACpop, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Vermont Populations within Hurricane Evacuation Zones."}
...
#   kableExtra::kable(longtable = T, booktabs = T,
#                     format.args = list(big.mark = ','),
#                     caption = "Vermont Populations within Hurricane Evacuation Zones",
#                     digits = 0, align = "r") %>% 
#   kableExtra::kable_styling(latex_options = c("repeat_header"))
# ```
# 
...
# 
# ```{r chartHEVACpop, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Vermont Populations within Hurricane Evacuation Zones."}
# # Create lollipop plot of pops within flood zones
# vt_HevacPops_df %>% 
...
#                    y = vt_HevacPops_df[1,4], yend = PctHevac), 
#                color = "skyblue") +
#   geom_point(color = "blue", size = 4, alpha = 0.8) +
#   coord_flip() + xlab("") + ylab("") + 
#   ggtitle("Vermont Populations within Hurricane Evacuation\nZones") + theme_light() +
#   theme(panel.grid.major.y = element_blank(),
#         panel.border = element_blank(),
#         axis.ticks.y = element_blank()) +
...
#             color=rgb(100,100,100, maxColorValue=255)) +
#   scale_y_continuous(labels = function(x) paste0(x, "%")) +
#   geom_hline(yintercept = vt_HevacPops_df[1,4], linetype = "dashed") +
#   geom_text(aes(x = "VT Low Income", y = 21, label = "Above state avg"),
#             color = "gray48") +
...
# ```
# 
# ```{r mapHEVACPopConcern, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Vermont Populations within Hurricane Evacuation Zones."}
# # # Create a dot density map of over-represented populations in hurricane evacuation zones
# # VTNoEngHH_hevac_pts <- pop2points(vt_blkgrps_hevac,NewVT_NOENGLISH,"VT Limited English HH")
# # 
# # LimitEngHH_hevac_pts <- pop2points(vt_blkgrps_hevac,NewEng_limit,"Limited English HH")
# # 
# # NoCarHH_hevac_pts <- pop2points(vt_tracts_hevac,NewNoCar,"No Car HH")
# # 
# # NoHSdip_hevac_pts <- pop2points(vt_blkgrps_hevac,NewLths,"No HS Dip")
# # 
# # LowInc_hevac_pts <- pop2points(vt_blkgrps_hevac,NewPov,"Low Income")
# # 
# # VTMinority_hevac_pts <- pop2points(vt_blkgrps_hevac,NewVT_MINORITY,"VT Minority")
# # 
# # VTLowInc_hevac_pts <- pop2points(vt_blkgrps_hevac,NewVT_LOWINC,"VT Low Income")
# # 
# # Minor_hevac_pts <- pop2points(vt_blkgrps_hevac,NewMinority,"Minority")
# # 
# # # Bring them together
# # vt_hevac_vulnerable <- rbind(NoCarHH_hevac_pts,
# #                              VTLowInc_hevac_pts,
# #                              VTMinority_hevac_pts,
# #                              VTNoEngHH_hevac_pts,
...
# # 
# # # save time by just loading file
# # saveRDS(vt_hevac_vulnerable, file = "DATA/FEMA/VT/vt_hevac_vulnerable.Rds")
# # # load vulnerable point file
# # vt_hevac_vulnerable <- readRDS("DATA/FEMA/VT/vt_hevac_vulnerable.Rds")
# # 
...
# #             asp = 12/10)
# # 
# # tmap_save(m2, "DATA/FEMA/VT/VT_hevac_vulnerable.png", 
# #           height = 9, width = 8, units = "in", dpi = 600)
# 
# knitr::include_graphics("DATA/FEMA/VT/VT_hevac_vulnerable.png")