...
# Create df of 2019 actual values
O319wSTAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  ...
  mutate(Year = 2016)
# Create df of 2015 values
O315wSTAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
# Create df of 2019 actual values
O319wAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(minorityE,
                num2povE,
  ...
  mutate(Year = 2016)
# Create df of 2015 values
O315wAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(mins_15,
                lowinc_15,
  ...
  mutate(Year = 2011)
# Create regional average benchmark
O315NEavg <- mean(ne_blkgrp_sf_DemoEJ$o3_15, na.rm = TRUE)
O319NEavg <- mean(ne_blkgrp_sf_DemoEJ$OZONE_19, na.rm = TRUE)
...
Because of the linear pattern of emissions along major traffic corridors, CO~2~ emissions exhibit little to no spatial clustering across the region (see Figure \@ref(fig:hotspotCO2NE) below).
...
```{r statsCO2NE, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Annual 2017 CO~2~ emissions (mtons/km^2^) at Census Block Group level."}
ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  ...
  kableExtra::kable(format.args = list(big.mark = ','), 
                    caption = "Annual 2017 CO~2~ emissions (mtons/km^2^)",
                    padding = 5)
```
...
ne_blkgrp_sf_DemoEJ %>% 
  ...
  geom_boxplot(notch = TRUE) + 
  ggtitle(expression(atop(paste("Carbon Dioxide (", CO[2], ") Emissions"), "by New England state, 2017"))) +
  theme_minimal() +
  theme(legend.position = "none", axis.text=element_text(size=8)) + 
  xlab(NULL) + 
...
Since 1990, CO~2~ emissions increased significantly across the region, on average by `r {round(mean(ne_blkgrp_sf_DemoEJ$CO2_pctChange90_17, na.rm = TRUE),1)}`%. These changes have not been uniform (see Figure \@ref(fig:mapCO2NEchange) below). Except for Maine, the greatest declines, up to 97%, have been in outer suburban and rural areas. By contrast, increases in CO~2~ emissions of up to 648% appear across most urban cores. Boston is the notable exception where increases have been relatively modest. Dramtic increases characterize areas around Worcester, Massachusetts and the South Shore of Massachusetts and the Cape, as well as around Portland, Maine, and from Stamford to Hartford, Connecticut. Significant increases also occur west and north of Montpelier, Vermont.
...
Averaged across the states, the differences in these changes are also apparent. Table \@ref(tab:changeCO2NE) and Figure \@ref(fig:slopegraphCO2NE) below compare the average annual CO~2~ emissions by state and for the region between 1990 and 2017. New England as a whole showed an almost 30% increase in average annual CO~2~ emissions. All states showed average increases, with New Hampshire leading with more than 90% increase. Rhode Island showed the smallest increase since 1990 (4.6%), although note that Rhode Island's average annual emissions still remain significantly above the regional average. 

```{r changeCO2NE, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Annual Average 2017 CO~2~ emissions (mtons/km^2^) and Percent Change by state at Census Block Group level."}
ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  summarize(`1990 Avg (mtons/km^2)` = mean(onroad_1990, na.rm = TRUE),
            `2017 Avg (mtons/km^2)` = mean(onroad_2017, na.rm = TRUE),
            `2017 Per Capita (kg/person)` = (`2017 Avg (mtons/km^2)`*1000)/sum(totalpopE),
            `Avg Pct Change` = 
              paste0(
                round(
                  (`2017 Avg (mtons/km^2)` - `1990 Avg (mtons/km^2)`)/`1990 Avg (mtons/km^2)`*100,0),"%")) %>% 
  left_join(., ne_state_90, by = c("STATE" = "NAME")) %>% 
  mutate(`1990 Per Capita (kg/person)` = (`1990 Avg (mtons/km^2)`*1000)/sum(totalpop),
         `Per Capita Pct Change` = 
           paste0(
           round(
             (`2017 Per Capita (kg/person)` - `1990 Per Capita (kg/person)`)/`1990 Per Capita (kg/person)`*100,0),"%")) %>% 
  transmute(STATE = STATE, 
            `1990 Avg (mtons/km^2^)` = `1990 Avg (mtons/km^2)`, 
            `2017 Avg (mtons/km^2^)` = `2017 Avg (mtons/km^2)`,
            `Avg Pct Change` = `Avg Pct Change`,
            `1990 Per Capita (kg/person)` = 
              as.character(round(`1990 Per Capita (kg/person)`,2)),
            `2017 Per Capita (kg/person)` = 
              as.character(round(`2017 Per Capita (kg/person)`,2)),
            `Per Capita Pct Change` = `Per Capita Pct Change`) %>% 
  kableExtra::kable(format.args = list(big.mark = ','),
                    caption = "Annual Average On-road CO~2~ Emissions",
                    digits = 0, align = "r")
```


```{r slopegraphCO2NE, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.cap="Slope graph of summer average Ozone (O~3~) concentrations for 2011 and 2016 for New England and states."}
# Slope graph of CO2 by state and region between 1990 and 2017
# Create df of 2017 actual values
CO217wSTAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
...
CO290wSTAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  ...
  mutate(Year = 1990)
# Create regional average benchmark
CO290NEavg <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  summarize(CO2Mean = mean(onroad_1990, na.rm = TRUE),
            CO2wMean = weighted.mean(x = onroad_1990,
                                      w = pop_15, na.rm = TRUE)) %>% 
  transmute(STATE = "NEW ENGLAND", 
            ...
            CO2wMean = CO2wMean,
            Year = 1990)
CO217NEavg <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  summarize(CO2Mean = mean(onroad_2017, na.rm = TRUE),
            CO2wMean = weighted.mean(x = onroad_2017,
                                      w = totalpopE, na.rm = TRUE)) %>% 
...
The growth in average CO~2~ emissions exceeds population growth as is evident in the per capita emissions (see Table \@ref(tab:changeCO2NE)). 
...
# Create df of 2017 actual values
CO217wAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(minorityE,
                num2povE,
  ...
  mutate(Year = 2017)
# Create df of 1990 values
CO290wAvgs_actual <- ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  # filter(STATE == "Massachusetts") %>%
  dplyr::select(mins_15,
                lowinc_15,
  ...
  mutate(Year = 1990)
# Create regional average benchmark
CO290NEavg <- mean(ne_blkgrp_sf_DemoEJ$onroad_1990, na.rm = TRUE)
CO217NEavg <- mean(ne_blkgrp_sf_DemoEJ$onroad_2017, na.rm = TRUE)
