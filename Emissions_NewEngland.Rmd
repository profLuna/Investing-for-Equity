...
Because of the linear pattern of emissions, CO~2~ emissions exhibit little to no spatial clustering (see Figure \@ref(fig:hotspotCO2NE) below).
...
Averaged across the states, the differences in these changes are also apparent. Table \@ref(tab:changeCO2NE) and Figure \@ref(fig:slopegraphCO2NE) below compare the average annual CO~2~ emissions by state and for the region between 1990 and 2017. New England as a whole showed an almost 30% increase in annual CO~2~ emissions. All states showed average increases, with New Hampshire leading with more than 90% increase. Rhode Island showed the smallest increase since 1990 (4.6%), although note that Rhode Island's average annual emissions still remain significantly above the regional average. 
...
ne_blkgrp_sf_DemoEJ %>% 
  as.data.frame() %>% 
  group_by(STATE) %>% 
  summarize(`1990 Mean` = mean(onroad_1990, na.rm = TRUE),
            `2017 Mean` = mean(onroad_2017, na.rm = TRUE),
            `Pct Change` = paste0(round((`2017 Mean` - `1990 Mean`)/`1990 Mean`*100,1),"%")) %>% 
  kableExtra::kable(format.args = list(big.mark = ','),
