...
m <- tm_shape(ne_blkgrp_sf, unit = "mi") + 
...
tmap_save(m, "DATA/EJSCREEN/ne_pm25.png",
          height = 8, width = 8, units = "in", dpi = 600)

knitr::include_graphics("DATA/EJSCREEN/ne_pm25.png")
```
...
# ne_pm25avg <- median(ne_blkgrp_sf$PM25_19, na.rm = TRUE)
...
ne_blkgrp_sf %>% 
  ...
  # geom_hline(aes(yintercept = ne_pm25avg), color = "gray", 
  #            linetype = "dashed") +
  # geom_text(aes(x = "New Hampshire", y = 6.7,
  #               label = "NE median", color = "gray", angle = 270))
```
...
ne_blkgrp_sf %>% 
  ...
  geom_text(aes(x = Group, y = Pct + 0.2 * sign(Pct), 
            ...
            hjust = 1.3, vjust = -.8, size = 3,
            color=rgb(100,100,100, maxColorValue=255)) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 2.5, label = "Above\nregional average"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -1.3, label = "Below\nregional average"),
            color = "gray48") +
  
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 0.5, yend = 2.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-2.5,6))
```
...
# compute regional median
ne_O3med <- median(ne_blkgrp_sf$OZONE_19, na.rm = TRUE)
...
ne_blkgrp_sf %>% 
  ...
  geom_text(aes(x = "Massachusetts", y = 72, 
            ...
            color = "red4", angle = 270)
  # geom_hline(aes(yintercept = ne_O3med), color = "gray", 
  #            linetype = "dashed") +
  # geom_text(aes(x = "Vermont", y = 43,
  #               label = "NE median", color = "gray", angle = 270)) 
```
...
ne_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 2, label = "Above\nregional average"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -1, label = "Below\nregional average"),
            color = "gray48") +
  # geom_segment(aes(x = "Over 64", xend = "Over 64", y = 0.5, yend = 2.5), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
...
# create row for region
ne_CO2_row <- ne_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(onroad_2017 = kgco2_2017/1000) %>% 
  summarize(STATE = "NEW ENGLAND",
            Mean = round(mean(onroad_2017, na.rm = TRUE),0),
...
ne_blkgrp_sf %>% 
  ...
  kableExtra::kable(longtable = T, booktabs = T,
                    ...
                    caption = "Annual 2017 On-road CO2 emissions (mtons) by Census block group by state and region", align = "r") %>% 
...
ne_states_df <- map_df(ne_states, function(x) {
  get_acs(geography = "state", year = 2017,
...
# acquire pop90 data previously downloaded dbf files from https://www2.census.gov/census_1990/CD90_1A_1/" because Census API endpoints changed and not working in tidycensus as of 9/18/2020
library(foreign)
# read in files and bind into one table
ne_states_df90 <- list.files(path = "DATA", pattern = "\\.dbf$", 
                               full.names = TRUE) %>% 
  lapply(., function(x) read.dbf(x, as.is = TRUE)) %>% 
  lapply(., function(x) transmute(x, NAME = ANPSADPI, pop90 = P0010001)) %>% 
...
# create row for region
ne_DPM_row <- ne_blkgrp_sf %>% 
  as.data.frame() %>% 
  summarize(STATE = "NEW ENGLAND",
            Mean = round(mean(DSLPM_19, na.rm = TRUE),3),
...
ne_blkgrp_sf %>% 
  ...
  ggtitle("Population-Weighted Exposure to DPM \n(relative to New England average)") + theme_light() +
  theme(panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()) +
  geom_text(aes(x = Group, y = Pct + 0.2 * sign(Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 20, label = "Above\nregional avg"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -8, label = "Below\nregional avg"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 5, yend = 25), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
...
# create row for region
ne_ATR_row <- ne_blkgrp_sf %>% 
  as.data.frame() %>% 
  summarize(STATE = "NEW ENGLAND",
            Mean = round(mean(CANCER_19, na.rm = TRUE),1),
...
ne_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Under 5", y = 6, label = "Above\nregional average"),
            color = "gray48") +
  geom_text(aes(x = "Under 5", y = -3, label = "Below\nregional average"),
            color = "gray48") +
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 2, yend = 8), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
...
# create row for region
ne_RHI_row <- ne_blkgrp_sf %>% 
  as.data.frame() %>%  
  summarize(STATE = "NEW ENGLAND",
            Mean = round(mean(RESP_19, na.rm = TRUE),2),
...
ne_blkgrp_sf %>% 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "Low Income", y = 7, label = "Above\nregional average"),
            color = "gray48") +
  geom_text(aes(x = "Low Income", y = -3, label = "Below\nregional average"),
            color = "gray48") +
  # geom_segment(aes(x = "Under 5", xend = "Under 5", y = 2, yend = 7), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
...
# create row for region
ne_PTRAF_row <- ne_blkgrp_sf %>% 
  as.data.frame() %>% 
  summarize(STATE = "NEW ENGLAND",
            Mean = round(mean(PTRAF_19, na.rm = TRUE),0),
...
ne_blkgrp_sf %>% 
  ...
  geom_point(color = "blue", size = 4, alpha = 0.8) +
  coord_flip() + xlab("") + ylab("") + 
  ggtitle("Population-Weighted Average Traffic Proximity and Volume \n(relative to New England average)") + theme_light() +
  theme(panel.grid.major.y = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank()) +
  geom_text(aes(x = Group, y = Pct + 0.2 * sign(Pct), 
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text(aes(x = "No HS Dip", y = 60, label = "Above\nregional average"),
            color = "gray48") +
  geom_text(aes(x = "No HS Dip", y = -22, label = "Below\nregional average"),
            color = "gray48") +
  # geom_segment(aes(x = "Low Income", xend = "Low Income", y = 30, yend = 70), 
  #              arrow = arrow(length = unit(0.3,"cm"))) +
  expand_limits(y = c(-35,110))
```
...
The total or sum of the products (i.e., Minority pop x PM~2.5~ concentrations) is then divided by the sum of the weights (i.e., total Minority population), so that 324.5/45 = `r round(324.5/45,2)`. The result is a weighted average PM~2.5~ concentration that is influenced by the number of minorities. 
